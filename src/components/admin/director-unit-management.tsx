...
export function DirectorUnitManagement() {
  ...
  const { unitsInCampus, availableUnits } = useMemo(() => {
    ...
    const unitsInCampus = allUnits.filter((unit) => unit.campusIds?.includes(directorCampusId));

    const availableUnits = allUnits.filter(
      (unit) =>
        !unit.campusIds?.includes(directorCampusId) && unit.name.toLowerCase().includes(searchTerm.toLowerCase()),
    );
                          ...
                          {isConfirming ? (
                            <div className="flex items-center justify-end gap-2 animate-in fade-in slide-in-from-right-2 duration-300">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  isConfirmingDelete ? handleDeleteUnit(unit) : handleRemoveUnitFromCampus(unit)
                                }
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onSelect={() => {
                                    setTimeout(() => setConfirmRemoveId(unit.id), 0);
                                  }}
                                >
                            ...
                            <TableCell className="text-right pr-6">
                              ...
                              >
                                {isSubmitting ? (
                                  ...
                                  <PlusCircle className="mr-2 h-3 w-3 mr-1" />
                                )}
                                Assign
                              </Button>
                            </TableCell>
