...
export function UserReportPrintTemplate({
  ...
  const universityPresident = signatories?.universityPresident || 'University President';
  ...
  return (
    ...
    >
      ...
      {/* INSTITUTIONAL SIGNATORIES BLOCK */}
      <div className="pt-4 border-t-2 border-slate-900 mt-6" style={{ pageBreakInside: 'avoid' }}>
        ...
        <div className="grid grid-cols-3 gap-6 text-[8pt]">
          ...
          {/* Approved By */}
          <div>
            <p className="text-slate-500 mb-8 font-medium">Noted & Approved by:</p>
            <p className="font-black uppercase text-slate-900 border-b border-black pb-1 leading-none">
              {universityPresident}
            </p>
            <p className="text-[7pt] text-slate-600 mt-1">University President</p>
            <p className="text-[6.5pt] text-slate-400 mt-0.5">Romblon State University</p>
          </div>
        </div>

        {/* SYSTEM CONTROL FOOTNOTE */}
        <div className="mt-8 pt-2 border-t border-slate-200 text-center text-[6.5pt] text-slate-500">
          ...
          Information System • Ref: {refNo} • {format(currentDate, 'yyyy-MM-dd HH:mm:ss')}
        </div>
      </div>
    </div>
  );
