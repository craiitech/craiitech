...
export function CampusSubmissionsView({
  ...
  const campusSummary = useMemo(() => {
    ...
    const yearSubmissions = allSubmissions.filter(
      (s) => s.campusId === selectedCampusId && s.year.toString() === selectedYear,
    );
    ...
    const unitPerformance = campusUnits.map((unit) => {
      ...
      const getMissing = (cycleId: 'first' | 'final') => {
        ...
        const approvedSet = new Set(
          unitSubs.filter((s) => s.cycleId === cycleId && s.statusId === 'approved').map((s) => s.reportType),
        );
  ...
  const unitData = useMemo(() => {
    ...
    const unitSubmissions = allSubmissions.filter(
      (s) => s.unitId === selectedUnitId && s.campusId === selectedCampusId && s.year.toString() === selectedYear,
    );
  ...
  const handlePrintCampusNotice = (
    type: 'Compliance' | 'Non-Compliance',
    cycleOverride?: 'first' | 'final' | 'both',
  ) => {
    ...
    try {
      const reportHtml = renderToStaticMarkup(
        type === 'Compliance' ? <CampusNoticeOfCompliance {...props} /> : <CampusNoticeOfNonCompliance {...props} />,
      );
      const printWindow = window.open('', '_blank');
      if (printWindow) {
        printWindow.document.open();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Comprehensive documentation tracking across all university campuses.
          </p>
        </div>
          ...
          {selectedUnitId && unitData && unitMap.get(selectedUnitId) ? (
            <ScrollArea className="h-full pr-4">
              <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
                  <div className="space-y-1">
                    <h3 className="font-black text-xl uppercase tracking-tight text-slate-900 dark:text-slate-100">
                      {unitMap.get(selectedUnitId)}
                    </h3>
                    <div className="flex items-center gap-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                      <span className="flex items-center gap-1.5">
                        <CalendarIcon className="h-3.5 w-3.5" /> AY {selectedYear}
                      </span>
                    </div>
                  </div>
                  <DropdownMenu>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <Chart3DDefs idPrefix="campsub3d" />

                  <Card className="lg:col-span-1 flex flex-col items-center justify-center bg-background rounded-2xl border-primary/10 shadow-lg hover:shadow-xl transition-all p-8">
                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-6 text-center">
                        ...
                        <PieChart>
                          ...
                          >
                            {unitData.chartData.map((entry, index) => (
                              <Cell
                                key={`cell-${index}`}
                                fill={index === 0 ? 'url(#campsub3d-grad-emerald)' : 'url(#campsub3d-grad-amber)'}
                                filter="url(#campsub3d-soft-depth)"
                              />
                            ))}
                          </Pie>
                        </PieChart>
                  ...
                  <div className="md:col-span-2 space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Card className="shadow-none border-dashed bg-primary/5 border-primary/20 p-5">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-2 font-medium">
                          Documents approved for this cycle.
                        </p>
                      </Card>
                      <Card
                        className={cn(
                    ...
                    <div className="p-4 bg-muted/10 border rounded-xl flex items-start gap-3">
                      ...
                      <p className="text-[10px] text-muted-foreground italic leading-tight">
                        <strong>Analytical Perspective:</strong> Data roll-up for {selectedYear}.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 bg-muted/5 border border-primary/10 rounded-2xl p-5">
                  ...
                  {isLoadingOrgStructures ? (
                    ...
                    <div className="rounded-xl border border-dashed p-6 text-center bg-white/50 dark:bg-slate-900/50">
                      <Building2 className="h-6 w-6 mx-auto mb-1 opacity-15" />
                      ...
                      {[firstCycleOrgStructure, finalCycleOrgStructure].map((record, idx) => {
                        ...
                        if (!record) {
                          return (
                            <div
                              key={idx}
                              className="rounded-xl border border-dashed p-4 text-center bg-white/40 dark:bg-slate-900/40"
                            >
                              <Badge
                                variant="outline"
                                className={`${badgeColor} px-2.5 h-4 font-black text-[8px] uppercase mb-1.5`}
                              >
                                {label}
                              </Badge>
                        ...
                        return (
                          <div
                            key={idx}
                            className="rounded-xl border bg-white dark:bg-slate-900 shadow-xs overflow-hidden"
                          >
                            <div className="bg-muted/20 border-b px-3.5 py-2 flex items-center justify-between gap-2">
                              <Badge
                                variant="outline"
                                className={`${badgeColor} px-2.5 h-4 font-black text-[8px] uppercase`}
                              >
                                {label}
                              </Badge>
                              <div className="flex items-center gap-1.5">
                                {record.isCarriedOver && (
                                  <Badge className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-none h-4 text-[7px] font-black uppercase gap-1 px-1.5">
                            ...
                            <div className="p-3.5 space-y-2.5">
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                <div className="space-y-0.5">
                                  <p className="text-[8px] font-black uppercase text-muted-foreground tracking-widest flex items-center gap-1">
                                    <Hash className="h-2 w-2" /> Revision
                                  </p>
                              ...
                              <div className="space-y-0.5">
                                <p className="text-[8px] font-black uppercase text-muted-foreground tracking-widest flex items-center gap-1">
                                  <FileBadge className="h-2 w-2" /> Uploaded By
                                </p>
                                <p className="font-bold text-[10px] text-slate-700 dark:text-slate-300 truncate">
                                  {record.submittedBy}
                                </p>
                              </div>
                              <div className="pt-2 border-t flex items-center gap-2">
                                <div className="flex-1 bg-muted/30 rounded px-2 py-1 flex items-center gap-1.5 min-w-0">
                                  <LinkIcon className="h-2.5 w-2.5 text-muted-foreground shrink-0" />
                                ...
                                <Button
                                  size="sm"
                                  variant="default"
                                  className="h-6 text-[8px] font-black uppercase shrink-0 px-2.5 bg-primary shadow-xs"
                                  onClick={() => window.open(record.googleDriveLink, '_blank')}
                                >
                                  <ExternalLink className="h-2 w-2 mr-1" /> Open PDF
                                </Button>
                              </div>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-1">
                      <Badge
                        variant="outline"
                        className="bg-blue-50 text-blue-700 border-blue-200 px-4 h-6 font-black text-[10px] uppercase"
                      >
                        1st Cycle Submission Registry
                      </Badge>
                    </div>
                    <UnitTable
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-1">
                      <Badge
                        variant="outline"
                        className="bg-green-50 text-green-700 border-green-200 px-4 h-6 font-black text-[10px] uppercase"
                      >
                        Final Cycle Submission Registry
                      </Badge>
                    </div>
                    <UnitTable
            ...
            <ScrollArea className="h-full pr-4">
              <div className="space-y-8 animate-in fade-in duration-500 pb-10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
                  <div className="space-y-1">
                    <h3 className="font-black text-2xl uppercase tracking-tight text-primary">
                      {campusMap.get(selectedCampusId)}
                    </h3>
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                      Site Performance Dashboard & bull; AY {selectedYear}
                    </p>
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        ...
                        size="sm"
                        className={cn(
                          'h-9 text-[10px] font-black uppercase bg-white shadow-sm',
                          campusSummary.avgScore >= 100
                            ? 'text-emerald-600 border-emerald-250'
                    ...
                    >
                      ...
                      {campusSummary.unitPerformance.some(
                        (u) => u.missingFirst.length > 0 || u.missingFinal.length > 0,
                      ) && (
                        <>
                          <DropdownMenuSeparator className="my-1.5" />
                          <DropdownMenuItem
                ...
                <StrategicSwotAnalysis
                  ...
                  mrOutputs={
                    mrOutputs?.filter((o) => o.assignments?.some((a) => a.campusId === selectedCampusId)) || []
                  }
                  scope="campus"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="md:col-span-1 border-primary/10 shadow-lg p-8 bg-gradient-to-br from-primary/10 to-background flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-[0.2em] text-primary/60 mb-6">
                      Site Maturity Index
                    </span>
                    <div className="relative h-40 w-40">
                      <svg className="h-full w-full" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeWidth="8"
                          strokeDasharray={`${campusSummary.avgScore * 2.51} 251.2`}
                          strokeLinecap="round"
                          fill="transparent"
                          r="40"
                          cx="50"
                        ...
                        >
                          {campusSummary.avgScore}%
                        </text>
                      </svg>
                    </div>
                    <p className="mt-6 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                      Average Across {campusSummary.totalUnits} Units
                    </p>
                  </Card>
                </div>
              </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-muted-foreground border-dashed border-2 rounded-2xl bg-muted/5 animate-in fade-in duration-500">
              ...
              <p className="text-xs max-w-xs">
                Browse the campus directory on the left to view comprehensive site performance SWOT.
              </p>
            </div>
          )}
        </div>
...
function UnitTable({
  ...
  cycleSubs: Submission[];
  onView: (id: string) => void;
  isAdmin?: boolean;
  onDeleteClick?: (sub: Submission) => void;
}) {
