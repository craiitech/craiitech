...
export function ProcedureRevisionReviewDialog({
  requestId,
  isOpen,
  onOpenChange,
  onEditClick,
}: ProcedureRevisionReviewDialogProps) {
            ...
            {request && (
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className={cn(
                    'h-7 px-3 font-black uppercase text-[10px] tracking-widest',
                    request.requestType === 'New'
                      ? 'border-blue-400 bg-blue-50 text-blue-800'
                      : 'border-purple-400 bg-purple-50 text-purple-800',
                  )}
                >
                  {request.requestType === 'New' ? 'NEW SUBMISSION' : 'REVISION REQUEST'}
                </Badge>
                <Badge className="h-7 px-4 font-black uppercase text-[10px] tracking-widest bg-primary text-white border-none shadow-md">
                  {request.status}
                </Badge>
              </div>
            )}
                    ...
                    <div className="aspect-video w-full rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-muted overflow-hidden shadow-inner relative group">
                      ...
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="sm" className="h-8 font-black uppercase text-[9px] shadow-lg" asChild>
                          <a href={request.scannedDRRFLink} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="border rounded-2xl overflow-hidden shadow-md">
                      <Table>
                        <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
                          <TableRow>
                            <TableHead className="text-[10px] font-black uppercase pl-6 w-[200px]">
                              Section / Part
                            </TableHead>
                          ...
                          {request.revisedParts.map((part, i) => (
                            <TableRow key={i} className="hover:bg-muted/10 transition-colors">
                              <TableCell className="pl-6">
                                <span className="text-xs font-black text-primary">{part.part}</span>
                              </TableCell>
                              <TableCell className="text-[11px] font-bold text-slate-700 dark:text-slate-300">
                                {part.itemNumber}
                              </TableCell>
                              <TableCell className="text-[11px] text-muted-foreground italic py-3">
                                {part.itemContents}
                              </TableCell>
                            </TableRow>
                          ))}
                  ...
                  <section className="space-y-4">
                    <div className="flex items-center justify-between border-b pb-2">
                      ...
                      <Button size="sm" className="h-8 font-black uppercase text-[9px]" asChild>
                        <a href={request.revisedManualDocxLink} target="_blank" rel="noopener noreferrer">
                          <ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Inspect Word File
                        </a>
                      </Button>
                    </div>
                        ...
                        {isAwaitingPresident ? (
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <Alert className="bg-amber-50 border-amber-200 shadow-sm">
                              ...
                              <AlertTitle className="text-xs font-black uppercase tracking-tight text-amber-800">
                                Final Executive Action Required
                              </AlertTitle>
                                ...
                                <div className="pt-4 space-y-2">
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleUpdateStatus(
                                        'Approved & Registered',
                                        'Presidential approval verified. Manual revision officially committed.',
                                      )
                                    }
                                    disabled={
                                      isProcessing ||
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleUpdateStatus(
                                        ...
                                        'Returned to unit coordinator from presidential approval stage.',
                                      )
                                    }
                                ...
                                <form className="space-y-4">
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormControl>
                                          <Textarea
                                            {...field}
                                            placeholder="Enter review feedback or required changes..."
                                            rows={4}
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                      ...
                                      onClick={() => {
                                        ...
                                        if (!c) {
                                          form.setError('comment', {
                                            type: 'manual',
                                            message: 'Feedback required for return.',
                                          });
                                          return;
                                        }
                                        handleUpdateStatus('Returned for Revision', c);
                                      }}
                                      disabled={isProcessing}
                                    >
                                    ...
                                    <Button
                                      type="button"
                                      variant="outline"
                                      className="text-rose-600 border-rose-200 font-black h-10 text-[10px] uppercase hover:bg-rose-50 gap-1.5"
                                      onClick={() => {
                                        const c = form.getValues('comment');
                                        if (!c) {
                                          form.setError('comment', {
                                  ...
                                  <Button
                                    type="button"
                                    className="w-full h-11 font-black text-[10px] uppercase bg-emerald-600 text-white hover:bg-emerald-700 gap-2 shadow-xl shadow-emerald-200"
                                    onClick={() =>
                                      handleUpdateStatus(
                                        'Awaiting Presidential Approval',
                                        form.getValues('comment') ||
                                          'Review complete. Approved by Admin. Endorsed for executive signature.',
                                      )
                                    }
                          ...
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <Alert className="bg-rose-50 border-rose-200 shadow-sm">
                              ...
                              <AlertDescription className="text-[11px] leading-relaxed font-medium text-rose-700">
                                This application has been returned for revision. Please review the comments and findings
                                under the discussion tab to see what changes are needed, then resubmit.
                              </AlertDescription>
                            </Alert>
                            {onEditClick && (
                              <Button
                          ...
                          <div className="py-20 text-center opacity-40">
                            ...
                            <p className="text-[10px] mt-2 italic px-6">
                              The Quality Assurance Office is currently evaluating this revision application.
                            </p>
                          </div>
                        )}
                      </div>
