...
const revisionRequestSchema = z.object({
  requestType: z.enum(['New', 'Revision']),
...
export function ProcedureRevisionDialog({
  ...
  const requestTypeVal = useWatch({ control: form.control, name: 'requestType' });
  ...
  const canProceed = useMemo(() => {
    ...
    if (step === 2) {
      ...
      return (
        revisedPartsList &&
        revisedPartsList.length > 0 &&
        revisedPartsList.every(
          (p) => p.part?.trim().length > 0 && p.itemNumber?.trim().length > 0 && p.itemContents?.trim().length >= 5,
        )
      );
    }
    if (step === 3) {
  ...
  useEffect(() => {
    ...
    if (active && request) {
      form.reset({
        requestType: request.requestType || 'Revision',
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const prefix = values.requestType === 'New' ? 'RSU-NEW' : 'RSU-REV';
      const controlNumber = `${prefix}-${unitCode}-${format(phDate, 'yyyyMMdd-HHmm')}`;
      ...
      if (request) {
        ...
        toast({ title: 'Application Resubmitted', description: 'Your corrections have been logged.' });
      } else {
        const requestData = {
          requestType: values.requestType,
          scannedDRRFLink: values.scannedDRRFLink.trim(),
        ...
        toast({ title: 'Application Logged', description: `Application ${controlNumber} has been sent for review.` });
      }
  ...
  const StepGuidance = ({ step }: { step: number }) => {
    ...
    if (step === 1) {
      content = (
        <div className="space-y-1">
          <p className="font-black text-slate-800 dark:text-slate-200 uppercase text-[10px] tracking-widest">
            Protocol Stage: Preparation
          </p>
          <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
            Download the template and print/fill it out. Secure the signature of the Unit Head. Ensure you also prepare
            the revised Microsoft Word version (.docx) of your procedure manual.
          </p>
        </div>
      );
    } else if (step === 2) {
      content = (
        <div className="space-y-1">
          <p className="font-black text-slate-800 dark:text-slate-200 uppercase text-[10px] tracking-widest">
            Protocol Stage: Revision Details
          </p>
          <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
            Select the exact process manual part to revise. Detail the specific item number and write a clear
            description of the modified contents.
          </p>
        </div>
      );
    } else if (step === 3) {
      content = (
        <div className="space-y-1">
          <p className="font-black text-slate-800 dark:text-slate-200 uppercase text-[10px] tracking-widest">
            Protocol Stage: Uploading Evidence
          </p>
          <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
            Provide public Google Drive links to the signed scanned DRRF and the revised manual word file (.docx). Links
            must be set to <strong>"Anyone with the link can view"</strong>.
          </p>
        </div>
      );
    } else if (step === 4) {
      content = (
        <div className="space-y-1">
          <p className="font-black text-slate-800 dark:text-slate-200 uppercase text-[10px] tracking-widest">
            Protocol Stage: Final Verification
          </p>
          <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
            Review the revision summary. Submitting will register this request into the Quality Assurance Office review
            pipeline.
          </p>
        </div>
      );
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="space-y-3">
            <FormLabel className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200">
            ...
            <FormControl>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div
                  ...
                  className={cn(
                    'p-4 rounded-xl border-2 cursor-pointer transition-all flex flex-col justify-between gap-3',
                    field.value === 'New'
                      ? 'border-primary bg-primary/5 shadow-md'
                      : 'border-slate-200 dark:border-slate-800 bg-white hover:border-primary/40',
                  )}
                >
                  <div className="flex items-center justify-between">
                    <Badge
                      variant="outline"
                      className="h-5 text-[9px] font-black uppercase border-blue-400 bg-blue-50 text-blue-800"
                    >
                      New Submission
                    </Badge>
                    {field.value === 'New' && <CheckCircle2 className="h-4 w-4 text-primary" />}
                  </div>
                  <div>
                    <p className="font-black text-xs uppercase text-slate-900 dark:text-slate-100">
                      New Procedure Manual
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-1 leading-relaxed">
                      Register a brand new procedure manual for institutional accreditation and deployment.
                    </p>
                  </div>
                </div>

                <div
                  onClick={() => field.onChange('Revision')}
                  className={cn(
                    'p-4 rounded-xl border-2 cursor-pointer transition-all flex flex-col justify-between gap-3',
                ...
                >
                  <div className="flex items-center justify-between">
                    <Badge
                      variant="outline"
                      className="h-5 text-[9px] font-black uppercase border-purple-400 bg-purple-50 text-purple-800"
                    >
                      Revision Request
                    </Badge>
                    {field.value === 'Revision' && <CheckCircle2 className="h-4 w-4 text-primary" />}
                  </div>
                  <div>
                    <p className="font-black text-xs uppercase text-slate-900 dark:text-slate-100">
                      Procedure Manual Revision
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-1 leading-relaxed">
                      Request modifications or updates to existing registered procedure manual sections via DRRF.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="space-y-4 border-t pt-6">
        <h3 className="text-base font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight flex items-center gap-3">
          <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary">
            <Download className="h-4 w-4" />
          </div>
          2. Obtain & Prepare Template (DRRF)
        </h3>
        <p className="text-sm text-muted-foreground leading-relaxed pl-11 font-medium">
          Download the official Document Registration and Revision Form (DRRF) template. Print and fill it out, ensuring
          the Unit Head signs the completed document before scanning.
        </p>
      ...
      <div className="flex items-center justify-between border-b pb-2">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">
            2. Specify Revised Manual Parts
          </h3>
        </div>
        <Button
          type="button"
          size="sm"
          onClick={() => append({ part: '', itemNumber: '', itemContents: '' })}
          ...
          >
            ...
            <CardContent className="p-4 pt-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        {dropdownParts.length > 0 ? (
                          <Select onValueChange={inputField.onChange} value={inputField.value}>
                            ...
                            <SelectContent>
                              {dropdownParts.map((part, pIdx) => (
                                <SelectItem key={pIdx} value={part} className="text-xs">
                                  {part}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          ...
                          <Input
                            {...inputField}
                            placeholder="Enter manual section (e.g. Scope)"
              ...
              <div className="md:col-span-3">
                <FormField
                  control={form.control}
                  name={`revisedParts.${index}.itemNumber`}
                  render={({ field: inputField }) => (
                    <FormItem>
                      <FormLabel className="text-[9px] font-black uppercase text-muted-foreground">
                        Item / Section No.
              ...
              <div className="md:col-span-4">
                <FormField
                  control={form.control}
                  name={`revisedParts.${index}.itemContents`}
                  render={({ field: inputField }) => (
                    <FormItem>
                      <FormLabel className="text-[9px] font-black uppercase text-muted-foreground">
                      ...
                      <FormControl>
                        <Textarea
                          {...inputField}
                          placeholder="Detail the modifications made..."
                          className="min-h-8 text-[11px] bg-white py-1"
                          rows={2}
                        />
                      </FormControl>
                      <FormMessage />
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
              ...
              <FormControl>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 h-11 border-primary/20 bg-slate-50 dark:bg-slate-800/50 shadow-inner text-xs"
                  />
                </div>
              </FormControl>
              <FormDescription className="text-[10px]">
                Provide the link to the PDF scan of the filled-out, signed DRRF.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          control={form.control}
          name="revisedManualDocxLink"
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-[10px] font-black uppercase text-slate-700 dark:text-slate-300">
                Google Drive Link: Revised Word Manual Document (.docx)
              </FormLabel>
              <FormControl>
                <div className="relative">
                  <LinkIcon className="absolute left-3 top-3 h-4 w-4 text-muted-foreground opacity-50" />
              ...
              <FormDescription className="text-[10px]">
                Provide the link to the actual revised Word manual document for text processing.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
    ...
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
      <div className="text-center space-y-3 pb-6 border-b">
        ...
        <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-slate-100">
          Verify Revision Submission
        </h3>
        <p className="text-sm text-muted-foreground max-w-lg mx-auto font-medium">
          Review the summarized parts below. Make sure both Google Drive links are fully accessible.
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 px-1">
            Revision Parts Summary ({fields.length} Items)
          </p>
        ...
        <div className="flex flex-col w-full bg-white rounded-xl border shadow-sm overflow-hidden animate-in fade-in duration-300">
          <div className="p-6 border-b bg-slate-50 dark:bg-slate-800/50 shrink-0">
            ...
            <p className="text-xs text-muted-foreground">
              {request
                ? 'Update your revision details based on comments.'
                : 'Apply to modify official procedure manual contents.'}
            </p>
          </div>
          {stepsHeader}
          ...
          <AlertDialogHeader>
            <div className="flex items-center gap-2 text-primary mb-2">
              ...
              <AlertDialogTitle className="font-black uppercase tracking-tight">
                Institutional Revision Verification
              </AlertDialogTitle>
            </div>
            <AlertDialogDescription className="text-sm font-medium leading-relaxed">
              By submitting this revision application, you certify that:
              <br />
              <br />
              1. All Google Drive links are set to <strong>"Anyone with the link can view"</strong>.<br />
              2. The DRRF is filled, signed, and scanned appropriately.
              <br />
