...
import { doc, updateDoc, serverTimestamp, writeBatch, collection, arrayUnion } from '@/firebase/firestore-wrapper';
...
export function FormRequestReviewDialog({
  requestId,
  isOpen,
  onOpenChange,
  onEditClick,
}: FormRequestReviewDialogProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl h-[92dvh] flex flex-col p-0 overflow-hidden shadow-2xl border-none">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-3">
                ...
                {request?.isDraft && (
                  <Badge className="bg-blue-600 text-white border-none h-5 px-2 text-[9px] font-black uppercase shadow-sm">
                    DRAFT REVIEW MODE
                  </Badge>
                )}
              </div>
            ...
            {request && (
              <Badge className="h-8 px-5 font-black uppercase text-[10px] tracking-widest bg-primary text-white border-none shadow-md">
                {request.status}
              </Badge>
            )}
          </div>
        ...
        {isLoading ? (
                    ...
                    <div className="aspect-video w-full rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-muted overflow-hidden shadow-inner relative group">
                      ...
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="sm" className="h-8 font-black uppercase text-[9px] shadow-lg" asChild>
                          <a href={request.scannedRegistrationFormLink} target="_blank" rel="noopener noreferrer">
                            <ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Full Source
                          </a>
                        </Button>
                      </div>
                    </div>
                  ...
                  <section className="space-y-4">
                    <div className="flex items-center justify-between border-b pb-2">
                      ...
                      <p className="text-[9px] font-bold text-muted-foreground italic">
                        Click code to inspect specific form content
                      </p>
                    </div>
                      ...
                      <Table>
                        <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
                          <TableRow>
                            ...
                            <TableHead className="text-[10px] font-black uppercase text-center w-[80px]">
                              Rev.
                            </TableHead>
                            <TableHead className="text-right text-[10px] font-black uppercase pr-6 w-[80px]">
                              Action
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                          ...
                          {request.requestedForms.map((f, i) => (
                            <TableRow
                              ...
                              className={cn(
                                ...
                                activeFormPreview?.link === f.link ? 'bg-primary/5' : 'hover:bg-muted/20',
                              )}
                              onClick={() => setActiveFormPreview({ name: f.name, link: f.link })}
                            >
                              <TableCell className="pl-6">
                                <span className="font-mono text-[10px] font-black text-primary uppercase group-hover:underline underline-offset-4 decoration-primary/30">
                                  {f.code}
                                </span>
                              </TableCell>
                              ...
                              <TableCell className="text-center">
                                <Badge
                                  variant="outline"
                                  className="h-4 text-[8px] font-black border-primary/20 bg-white"
                                >
                                  Rev {f.revision}
                                </Badge>
                              </TableCell>
                      ...
                      <div className="flex items-center justify-between border-b pb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="text-[10px] font-black uppercase tracking-widest text-emerald-700">
                            Detailed Visual Inspection: {activeFormPreview.name}
                          </h4>
                        </div>
                        <button
                          onClick={() => setActiveFormPreview(null)}
                          className="text-[9px] font-black uppercase text-muted-foreground hover:text-rose-600 transition-colors"
                        >
                          Dismiss Viewer
                        </button>
                      </div>
                      <div className="aspect-[16/10] w-full rounded-2xl border-4 border-emerald-50 bg-muted overflow-hidden shadow-2xl relative group">
                        ...
                        <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button size="sm" className="h-8 bg-emerald-600 font-black uppercase text-[9px]" asChild>
                            <a href={activeFormPreview.link} target="_blank" rel="noopener noreferrer">
                              Inspect in GDrive
                            </a>
                          </Button>
                        </div>
                        ...
                        {request.status === 'Approved & Registered' ? (
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <Alert className="bg-emerald-50 border-emerald-200 shadow-sm">
                              ...
                              <AlertTitle className="text-xs font-black uppercase tracking-tight text-emerald-800">
                                Finalized & Registered
                              </AlertTitle>
                              <AlertDescription className="text-[11px] leading-relaxed font-medium text-emerald-700">
                                This application has been successfully verified, approved, and officially enrolled in
                                the unit's active forms roster.
                              </AlertDescription>
                            </Alert>
                            {request.presidentialApprovalLink && (
                              <div className="space-y-4">
                                <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">
                                  Approved DRF Form Evidence
                                </p>
                          ...
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <Alert className="bg-rose-50 border-rose-200 shadow-sm">
                              ...
                              <AlertTitle className="text-xs font-black uppercase tracking-tight text-rose-800">
                          ...
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <Alert className="bg-amber-50 border-amber-200 shadow-sm">
                              ...
                              <AlertTitle className="text-xs font-black uppercase tracking-tight text-amber-800">
                                Final Endorsement Action Required
                              </AlertTitle>
                              ...
                              <div className="space-y-4">
                                <div className="space-y-2">
                                  ...
                                  <p className="text-[9px] text-muted-foreground italic">
                                    Provide the link to the signed and approved DRF document to finalize enrollment.
                                  </p>
                                </div>
                                ...
                                <div className="pt-4 space-y-2">
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleUpdateStatus(
                                        'Approved & Registered',
                                        'Approved DRF form verified. Forms officially enrolled in unit roster.',
                                      )
                                    }
                                    disabled={
                                      isProcessing ||
                                      !/^https?:\/\/(drive|docs)\.google\.com\//i.test(presidentialLink.trim())
                                    }
                                  >
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleUpdateStatus(
                                        'Returned for Correction',
                                        'Reverted from endorsement stage for corrective action.',
                                      )
                                    }
                                ...
                                {request.isDraft ? (
                                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl space-y-2">
                                    ...
                                    <p className="text-[10px] text-blue-700 leading-relaxed font-medium">
                                      Verify form content, layout, and coding accuracy. Strict compliance checklists and
                                      presidential approvals are bypassed for preliminary draft review.
                                    </p>
                                  </div>
                                ) : (
                                  <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                      {adminChecklistItems.map((item) => (
                                        <div
                                          key={item.id}
                                          className="flex items-center gap-3 p-3 rounded-xl border bg-white shadow-sm transition-all hover:border-primary/30 cursor-pointer"
                                        ...
                                        render={({ field }) => (
                                          <FormItem>
                                            <FormControl>
                                              <Textarea
                                                {...field}
                                                placeholder="Enter review notes or required changes..."
                                                rows={4}
                                                className="text-xs italic bg-white shadow-inner"
                                              />
                                            </FormControl>
                                            <FormMessage />
                                          </FormItem>
                                      ...
                                      {request.isDraft ? (
                                        <div className="grid grid-cols-2 gap-3">
                                          <Button
                                            type="button"
                                            variant="outline"
                                            className="text-destructive font-black h-10 text-[10px] uppercase border-destructive/20 hover:bg-destructive/5 gap-1.5"
                                            onClick={() => {
                                              const c = form.getValues('comment');
                                              if (!c) {
                                                form.setError('comment', {
                                                  type: 'manual',
                                                  message: 'Feedback required for rejection.',
                                                });
                                                return;
                                              }
                                              handleUpdateStatus('Returned for Correction', c);
                                            }}
                                            disabled={isProcessing}
                                          >
                                          ...
                                          <Button
                                            type="button"
                                            className="h-10 font-black text-[10px] uppercase bg-emerald-600 text-white hover:bg-emerald-700 gap-1.5 shadow-lg shadow-emerald-200"
                                            onClick={() => {
                                              handleUpdateStatus(
                                                'Approved & Registered',
                                                form.getValues('comment') || 'Draft cleared as satisfactory.',
                                              );
                                            }}
                                            disabled={isProcessing}
                                          >
                                            <Check className="h-3.5 w-3.5" /> CLEAR DRAFT
                                          </Button>
                                        </div>
                                      ) : (
                                        <div className="space-y-3">
                                          ...
                                          <Button
                                            type="button"
                                            className="w-full h-11 font-black text-[10px] uppercase bg-amber-500 text-amber-950 hover:bg-amber-600 gap-2 shadow-lg shadow-amber-200"
                                            onClick={() =>
                                              handleUpdateStatus(
                                                'Endorsement for Approval',
                                                form.getValues('comment') ||
                                                  'Review complete. Endorsed for final approval.',
                                              )
                                            }
                                            disabled={isProcessing || !isChecklistComplete}
                                          >
                                            <Monitor className="h-4 w-4" /> Endorse for Final Approval
                                          </Button>
                                        </div>
                                      )}
                                    </form>
          ...
          <div className="p-12 text-center text-muted-foreground font-black uppercase tracking-widest opacity-20">
