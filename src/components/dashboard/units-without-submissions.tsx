...
import { doc, writeBatch } from '@/firebase/firestore-wrapper';
import { useToast } from '@/hooks/use-toast';
import { errorEmitter } from '@/firebase/error-emitter';
import { FirestorePermissionError } from '@/firebase/errors';
import { cn, isCycleActive, getUnitEstablishedYear } from '@/lib/utils';
...
export function UnitsWithoutSubmissions({
  ...
  const incompleteSubmissionsByCampus = useMemo(() => {
    ...
    const unitsByCampus = allUnits.reduce(
      (acc, unit) => {
    ...
    return relevantCampuses
      .map((campus) => {
        const campusUnits = (unitsByCampus[campus.id] || []).filter((unit) => {
          const estYear = getUnitEstablishedYear(unit);
          if (estYear && estYear > selectedYear) return false;
          return true;
        });
        const incompleteUnits = campusUnits
          .map((unit) => {
            const unitSubmissions = allSubmissions.filter(
              (s) => s.unitId === unit.id && s.campusId === campus.id && s.year === selectedYear,
            );
            ...
            const firstCycleRegistry = unitSubmissions.find(
              (s) => s.cycleId === 'first' && s.reportType === 'Risk and Opportunity Registry',
            );
            const requiredFirst = isFirstActive
              ? firstCycleRegistry?.riskRating === 'low'
                ? TOTAL_REPORTS_PER_CYCLE - 1
                : TOTAL_REPORTS_PER_CYCLE
              : 0;

            const finalCycleRegistry = unitSubmissions.find(
              (s) => s.cycleId === 'final' && s.reportType === 'Risk and Opportunity Registry',
            );
            const requiredFinal = isFinalActive
              ? finalCycleRegistry?.riskRating === 'low'
                ? TOTAL_REPORTS_PER_CYCLE - 1
                : TOTAL_REPORTS_PER_CYCLE
              : 0;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            ...
            {isAdmin && (
              <Button
                ...
                className="h-7 text-[9px] font-black uppercase bg-white"
              >
                  ...
                  <AccordionItem value={campus.campusId} key={campus.campusId} className="border-none">
                    <AccordionTrigger className="font-bold hover:no-underline py-2 px-2 hover:bg-muted/50 rounded-md transition-colors text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="destructive" className="h-4 text-[8px] font-black">
                          {campus.incompleteUnits.length}
                        </Badge>
                      </div>
                    </AccordionTrigger>
                          ...
                          <li key={unit.id}>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate text-left">
                                  {unit.name}
                                </span>
                              </div>
                              <Badge variant="destructive" className="text-[8px] font-black h-4">
                                0%
                              </Badge>
                            </Button>
                          </li>
      ...
      <AlertDialog open={isReminderDialogOpen} onOpenChange={setIsReminderDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will post a compliance reminder to the dashboard of all users in the{' '}
              {incompleteSubmissionsByCampus.length} campus(es) with zero-approved submissions for {selectedYear}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
