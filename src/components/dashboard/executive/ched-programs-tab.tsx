...
const COPC_GRADIENTS: Record<string, string> = {
  'With COPC': 'url(#ched3d-grad-emerald)',
  'In Progress': 'url(#ched3d-grad-amber)',
  'No COPC': 'url(#ched3d-grad-rose)',
};
...
export function ChedProgramsTab({ academicPrograms, allCompliances, campuses, selectedYear }: ChedProgramsTabProps) {
  ...
  const byCampus = useMemo(() => {
    ...
    return Object.entries(campusData)
      .map(([campus, d]) => ({
        campus: campus.replace('Campus', '').trim(),
        ...d,
        'With COPC': d.withCopc,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <CardTitle className="text-sm font-black uppercase tracking-wider">COPC Compliance Status (3D)</CardTitle>
              ...
              <PieChart>
                ...
                >
                  {copcStats.map((entry) => (
                    <Cell
                      key={entry.name}
                      fill={COPC_GRADIENTS[entry.name] || COPC_COLORS[entry.name] || '#94a3b8'}
                      filter="url(#ched3d-soft-depth)"
                    />
                  ))}
                </Pie>
            ...
            <div className="flex justify-center gap-4 mt-2">
              {copcStats.map((s) => (
                <div key={s.name} className="flex items-center gap-1.5">
                  <div className="h-2.5 w-2.5 rounded-full shadow-sm" style={{ background: COPC_COLORS[s.name] }} />
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <CardTitle className="text-sm font-black uppercase tracking-wider">
              Programs by Level & COPC Status (3D)
            </CardTitle>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
          ...
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={byLevel} margin={{ top: 20, right: 20, left: 0, bottom: 0 }}>
                ...
                {Object.keys(COPC_COLORS).map((key) => (
                  <Bar
                    key={key}
                    dataKey={key}
                    fill={COPC_GRADIENTS[key] || COPC_COLORS[key]}
                    radius={[6, 6, 0, 0]}
                    maxBarSize={32}
                    filter="url(#ched3d-soft-depth)"
                  >
      ...
      <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
        <CardHeader className="border-b bg-muted/20 pb-3">
          <CardTitle className="text-sm font-black uppercase tracking-wider">
            COPC Status by Campus (3D Stacked)
          </CardTitle>
          <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <div className="flex items-start justify-between">
              ...
              <Badge variant="destructive" className="text-[9px] font-black uppercase shrink-0 shadow-sm">
                {overdueRqat} Overdue
              </Badge>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y max-h-72 overflow-y-auto">
              {rqatData.slice(0, 12).map((prog) => (
                <div
                  ...
                  className={cn(
                    ...
                    prog.isOverdue && 'bg-red-50/30 dark:bg-red-950/20',
                  )}
                >
                  <div className="min-w-0 flex-1">
                    <p
                      className="text-xs font-black uppercase text-slate-800 dark:text-slate-200 truncate"
                      title={prog.fullName}
                    >
                      {prog.name}
                    </p>
                    <p className="text-[9px] text-muted-foreground font-medium">
                      {prog.campus} • Last Visit: {prog.lastVisit}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    {prog.ncCount > 0 && (
                      <Badge variant="outline" className="text-[8px] font-black border-amber-200 text-amber-700">
                        {prog.ncCount} NC
                      </Badge>
                    )}
                    <Badge
              ...
              {rqatData.length === 0 && (
                <div className="p-8 text-center text-[11px] text-muted-foreground font-bold uppercase">
                  No RQAT data recorded
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <CardTitle className="text-sm font-black uppercase tracking-wider">
              Board Exam Performance vs National (3D)
            </CardTitle>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
            ...
            {boardPerfData.length > 0 ? (
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={boardPerfData} layout="vertical" margin={{ top: 0, right: 50, left: 10, bottom: 0 }}>
                  ...
                  <Legend wrapperStyle={{ fontSize: 10, fontWeight: 700 }} />
                  <Bar dataKey="passRate" name="RSU Pass Rate" radius={[0, 6, 6, 0]} filter="url(#ched3d-soft-depth)">
                    {boardPerfData.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={entry.passRate >= entry.national ? 'url(#ched3d-grad-emerald)' : 'url(#ched3d-grad-rose)'}
                      />
                    ))}
                    <LabelList content={<RenderBar3DLabel />} />
                  </Bar>
              ...
              <div className="h-40 flex items-center justify-center">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Add board performance records in Program Compliance
                  </p>
                </div>
              </div>
            )}
