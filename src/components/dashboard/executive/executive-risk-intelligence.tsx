...
export function ExecutiveRiskIntelligence({ risks, allUnits, campuses, selectedYear }: ExecutiveRiskIntelligenceProps) {
  ...
  const riskRegister = useMemo(() => {
    return risksOnly.map((r) => {
      ...
      const daysOpen = r.createdAt
        ? differenceInDays(new Date(), r.createdAt.toDate ? r.createdAt.toDate() : new Date(r.createdAt))
        : 0;
      const hasTreatment = !!r.postTreatment;
      const isOverdue = daysOpen > 90 && r.status !== 'Closed';
      const needsTreatment =
        !hasTreatment && (r.preTreatment?.rating === 'High' || r.preTreatment?.rating === 'Very High');

      let recommendation = '';
      if (needsTreatment) recommendation = 'URGENT: Define and implement post-treatment mitigation plan immediately.';
      else if (isOverdue)
        recommendation = 'Review and expedite closure. Risk has been open >90 days. Escalate to unit head.';
      else if (!hasTreatment && r.status !== 'Closed')
        recommendation = 'Consider developing a treatment plan to reduce residual risk.';
      else if (hasTreatment && r.status !== 'Closed')
        recommendation = 'Monitor treatment effectiveness and verify evidence for closure.';
  ...
  const objectiveRiskMap = useMemo(() => {
    const map: Record<
      string,
      { total: number; open: number; critical: number; untreated: number; avgMagnitude: number; objectives: string[] }
    > = {};
    risksOnly.forEach((r) => {
      ...
      if ((r.preTreatment?.rating === 'Very High' || r.preTreatment?.rating === 'High') && r.status !== 'Closed')
        map[key].critical++;
      if (!r.postTreatment && r.status !== 'Closed') map[key].untreated++;
      map[key].avgMagnitude += r.preTreatment?.magnitude || 0;
      if (!map[key].objectives.includes(r.objective || 'Unspecified'))
        map[key].objectives.push(r.objective || 'Unspecified');
    });
  ...
  const campusProfile = useMemo(() => {
    return campuses
      .map((c) => {
        ...
        const campusCritical = campusOpen.filter(
          (r) => r.preTreatment?.rating === 'Very High' || r.preTreatment?.rating === 'High',
        );
        const campusTreated = campusRisks.filter((r) => r.postTreatment);
        const avgMag =
          campusRisks.length > 0
            ? parseFloat(
                (campusRisks.reduce((s, r) => s + (r.preTreatment?.magnitude || 0), 0) / campusRisks.length).toFixed(1),
              )
            : 0;
        const resMag =
          campusRisks.filter((r) => r.postTreatment).length > 0
            ? parseFloat(
                (
                  campusRisks
                    .filter((r) => r.postTreatment)
                    .reduce((s, r) => s + (r.postTreatment?.magnitude || 0), 0) /
                  campusRisks.filter((r) => r.postTreatment).length
                ).toFixed(1),
              )
            : 0;
        return {
  ...
  const executiveScore = useMemo(() => {
    ...
    const untreatedCriticalRate =
      risksOnly.length > 0 ? Math.round((untreatedHigh.length / risksOnly.length) * 100) : 0;
    const avgReduction =
      riskRegister.filter((r) => r.hasTreatment).length > 0
        ? Math.round(
            riskRegister.filter((r) => r.hasTreatment).reduce((s, r) => s + r.reduction, 0) /
              riskRegister.filter((r) => r.hasTreatment).length,
          )
        : 0;
    ...
    const score = Math.max(
      ...
      Math.min(
        ...
        Math.round(
          treatmentRate * 0.25 +
            closureRate * 0.25 +
            (100 - criticalRate) * 0.2 +
            (100 - untreatedCriticalRate) * 0.2 +
            avgReduction * 0.1,
        ),
      ),
    );
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <div className="space-y-6">
        ...
        <Chart3DDefs idPrefix="execrisk3d" />
            ...
            <div className="text-center">
              <div
                className={cn(
                  ...
                  executiveScore.score >= 75
                    ? 'text-emerald-400'
                    : executiveScore.score >= 50
                      ? 'text-amber-400'
                      : 'text-red-400',
                )}
              >
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[11px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Active Alerts Requiring Executive Attention
              </h4>
            </div>
              ...
              {alertWatch.map((alert, i) => (
                ...
                >
                  <CardContent className="p-4">
                    ...
                    <p className="text-[10px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed mb-2">
                      {alert.description}
                    </p>
          ...
          {[
            {
              ...
              value: executiveScore.treatmentRate,
              icon: <ShieldAlert className="h-4 w-4" />,
              detail: `${withPostTreatment.length} of ${risksOnly.length} risks treated`,
              color: 'text-indigo-600',
            },
            {
              label: 'Closure Rate',
              value: executiveScore.closureRate,
              icon: <CheckCircle2 className="h-4 w-4" />,
              detail: `${closedRisks.length} of ${risksOnly.length} risks closed`,
              color: 'text-emerald-600',
            },
            {
              label: 'Critical Risk Ratio',
              value: 100 - executiveScore.criticalRate,
              icon: <AlertTriangle className="h-4 w-4" />,
              detail: `${criticalRisks.length} of ${risksOnly.length} are critical`,
              color: 'text-red-600',
              invert: true,
            },
            {
              label: 'Untreated Critical',
              value: 100 - executiveScore.untreatedCriticalRate,
              icon: <AlertOctagon className="h-4 w-4" />,
              detail: `${untreatedHigh.length} untreated high-risk items`,
              color: 'text-orange-600',
              invert: true,
            },
            {
              label: 'Avg Reduction',
              value: Math.min(executiveScore.avgReduction, 25),
              icon: <TrendingUp className="h-4 w-4" />,
              detail: `${executiveScore.avgReduction} pts avg magnitude reduction`,
              color: 'text-teal-600',
              max: 25,
            },
          ].map((metric, i) => (
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-1 mb-2">
                  <span className={cn('text-2xl font-black tracking-tight tabular-nums', metric.color)}>
                    {metric.invert ? 100 - metric.value : metric.value}
              ...
              <Tooltip>
                ...
                >
                  ...
                  <p className="text-slate-200">
                    Critical×3 + Untreated×2 + Open, normalized by total risks per objective. Higher score = greater
            ...
            {objectiveRiskMap.length > 0 ? (
              <div className="space-y-3">
                {objectiveRiskMap.slice(0, 10).map((obj, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 p-3 rounded-lg bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700"
                  >
                    <div
                      className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-black text-xs"
                      style={{
                        backgroundColor: obj.riskScore >= 20 ? '#fef2f2' : obj.riskScore >= 10 ? '#fff7ed' : '#f0fdf4',
                      }}
                    >
                      <span
                        className={
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 text-[8px] font-bold text-slate-500">
                        <span className={obj.open > 0 ? 'text-amber-600' : 'text-emerald-600'}>{obj.open} open</span>
                        <span className={obj.critical > 0 ? 'text-red-600' : ''}>{obj.critical} critical</span>
                        <span className={obj.untreated > 0 ? 'text-orange-600' : ''}>{obj.untreated} untreated</span>
              ...
              <p className="text-center text-xs text-muted-foreground py-8 font-bold">
                No risks registered for {selectedYear}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <CardTitle className="text-sm font-black uppercase tracking-wider flex items-center gap-2">
              ...
              Campus Risk Profile Comparison (3D)
            </CardTitle>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
          ...
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={230}>
              <BarChart data={campusProfile} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                        ...
                        {data && (
                          <div className="space-y-0.5">
                            <p className="font-bold">
                              Total: <span className="text-slate-700 dark:text-slate-300">{data.total}</span>
                            </p>
                ...
                <Bar
                  dataKey="total"
                  name="Total Risks"
                  fill="url(#execrisk3d-grad-indigo)"
                  radius={[6, 6, 0, 0]}
                  maxBarSize={28}
                  filter="url(#execrisk3d-soft-depth)"
                >
          ...
          <CardHeader className="border-b bg-muted/20 pb-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <label
                  className={cn(
                    'flex items-center gap-1 cursor-pointer text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded transition-colors',
                    riskFilter === 'all' ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-muted/80',
                  )}
                >
                  <input
                    type="radio"
                ...
                >
                  <input
                    ...
                    name="riskFilter"
                    className="sr-only"
                    checked={riskFilter === 'open'}
                    onChange={() => setRiskFilter('open')}
                  />
                  Open Risks
                </label>
                <label
                ...
                >
                  <input
                    ...
                    name="riskFilter"
                    className="sr-only"
                    checked={riskFilter === 'ongoing'}
                    onChange={() => setRiskFilter('ongoing')}
                  />
                  On-going Risks
                </label>
                <label
                ...
                >
                  <input
                    ...
                    name="riskFilter"
                    className="sr-only"
                    checked={riskFilter === 'closed'}
                    onChange={() => setRiskFilter('closed')}
                  />
                  Closed Risks
                </label>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-2 text-[8px] font-bold text-slate-500">
                        ...
                        {entry.reduction > 0 && <span className="text-emerald-600">-{entry.reduction}pts</span>}
                      </div>
                      <div
                        className={cn(
                          'p-2.5 rounded-lg border',
                          entry.needsTreatment
            ...
            {filteredRegister.length > 20 && (
              <div className="p-4 text-center border-t border-slate-100 dark:border-slate-700">
                <p className="text-[10px] text-slate-400 font-bold">
                  + {filteredRegister.length - 20} more risks — refine filters for focused view
                </p>
              </div>
            )}
          </CardContent>
            ...
            <div className="space-y-4">
              {recommendations.length > 0 ? (
                ...
                <p className="text-center text-xs text-muted-foreground py-4 font-bold">
                  No recommendations generated — risk register is empty for {selectedYear}
                </p>
              )}
            </div>
          </CardContent>
