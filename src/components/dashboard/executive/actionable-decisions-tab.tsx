...
export function ActionableDecisionsTab({
    ...
    () =>
      yearRisks.filter(
        (r) =>
          r.type === 'Risk' &&
          r.status !== 'Closed' &&
          (r.preTreatment?.rating === 'Very High' || r.preTreatment?.rating === 'High') &&
          !r.postTreatment,
      ),
    [yearRisks],
  );
  ...
  const topLevelAccredPrograms = useMemo(
    () =>
      allCompliances.filter((c) =>
        c.accreditationRecords?.some((r) => r.level?.toLowerCase().includes('iv') || r.level === '4'),
      ).length,
    [allCompliances],
  );

  const iqa_completion =
    yearSchedules.length > 0
      ? Math.round((yearSchedules.filter((s) => s.status === 'Completed').length / yearSchedules.length) * 100)
      : 0;
  const car_closure =
    cars.length > 0 ? Math.round((cars.filter((c) => c.status === 'Closed').length / cars.length) * 100) : 0;
  const risk_treatment =
    yearRisks.length > 0 ? Math.round((yearRisks.filter((r) => r.postTreatment).length / yearRisks.length) * 100) : 0;
  const ched_compliance =
    academicPrograms.filter((p) => p.isActive).length > 0
      ? Math.round(
          (academicPrograms.filter(
            (p) => p.isActive && allCompliances.find((c) => c.programId === p.id)?.ched?.copcStatus === 'With COPC',
          ).length /
            academicPrograms.filter((p) => p.isActive).length) *
            100,
        )
      : 0;
  const submission_compliance =
    yearSubs.length > 0
      ? Math.round((yearSubs.filter((s) => s.statusId === 'approved').length / yearSubs.length) * 100)
      : 0;
  const qualityScore = Math.round(
    (iqa_completion + car_closure + risk_treatment + ched_compliance + submission_compliance) / 5,
  );
  ...
  const qualityMetrics = useMemo(() => {
    ...
    const chedWithCopc = activeProgs.filter(
      (p) => allCompliances.find((c) => c.programId === p.id)?.ched?.copcStatus === 'With COPC',
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        definition:
          'Percentage of Corrective Action Requests (CARs) successfully closed and verified by the QA office.',
      ...
      },
      {
        ...
        definition:
          'Percentage of active academic offerings holding a valid CHED Certificate of Program Compliance (COPC).',
  ...
  const totalHighRisks = useMemo(
    () =>
      yearRisks.filter(
        (r) => r.type === 'Risk' && (r.preTreatment?.rating === 'Very High' || r.preTreatment?.rating === 'High'),
      ).length,
    [yearRisks],
  );
  ...
  const flags: DecisionFlag[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description:
          carsAwaitingVerification.length > 0
    ...
    [
      ...
      mandatoryOpenRecs,
      totalMandatoryRecs,
      carsAwaitingVerification,
      iqa_completion,
      yearSchedules,
      closedCarsThisYear,
      cars,
      topLevelAccredPrograms,
      allCompliances,
    ],
  );
  ...
  const gaugeData = [
    {
      name: 'Quality Score',
      value: qualityScore,
      fill: qualityScore >= 75 ? '#10b981' : qualityScore >= 50 ? '#f59e0b' : '#ef4444',
    },
  ];
  ...
  const trendData = useMemo(() => {
    ...
    > = {};
    qKeys.forEach((k) => {
      acc[k] = { subApproved: 0, subTotal: 0, carClosed: 0, carTotal: 0, riskTreated: 0, riskTotal: 0 };
    });
  ...
  const quadrants = [
    ...
    {
      id: 'not-urgent-low',
      label: 'Low Impact, Not Urgent',
      sub: 'Monitor',
      color: 'border-slate-200 dark:border-slate-700 bg-slate-50/40 dark:bg-slate-800/40',
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-white/70 font-medium mt-1">
                Synthesized quality intelligence for AY {selectedYear} — flags requiring executive action
              </p>
            </div>
            <div className="text-center">
              ...
              <div
                className={cn(
                  ...
                  qualityScore >= 75 ? 'text-emerald-300' : qualityScore >= 50 ? 'text-amber-300' : 'text-red-300',
                )}
              >
                {qualityScore}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {qualityMetrics.map((metric) => (
            <Card
              key={metric.label}
              className="bg-white border-primary/10 shadow-sm transition-all hover:shadow-md flex flex-col justify-between"
            >
              <CardContent className="p-4 flex flex-col h-full justify-between">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          aria-label={`Information for ${metric.label}`}
                        >
                  ...
                  <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden mb-3">
                    <div
                      className={cn(
                        ...
                        metric.value >= 75 ? 'bg-emerald-500' : metric.value >= 50 ? 'bg-amber-500' : 'bg-red-500',
                      )}
                ...
                <div className="mt-auto space-y-1">
                  ...
                  <p className="text-[9px] text-slate-400 font-bold leading-tight">{metric.description}</p>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <Card className="shadow-md bg-white border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            ...
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
              Quality issues plotted by impact and urgency — Eisenhower Matrix adapted for institutional quality
              management
            </CardDescription>
          </CardHeader>
            ...
            {flags.map((flag) => {
              ...
              return (
                <Card
                  key={flag.id}
                  className={cn(
                    'border shadow-sm transition-all hover:shadow-md overflow-hidden flex flex-col justify-between',
                    config.border,
                    config.bg,
                  )}
                >
                  ...
                  <CardContent className="p-4 flex flex-col justify-between h-full gap-4">
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-2.5">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h4 className="text-[11px] font-black uppercase text-slate-800 dark:text-slate-200 leading-tight truncate">
                              {flag.title}
                            </h4>
                        ...
                        <div className="text-right shrink-0">
                          <span
                            className={cn(
                              'text-2xl font-black tabular-nums block leading-none',
                              flag.severity === 'critical'
                                ? 'text-red-600'
                                : flag.severity === 'warning'
                                  ? 'text-amber-600'
                      ...
                      <div className="bg-white/80 p-3 rounded-lg border border-slate-200/50 dark:border-slate-700/50 space-y-2">
                        ...
                        <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-200/60 dark:border-slate-700/60">
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">
                            Data Label:{' '}
                            <span className="text-slate-700 dark:text-slate-300 normal-case font-extrabold">
                              {flag.breakdownLabel}
                            </span>
                          </p>
                        </div>
                    ...
                    <div
                      className={cn(
                        'border-t pt-3 flex items-start gap-2.5 mt-auto',
                        flag.severity === 'critical'
                          ? 'border-red-100/80'
                          : flag.severity === 'warning'
                            ? 'border-amber-100/80'
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl bg-white dark:bg-slate-900 border-primary/10">
          <CardHeader className="border-b bg-muted/20 pb-3">
            <CardTitle className="text-sm font-black uppercase tracking-wider flex items-center gap-2">
              ...
              Institutional Quality Trend (3D Curves) — AY {selectedYear}
            </CardTitle>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
          ...
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={230}>
              <LineChart data={trendData} margin={{ top: 15, right: 30, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" opacity={0.15} />
                <XAxis dataKey="period" tick={{ fontSize: 10, fontWeight: 700 }} />
                <YAxis domain={[0, 100]} tickFormatter={(v) => `${v}%`} tick={{ fontSize: 9, fontWeight: 'bold' }} />
                <RechartsTooltip formatter={(v: number) => `${v}%`} />
                <Legend wrapperStyle={{ fontSize: 10, fontWeight: 700 }} />
                <Line
                  type="monotone"
                  dataKey="Submission Compliance"
                  stroke="#6366f1"
                  strokeWidth={3.5}
                  dot={{ r: 5, fill: '#6366f1' }}
                  filter="url(#dec3d-soft-depth)"
                />
                <Line
                  type="monotone"
                  dataKey="CAR Closure"
