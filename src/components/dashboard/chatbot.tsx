  ...
  if (bestMatch && bestScore >= 4) {
    const ans = bestMatch.answerBlocks ? (
      <div className="space-y-2">
        <p className="font-semibold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wide">
          Detailed Guide:
        </p>
...
export function Chatbot() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const welcomeContent = (
        <div className="space-y-3">
          <p className="font-semibold">
            Good day, {userProfile?.unitName || 'esteemed colleague'}, I am your RSU Drongos EOMS Assistant — your RSU
            quality management companion.
          </p>
          {pageHelp && (
            <div className="p-3 rounded-xl bg-primary/5 border border-primary/10 mt-2 space-y-2 animate-in fade-in zoom-in duration-300">
              ...
              <p className="text-[11px] text-slate-700 dark:text-slate-300 italic">
                "I see you are currently viewing the **{pageHelp.title}** module. Would you like a step-by-step summary
      ...
      speak(`Good day, ${unitName}, I am your RSU Drongos EOMS Assistant, your RSU quality management companion.`);
    }
  ...
  const handleExplainPage = () => {
    ...
    setTimeout(() => {
      const explanationMsg = (
        <div className="space-y-3">
          <p className="font-bold text-xs uppercase text-slate-800 dark:text-slate-200 border-b pb-1">
            Procedure: {pageHelp.title}
          </p>
          ...
          <div className="space-y-3 mt-2">
            {pageHelp.steps.map((step, i) => (
              <div key={i} className="flex gap-2.5 items-start">
                <span className="h-4 w-4 shrink-0 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[9px] font-black">
                  {i + 1}
                </span>
                <div>
          ...
          {pageHelp.alert && (
            <div className="p-2.5 rounded-lg border border-destructive/20 bg-destructive/5 text-destructive text-[9px] font-medium italic mt-3">
              <span className="font-black uppercase not-italic block mb-0.5">⚠️ Alert Notice:</span>"{pageHelp.alert}"
            </div>
          )}
        </div>
      );

      setMessages((prev) => [...prev, { role: 'model', content: explanationMsg }]);
      setIsLoading(false);
    }, 800);
  };
    ...
    <>
            ...
            <Card className="w-[380px] sm:w-[440px] h-[75dvh] max-h-[600px] flex flex-col shadow-2xl border-primary/20 bg-white/80 backdrop-blur-md overflow-hidden shadow-[0_0_50px_-12px_rgba(15,23,42,0.15)] select-none">
              <CardHeader className="flex flex-row items-center justify-between bg-primary/5 border-b py-3.5 px-5 shrink-0">
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-xl bg-white flex items-center justify-center shadow-lg shadow-primary/10 overflow-hidden border border-slate-200 relative group">
                    <img
                      src="/assistant/drongos_static.png"
                      alt="RSU Drongos EOMS Assistant"
                      className="h-full w-full object-cover group-hover:hidden"
                    />
                    <img
                      src="/assistant/drongos.gif"
                      alt="RSU Drongos EOMS Assistant"
                      className="h-full w-full object-cover hidden group-hover:block"
                    />
                  </div>
                  <div>
                    <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-800 dark:text-slate-200">
                      RSU Drongos EOMS Assistant
                    </CardTitle>
                    <CardDescription className="text-[9px] font-black uppercase text-muted-foreground tracking-wider mt-0.5">
                      RSU EOMS Digital Companion
                    </CardDescription>
                  </div>
                </div>
                ...
                <ScrollArea className="flex-1 px-5 py-4" ref={scrollAreaRef}>
                  <div className="space-y-4">
                    {messages.map((message, index) => (
                      ...
                      >
                        {message.role === 'model' && (
                          <Avatar className="h-7 w-7 border shadow-sm mt-0.5 overflow-hidden bg-white relative group">
                            <img
                              src="/assistant/drongos_static.png"
                              alt="Assistant"
                              className="h-full w-full object-cover group-hover:hidden"
                        ...
                        {message.role === 'user' && (
                          <Avatar className="h-7 w-7 border shadow-sm mt-0.5">
                            <AvatarFallback className="bg-slate-100 dark:bg-slate-700 text-[10px] font-bold text-slate-700 dark:text-slate-300">
                              {userFallback}
                            </AvatarFallback>
                          </Avatar>
                        )}
                    ...
                    {isLoading && (
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="max-w-[80%] rounded-2xl rounded-tl-none px-4 py-3 text-xs bg-white border flex items-center gap-2 text-slate-500 italic shadow-sm">
                          ...
                          RSU Drongos EOMS Assistant is typing...
                        </div>
                      </div>
                    )}
      ...
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-3">
        ...
        <Button
          ...
          className="h-14 w-14 rounded-full shadow-2xl bg-white hover:scale-105 transition-all duration-300 shadow-primary/30 flex items-center justify-center border border-slate-200 overflow-hidden p-0 relative"
          size="icon"
          title="Get Help Chatbot"
          id="tour-chatbot"
        >
          {isOpen ? (
            <X className="h-5 w-5 text-slate-800 dark:text-slate-200 animate-in spin-in duration-300" />
          ) : (
            <img
              src={isButtonHovered ? '/assistant/drongos.gif' : '/assistant/drongos_static.png'}
              alt="RSU Drongos EOMS Assistant"
