...
import { AiExecutiveBriefing } from '@/components/ai/ai-executive-briefing';
...
export function ExecutiveOverview({
  ...
  const pendingSubs = useMemo(
    () => scopedSubmissions.filter((s) => Number(s.year) === Number(selectedYear) && s.statusId === 'submitted'),
  ...
  const approvedComboCount = useMemo(() => {
    const yearApproved = scopedSubmissions.filter(
  ...
  const submissionRate = useMemo(
    () => (expectedSubs > 0 ? Math.min(100, Math.round((approvedComboCount / expectedSubs) * 100)) : 0),
    [approvedComboCount, expectedSubs],
  );
  ...
  const carResolutionRate = useMemo(
  ...
  const recommendationsList = useMemo(
    () =>
      scopedCompliances.reduce((acc: any[], c) => {
  ...
  const accreditationResolutionRate = useMemo(
    () =>
      recommendationsList.length > 0
        ? Math.min(100, Math.round((closedRecs.length / recommendationsList.length) * 100))
        : 0,
    [closedRecs, recommendationsList],
  );
  ...
  // 5. CHED COPC RATE
  const copcCompliant = useMemo(
    () => scopedCompliances.filter((c) => c.ched?.copcStatus === 'With COPC'),
    [scopedCompliances],
  );
  const totalProgramsCount = useMemo(() => scopedPrograms.length, [scopedPrograms]);
  const copcComplianceRate = useMemo(
    () => (totalProgramsCount > 0 ? Math.min(100, Math.round((copcCompliant.length / totalProgramsCount) * 100)) : 0),
    [copcCompliant, totalProgramsCount],
  );

  // 6. RISK MITIGATION RATE
  const yearRisks = useMemo(
    () => scopedRisks.filter((r) => Number(r.year) === Number(selectedYear)),
    [scopedRisks, selectedYear],
  );
  const mitigatedRisks = useMemo(
    () =>
      yearRisks.filter(
  ...
  const activeMetrics = useMemo(() => {
    const metrics = [
      ...
      {
        name: 'IQA Audit Progress',
        value: iqaProgressRate,
        weight: 0.2,
        color: 'bg-indigo-500',
        active: yearSchedules.length > 0,
      },
      {
        name: 'CAR Resolution Rate',
        value: carResolutionRate,
        weight: 0.2,
        color: 'bg-rose-500',
        active: yearCars.length > 0,
      },
      {
        ...
        weight: 0.15,
        color: 'bg-amber-500',
        active: yearRisks.length > 0,
      },
      {
        name: 'CHED Program COPC',
        value: copcComplianceRate,
        weight: 0.1,
        color: 'bg-blue-500',
        active: totalProgramsCount > 0,
      },
      {
        name: 'Accreditation Performance',
        value: accreditationRate,
        weight: 0.1,
        color: 'bg-teal-500',
        active: scopedPrograms.filter((p) => p.isActive).length > 0,
      },
    ];
  ...
  const statusDetails = useMemo(() => {
    if (eomsQualityScore >= 85)
      return {
        text: 'Optimal Compliance',
        desc:
          scope === 'unit'
            ? 'The department exhibits outstanding alignment with EOMS standards.'
            : scope === 'campus'
              ? 'The campus exhibits outstanding alignment with EOMS standards.'
              : 'The University exhibits outstanding alignment with EOMS standards.',
        color: 'text-emerald-600 border-emerald-200 bg-emerald-50',
      };
    if (eomsQualityScore >= 65)
      return {
        text: 'Good standing',
        desc:
          scope === 'unit'
            ? 'Department operations are stable, though minor gaps require correction.'
            : scope === 'campus'
              ? 'Campus is stable, though minor compliance gaps require correction.'
              : 'System is stable, though minor compliance gaps require correction.',
        color: 'text-blue-600 border-blue-200 bg-blue-50',
      };
    if (eomsQualityScore >= 45)
      return {
        text: 'Needs Improvement',
        desc: 'Active non-conformities and missing evidence registries detected.',
    ...
    return {
      ...
      desc:
        scope === 'unit'
          ? 'Substantial compliance deficiencies found in this department.'
          : scope === 'campus'
            ? 'Substantial compliance deficiencies found across campus departments.'
            : 'Substantial compliance deficiencies found across several sites.',
  ...
  const complianceStandings = useMemo(() => {
    ...
    const getUnitExpected = (unitId: string) => {
      const unitSubs = (submissions || []).filter(
        (s) => s.unitId === unitId && Number(s.year) === Number(selectedYear),
      );
      let total = 0;
      if (isFirstActive) {
        const firstRor = unitSubs.find(
    ...
    const getUnitApprovedComboCount = (unitId: string) => {
      const unitApproved = (submissions || []).filter(
        (s) => s.unitId === unitId && Number(s.year) === Number(selectedYear) && s.statusId === 'approved',
      );
    ...
    if (scope === 'campus' && scopeId) {
      const campusUnits =
        units?.filter(
          (u) =>
            u.campusIds?.includes(scopeId) &&
            u.name?.toLowerCase() !== 'internal quality audit' &&
            u.name?.toLowerCase() !== 'iqa',
        ) || [];
      return campusUnits
        .map((unit) => {
      ...
      return campuses
        .map((campus) => {
          const campusUnits =
            units?.filter(
              (u) =>
                u.campusIds?.includes(campus.id) &&
                u.name?.toLowerCase() !== 'internal quality audit' &&
                u.name?.toLowerCase() !== 'iqa',
            ) || [];
          let totalApproved = 0;
  ...
  const insights = useMemo(() => {
    ...
    if (submissionRate === 100) {
      ...
      analysis.push(
        `Submission Compliance is critical (${submissionRate}%) due to multiple missing evidence registries.`,
      );
    ...
    if (yearSchedules.length > 0) {
      ...
      if (iqaProgressRate === 100) {
        ...
        analysis.push(
          `Internal quality audits are lagging heavily with only ${iqaProgressRate}% completed (${completedAudits.length}/${yearSchedules.length}).`,
        );
        recommendations.push(
          `Expedite the ${pendingAudits} pending internal quality audits (IQA) to document compliance findings.`,
        );
      }
    }
    ...
    if (yearCars.length > 0) {
      ...
      if (carResolutionRate === 100) {
        ...
        analysis.push(
          `A high volume of unresolved Corrective Action Requests (${openCars} open CARs) is holding back compliance health.`,
        );
        recommendations.push(
          `Follow up on the ${openCars} open CARs and request verification evidence from responsible offices.`,
        );
      }
    }
    ...
    if (yearRisks.length > 0) {
      ...
      if (riskControlRate === 100) {
        ...
        analysis.push(
          `Risk registers are largely untreated with only ${riskControlRate}% mitigated (${mitigatedRisks.length}/${yearRisks.length}).`,
        );
        recommendations.push(
          `Ensure mitigation actions and treatment evidence are logged for the ${unmitigated} open risks.`,
        );
      }
    }
    ...
    if (totalProgramsCount > 0) {
      ...
      if (copcComplianceRate === 100) {
        ...
        if (missingCopcCount > 0) {
          recommendations.push(
            `Apply for CHED COPC certificate for the remaining ${missingCopcCount} academic program(s).`,
          );
        }
      } else {
        analysis.push(
          `A significant number of programs (${missingCopcCount} out of ${totalProgramsCount}) lack CHED COPC certificates.`,
        );
        recommendations.push(
          `Initiate CHED quality reviews to obtain COPC status for the ${missingCopcCount} program(s) in need.`,
        );
      }
    }
    ...
    if (recommendationsList.length > 0 || scopedPrograms.filter((p) => p.isActive).length > 0) {
      if (accreditationRate === 100) {
        ...
        analysis.push(
          `Accreditation performance is strong at ${accreditationRate}%, combining level achievement and gap resolution.`,
        );
      } else if (accreditationRate >= 50) {
        analysis.push(
          `Accreditation performance is at ${accreditationRate}%. Programs need improvement in accreditation level or gap closure.`,
        );
        const openRecs = recommendationsList.length - closedRecs.length;
        if (openRecs > 0)
          recommendations.push(
            `Action and close the remaining ${openRecs} pending mandatory accreditation survey recommendations.`,
          );
      } else {
        analysis.push(
          `Accreditation performance is at ${accreditationRate}%, requiring significant improvement in program accreditation levels and gap resolution.`,
        );
        const openRecs = recommendationsList.length - closedRecs.length;
        if (openRecs > 0)
          recommendations.push(
            `Resolve and document compliance evidence for the ${openRecs} pending mandatory accreditation gaps.`,
          );
      }
    }
    ...
    if (eomsQualityScore >= 85) {
      ...
      overallExplanation =
        'The system is in good standing, but overall performance is capped by minor outstanding quality actions.';
    } else {
  ...
  const bottlenecks = useMemo(() => {
    ...
    // 2. High Risks
    const highRisks = yearRisks.filter(
      (r) => r.status !== 'Closed' && (r.preTreatment?.rating === 'high' || r.preTreatment?.rating === 'critical'),
    );
    ...
    if (complianceStandings && complianceStandings.length > 0) {
      ...
      if (topSites.length > 0) {
        strengthsList.push({
          ...
          desc: `${topSites
            .map((s) => s.name.replace('Campus', '').trim())
            .slice(0, 3)
            .join(', ')} achieved 100% EOMS submission compliance.`,
          tag: '[Site Excellence]',
          icon: ShieldCheck,
        });
      }
      ...
      if (lowSites.length > 0) {
        weaknessesList.push({
          ...
          desc: `${lowSites
            .map((s) => s.name.replace('Campus', '').trim())
            .slice(0, 3)
            .join(', ')} have submission rates below 40% and need support.`,
  ...
  return (
    <div className="space-y-6">
      {/* ADMIN-ONLY: Executive AI Briefing (local WebLLM) */}
      <AiExecutiveBriefing
        contextData={{
          scope,
          eomsQualityScore,
          ...
          copcComplianceRate,
          accreditationRate,
          openCars: yearCars.length - closedCars.length,
          pendingAudits: yearSchedules.length - completedAudits.length,
          openRisks: yearRisks.length - mitigatedRisks.length,
          missingCopc: totalProgramsCount - copcCompliant.length,
        }}
      />
        ...
        >
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-sm font-black uppercase tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-2">
                  ...
                  {scope === 'unit'
                    ? 'Unit EOMS Quality Score'
                    : scope === 'campus'
                      ? 'Campus EOMS Health Score'
                ...
                <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-0.5">
                  {scope === 'unit'
                    ? 'Departmental Quality Performance Index'
                    : scope === 'campus'
                      ? 'Site Quality Performance Index'
              ...
              <Badge
                variant="outline"
                className={cn(
                  'text-[9px] font-black uppercase px-2.5 py-1 border rounded-full shadow-xs',
                  statusDetails.color,
                )}
              >
                {statusDetails.text}
          ...
          <CardContent className="pt-6 pb-6 flex flex-col sm:flex-row items-center gap-8">
            ...
            <div className="relative flex items-center justify-center shrink-0 w-[180px] h-[180px]">
              <svg className="w-full h-full transform -rotate-95" viewBox="0 0 100 100">
                {/* Background Ring */}
                <circle cx="50" cy="50" r="40" stroke="hsl(var(--primary) / 0.1)" strokeWidth="8" fill="transparent" />
                {/* Progress Ring */}
                <circle
                  cx="50"
                  cy="50"
          ...
          <div className="border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 p-6 space-y-4">
            ...
            <div className="space-y-2">
              <h5 className="text-[9px] font-black uppercase text-slate-400 tracking-wider">
                Required Actions to Improve Score
              </h5>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5 list-none">
                {insights.recommendations.map((rec, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-2 text-[10px] font-bold text-slate-600 dark:text-slate-400"
                  >
                ...
                <div className="space-y-2 pt-4 border-t border-slate-100 dark:border-slate-700">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-500 tracking-wider">
                    ...
                    <span className="font-extrabold">
                      {currentStartIndex + 1}-{currentStartIndex + visibleStandings.length} of{' '}
                      {complianceStandings.length}
                    </span>
                  </div>
                  <Slider
                  ...
                  return (
                    <div key={idx} className="space-y-1 group">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Badge className="bg-emerald-100 text-emerald-700 border-none h-4 px-1.5 text-[8px] font-black">
                          {item.tag}
                        </Badge>
                      </div>
                    ...
                    <div key={idx} className="space-y-1 group">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          {item.priority === 'High' && (
                            <Badge variant="destructive" className="h-4 px-1 text-[7px] font-black uppercase">
                              Critical
                            </Badge>
                          )}
                          <Badge className="bg-rose-100 text-rose-700 border-none h-4 px-1.5 text-[8px] font-black">
                            {item.tag}
                          </Badge>
                        </div>
                      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <Card className="border-primary/10 bg-white/50 dark:bg-slate-800/50 backdrop-blur-xs shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
          <CardHeader className="p-4 pb-2">
            ...
            <CardTitle className="text-xs font-black uppercase text-slate-900 dark:text-slate-100 mt-2">
              IQA Schedule
            </CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-1 space-y-2">
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
              {yearSchedules.length} <span className="text-[10px] text-muted-foreground font-bold">Planned</span>
            </div>
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase">
              ...
              <span className="text-slate-900 dark:text-slate-100 font-extrabold">
                {completedAudits.length} ({iqaProgressRate}%)
              </span>
            </div>
        ...
        <Card className="border-primary/10 bg-white/50 backdrop-blur-xs shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
          ...
          <CardContent className="p-4 pt-1 space-y-2">
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
              {yearCars.length} <span className="text-[10px] text-muted-foreground font-bold">Issued</span>
            </div>
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase">
              <span>Closed</span>
              <span className="text-slate-900 dark:text-slate-100 font-extrabold">
                {closedCars.length} ({carResolutionRate}%)
              </span>
            </div>
        ...
        <Card className="border-primary/10 bg-white/50 dark:bg-slate-800/50 backdrop-blur-xs shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
          ...
          <CardContent className="p-4 pt-1 space-y-2">
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
              {accreditationRate}% <span className="text-[10px] text-muted-foreground font-bold">Score</span>
            </div>
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase">
                ...
                <div className="flex flex-wrap gap-1">
                  {accreditationByLevel.undergrad.map((item) => (
                    <Badge
                      key={item.level}
                      variant="outline"
                      className="text-[7px] font-black px-1.5 py-0 rounded-full bg-white"
                    >
        ...
        <Card className="border-primary/10 bg-white/50 backdrop-blur-xs shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
          ...
          <CardContent className="p-4 pt-1 space-y-2">
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
              {yearRisks.length} <span className="text-[10px] text-muted-foreground font-bold">Identified</span>
            </div>
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase">
              <span>Mitigated</span>
              <span className="text-slate-900 dark:text-slate-100 font-extrabold">
                {mitigatedRisks.length} ({riskControlRate}%)
              </span>
            </div>
        ...
        <Card className="border-primary/10 bg-white/50 backdrop-blur-xs shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
          ...
          <CardContent className="p-4 pt-1 space-y-2">
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
              {totalProgramsCount} <span className="text-[10px] text-muted-foreground font-bold">Programs</span>
            </div>
            <div className="flex justify-between items-center text-[9px] text-slate-500 font-bold uppercase">
              <span>With COPC</span>
              <span className="text-slate-900 dark:text-slate-100 font-extrabold">
                {copcCompliant.length} ({copcComplianceRate}%)
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {bottlenecks.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex gap-3 items-start p-3 bg-white border border-rose-100 rounded-xl shadow-xs"
                  >
                    <div
                      className={cn(
