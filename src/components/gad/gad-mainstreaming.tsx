...
export function GADMainstreaming({ units, selectedYear }: GADMainstreamingProps) {
  ...
  const checklistRef = useMemoFirebase(
    () => (firestore && userProfile ? doc(firestore, 'gadMainstreaming', checklistId) : null),
    [firestore, userProfile, checklistId],
  );
  ...
  const handleToggle = async (itemId: string) => {
    ...
    if (!isAdmin && isGadCoordinator && userProfile.unitId !== activeUnitId) {
      toast({
        title: 'Access Denied',
        description: 'You can only update mainstreaming status for your assigned unit.',
        variant: 'destructive',
      });
      return;
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100dvh-20rem)] overflow-hidden">
      ...
      <Card
        className={cn(
          'lg:col-span-1 flex flex-col overflow-hidden shadow-sm border-primary/10',
          isGadCoordinator && 'hidden lg:hidden',
        )}
      >
      ...
      <Card
        className={cn(
          'flex flex-col overflow-hidden shadow-lg border-primary/10',
      ...
      >
                    ...
                    <div className="space-y-2">
                      {PCW_MAINSTREAMING_CRITERIA.filter((c) => c.category === category).map((item) => (
                        <div
                          ...
                          className={cn(
                            ...
                            currentScores[item.id]
                              ...
                              : 'bg-background border-slate-100 dark:border-slate-700 hover:border-primary/20',
                          )}
                        ...
                        >
                          ...
                          <div className="flex-1 space-y-1">
                            <p
                              className={cn(
                                ...
                                currentScores[item.id] ? 'text-emerald-900' : 'text-slate-700 dark:text-slate-300',
                              )}
                            >
                              {item.label}
                            </p>
                          ...
                          <ChevronRight
                            className={cn(
                              'h-4 w-4 shrink-0 mt-1 opacity-0 group-hover:opacity-100 transition-opacity',
                              currentScores[item.id] ? 'text-emerald-400' : 'text-slate-300',
                            )}
                          />
                        </div>
                      ))}
        ...
        <CardFooter className="p-4 bg-slate-50 dark:bg-slate-800/50 border-t flex justify-between items-center px-8">
          <div className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground uppercase">
            ...
            Last Update:{' '}
            {checklist?.updatedAt ? format(checklist.updatedAt.toDate(), 'PPP p') : 'Pending Initial Assessment'}
          </div>
