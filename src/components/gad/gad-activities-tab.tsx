  ...
  const onSubmit = async (values: z.infer<typeof activitySchema>) => {
    ...
    try {
      ...
      if (editingActivity) {
        ...
        toast({ title: 'Event Registered', description: 'Participant data and evidence link successfully logged.' });
      }
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[9px] font-bold text-muted-foreground">
                        {unitMap.get(a.implementingUnitId)}
                      </span>
                      {a.driveLink && (
                        <a
                          href={a.driveLink}
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-40 text-center opacity-20">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">
                    No events registered for {selectedYear}
                  </p>
                </TableCell>
              </TableRow>
            )}
                ...
                <form id="activity-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
                  <section className="space-y-6">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2 border-b pb-2">
                      <Info className="h-4 w-4" /> 1. Activity Context
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Official Activity Code</FormLabel>
                            <FormControl>
                              <Input
                                {...field}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Event Title</FormLabel>
                            <FormControl>
                              <Input
                                {...field}
                                placeholder="e.g. Gender Sensitivity Seminar"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Host Site</FormLabel>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Implementing Office</FormLabel>
                            <Select
                              onValueChange={field.onChange}
                              value={field.value}
                              disabled={!isAdmin && !!userProfile?.unitId}
                            >
                              <FormControl>
                                <SelectTrigger className="h-11 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <section className="space-y-6 pt-6 border-t border-dashed">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2 border-b pb-2">
                      <Users className="h-4 w-4" /> 2. Participant Headcount (SDD)
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="p-6 rounded-2xl bg-slate-900 border-none flex flex-col items-center justify-center text-white">
                        ...
                        <span className="text-[10px] font-bold opacity-40 mt-1 uppercase tracking-tighter">
                  ...
                  <section className="space-y-6 pt-6 border-t border-dashed">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2 border-b pb-2">
                      <LayoutList className="h-4 w-4" /> 3. Sectoral Distribution
                    </h4>
                      ...
                      {sectors.map((sector) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <Badge
                              variant="secondary"
                              className="h-5 text-[8px] font-black bg-white border-none shadow-sm"
                            >
                              {(form.watch(`sectors.${sector}.male`) || 0) +
                                (form.watch(`sectors.${sector}.female`) || 0)}{' '}
                              TOTAL
                            </Badge>
                          </div>
                  ...
                  <section className="space-y-6 pt-6 border-t border-dashed">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2 border-b pb-2">
                      <Target className="h-4 w-4" /> 4. Outcome & Budget
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormField
                        control={form.control}
                        name="actualBudgetUsed"
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Actual Budget Expended (₱)</FormLabel>
                            <FormControl>
                              <Input
                                type="number"
                                {...field}
                                className="h-11 bg-emerald-50/30 border-emerald-100 font-mono font-black"
                              />
                            </FormControl>
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-xs font-black uppercase">Actual Output Status</FormLabel>
                            <FormControl>
                              <Input
                                {...field}
                                placeholder="e.g. Conducted sensitivity workshop"
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-black uppercase">
                            Variance / Implementation Remarks
                          </FormLabel>
                          <FormControl>
                            <Textarea
                              {...field}
                              rows={3}
                              placeholder="Explain any deviations from the original plan..."
                              className="bg-slate-50 dark:bg-slate-800/50 italic"
                            />
                          </FormControl>
                        </FormItem>
                      )}
                  ...
                  <section className="space-y-6 pt-6 border-t border-dashed">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-blue-600 flex items-center gap-2 border-b pb-2">
                      <FolderGit2 className="h-4 w-4" /> 5. Documentation & Evidence (Google Drive)
                    </h4>
                    <FormField
                      control={form.control}
                      name="driveLink"
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className="text-[10px] italic text-muted-foreground">
                            Paste the public Google Drive URL containing event photos, signed attendance sheets,
                            activity reports, and verification documents. Saved directly to Firestore.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
          ...
          <DialogFooter className="p-6 border-t bg-slate-50 dark:bg-slate-800/50 shrink-0 gap-2 sm:gap-0">
            <div className="flex w-full items-center justify-between">
              ...
              <Button
                ...
                className="min-w-[200px] shadow-xl shadow-primary/20 font-black uppercase text-[10px] h-11"
              >
                {isSubmitting ? (
