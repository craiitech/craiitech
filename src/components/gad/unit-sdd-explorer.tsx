      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        ...
        <CollapsibleContent className="animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-6 border-t bg-white dark:bg-slate-900">
            ...
            <Chart3DDefs idPrefix="unitsdd3d" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {unitData.map((unit) => (
                <Card
                  key={unit.id}
                  className="shadow-lg hover:shadow-xl rounded-2xl border-slate-100 dark:border-slate-700 hover:border-primary/20 transition-all flex flex-col group bg-white dark:bg-slate-800"
                >
                  <CardHeader className="p-3 border-b bg-slate-50/50 dark:bg-slate-800/50 group-hover:bg-primary/5 transition-colors text-center">
                    <CardTitle className="text-[10px] font-black uppercase truncate" title={unit.name}>
                      {unit.name}
                    </CardTitle>
                  </CardHeader>
                      ...
                      <ResponsiveContainer>
                        <PieChart>
                          ...
                          <Pie
                            ...
                            labelLine={false}
                          >
                            {unit.chartData.map((e, j) => (
                              <Cell
                                key={j}
                                fill={
                                  j === 0
                                    ? 'url(#unitsdd3d-grad-indigo)'
                                    : j === 1
                                      ? 'url(#unitsdd3d-grad-rose)'
                                      : 'url(#unitsdd3d-grad-amber)'
                                }
                                filter="url(#unitsdd3d-soft-depth)"
                              />
                            ))}
                          </Pie>
                        </PieChart>
              ...
              {unitData.length === 0 && (
                <div className="col-span-full py-20 text-center opacity-20 border-2 border-dashed rounded-2xl">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest">
                    No matching unit data found for {selectedYear}
                  </p>
                </div>
              )}
            </div>
          ...
          <CardFooter className="bg-muted/5 border-t py-3 px-8">
            <div className="flex items-start gap-3">
              ...
              <p className="text-[9px] text-muted-foreground italic leading-tight">
                <strong>Admin Oversight:</strong> This explorer provides an automated roll-up of all students and
                faculty mapped to academic units. Use this to verify that unit-level GPB planning is consistent with
                their actual sex-disaggregated population.
              </p>
            </div>
          </CardFooter>
        </CollapsibleContent>
