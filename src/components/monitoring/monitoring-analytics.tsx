...
const GRADIENT_COLORS = [
  'url(#mon3d-grad-indigo)',
  'url(#mon3d-grad-sky)',
  'url(#mon3d-grad-emerald)',
  'url(#mon3d-grad-amber)',
  'url(#mon3d-grad-violet)',
];
...
export function MonitoringAnalytics({ records, campuses, units, isLoading, selectedYear }: MonitoringAnalyticsProps) {
  const analytics = useMemo(() => {
    ...
    const campusChartData = Object.entries(campusPerformance)
      .map(([id, data]) => ({
    ...
    const unitLeaderboard = Object.entries(unitPerformance)
      .map(([id, data]) => ({
        id,
    ...
    records.forEach((r) => {
      r.observations.forEach((o) => {
        let cat = '';
        if (['Operational Plan', 'Procedure Manual', 'ROR', 'ROA'].some((s) => o.item.includes(s)))
          cat = 'Documentation';
        else if (['Signages', 'Policy', 'Organizational Structure'].some((s) => o.item.includes(s)))
          cat = 'Transparency';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <Chart3DDefs idPrefix="mon3d" />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-white to-primary/5 dark:from-slate-900 dark:to-slate-850 border-primary/15 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs uppercase font-bold text-slate-600 dark:text-slate-400">
              Visits in {selectedYear}
            </CardTitle>
          </CardHeader>
        ...
        <Card className="bg-gradient-to-br from-white to-emerald-50/40 dark:from-slate-900 dark:to-slate-850 border-emerald-200 dark:border-emerald-800/40 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs uppercase text-emerald-700 dark:text-emerald-400 font-bold">
              Avg. Compliance
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
        ...
        <Card className="bg-gradient-to-br from-white to-amber-50/40 dark:from-slate-900 dark:to-slate-850 border-amber-200 dark:border-amber-800/40 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs uppercase text-amber-700 dark:text-amber-400 font-bold">
              Total Findings
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
        ...
        <Card className="bg-gradient-to-br from-white to-blue-50/40 dark:from-slate-900 dark:to-slate-850 border-blue-200 dark:border-blue-800/40 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs uppercase text-blue-700 dark:text-blue-400 font-bold">Top Unit</CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="flex flex-col shadow-lg rounded-2xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900">
          <CardHeader className="py-4 bg-muted/10 border-b">
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-5 w-5 text-indigo-600" />
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                On-Site Category Quality Radar
              </CardTitle>
            </div>
          </CardHeader>
            ...
            <ChartContainer config={{}} className="h-[280px] w-full">
              <ResponsiveContainer>
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={analytics.radarData}>
                  ...
                  <PolarAngleAxis dataKey="subject" tick={{ fontSize: 9.5, fontWeight: 'bold', fill: '#64748b' }} />
                  <Radar
                    name="Maturity"
                    dataKey="A"
                    stroke="#6366f1"
                    strokeWidth={3}
                    fill="url(#mon3d-grad-indigo)"
                    fillOpacity={0.65}
                    filter="url(#mon3d-soft-depth)"
                  />
        ...
        <Card className="flex flex-col lg:col-span-2 shadow-lg rounded-2xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900">
          <CardHeader className="flex flex-row items-center justify-between py-4 bg-muted/10 border-b">
            <div className="flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-rose-600" />
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Top Monitoring Findings & Gaps
              </CardTitle>
            </div>
            <Badge variant="destructive" className="h-6 px-3 font-black text-[10px] uppercase shadow-sm">
              TOTAL: {analytics.criticalCount}
            </Badge>
          </CardHeader>
        ...
        <Card className="flex flex-col shadow-lg rounded-2xl border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900">
          ...
          <CardContent className="pt-6 flex-1">
            <ChartContainer config={{}} className="h-[300px] w-full">
              <ResponsiveContainer>
                <BarChart data={analytics.campusChartData} margin={{ top: 25, right: 20, left: 0, bottom: 5 }}>
                  ...
                  />
                  <RechartsTooltip content={<ChartTooltipContent />} />
                  <Bar dataKey="rate" radius={[6, 6, 0, 0]} barSize={44} filter="url(#mon3d-soft-depth)">
                    <LabelList content={<RenderBar3DLabel />} />
                    {analytics.campusChartData.map((_, index) => (
        ...
        <Card className="border-primary/10 shadow-lg overflow-hidden flex flex-col">
          <CardHeader className="bg-primary/5 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Monitoring Excellence Leaderboard
              </CardTitle>
            </div>
          </CardHeader>
                  ...
                  <div key={unit.id} className="flex items-center gap-4 p-4 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Badge variant="outline" className="bg-green-50 text-green-700 h-5 text-[10px] font-black">
                          {unit.rate}% Score
                        </Badge>
                      </div>
                      <div className="flex items-center gap-4 text-[10px] text-muted-foreground font-medium uppercase tracking-wider">
                        <span className="flex items-center gap-1">
                          <School className="h-3 w-3" /> {unit.campus}
                        </span>
                        <span className="flex items-center gap-1">
                          <User className="h-3 w-3" /> OIC: {unit.oic}
                        </span>
                      </div>
                    </div>
                  </div>
