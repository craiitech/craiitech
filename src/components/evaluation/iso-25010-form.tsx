  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Thank you! The software quality audit has been securely recorded.',
      });
      onSuccess?.();
      onOpenChange(false);
      form.reset();
        ...
        <DialogHeader className="p-4 sm:p-6 border-b bg-slate-50 dark:bg-slate-800/50 shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden sm:flex gap-2 items-center">
              <Badge variant="outline" className="h-6 font-black bg-white border-primary/20 text-primary">
                AY 2025
              </Badge>
              <Badge
                                ...
                                <FormItem className="space-y-3">
                                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                                    <div className="space-y-1 flex-1">
                                      <FormLabel className="text-sm sm:text-base font-black text-slate-800 dark:text-slate-200 tracking-tight">
                                        {sub.name}
                                      </FormLabel>
                                    ...
                                    <div className="h-6 w-[90px] shrink-0 text-center">
                                      <Badge
                                        className={cn(
                                          'h-6 px-4 font-black uppercase border-none text-[10px] w-full transition-colors duration-200',
                                          field.value
                                            ? cn(
                                                LIKERT_OPTIONS.find((o) => o.value === field.value)?.bg,
                                                LIKERT_OPTIONS.find((o) => o.value === field.value)?.color,
                                              )
                                            : 'invisible bg-slate-100 dark:bg-slate-700 text-slate-400',
                                        )}
                                      >
                                        {field.value
                                            ...
                                            >
                                              <span
                                                className={cn(
                                                  ...
                                                  field.value === opt.value ? 'text-primary' : 'text-slate-400',
                                                )}
                                              >
                                                {opt.value}
                                              </span>
                                              <span
                                                className={cn(
                                                  ...
                                                  field.value === opt.value ? 'text-primary' : 'text-slate-500',
                                                )}
                                              >
                                                {opt.label}
                                              </span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                          Concluding Remarks for the System Maturity Report.
                        </p>
                      </div>
                    </div>
