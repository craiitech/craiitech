...
export function AuditorOfflineManager() {
  ...
  const handleDownloadForOffline = async () => {
    ...
    try {
      ...
      const filteredScheds =
        selectedSite === 'university-wide' ? allScheds : allScheds.filter((s: any) => s.campusId === selectedSite);
      ...
      try {
        ...
        try {
          ...
          // Ignore flight manifest fetch error
        }
  ...
  const handleExportWorkspace = async () => {
    if (!firestore || mirrorStatus === 'none') {
      toast({
        ...
        description: 'Create a local mirror before exporting.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const toggleNetworkLock = async (forceOffline: boolean) => {
    ...
    if (forceOffline && isOnline && mirrorStatus === 'expired') {
      toast({
        variant: 'destructive',
        title: 'Mandatory Refresh',
        description: 'Your mirror is expired. You must synchronize while online before locking.',
      });
      return;
    }
    if (forceOffline) {
      ...
      try {
        ...
        toast({ title: 'Cloud Sync Delay', variant: 'destructive' });
      } finally {
        setIsSyncing(false);
      }
    }
        ...
        <Card className="w-full max-w-xl border-destructive border-4 shadow-[0_0_50px_rgba(255,0,0,0.3)] bg-white scale-110">
          <CardHeader className="text-center space-y-4 pb-2 bg-destructive/10 border-b-2 border-destructive">
            ...
            <CardTitle className="text-3xl font-black uppercase text-destructive animate-emergency-flash">
              DEEP APPLICATION MIRRORING ACTIVE
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-8 px-10 pb-10 space-y-8">
            ...
            <p className="text-[11px] font-bold leading-relaxed text-center text-destructive uppercase">
              STRICT ISOLATION: ALL INTERACTIONS BLOCKED WHILE LOCKING APPLICATION CODE AND INSTITUTIONAL REGISTRY INTO
              LOCAL STORAGE.
            </p>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-primary/70">
                Mirror institutional data and application code for 100% offline stability.
              </CardDescription>
            </div>
            <div className="flex items-center gap-3">
              <Badge
                variant={mirrorStatus === 'found' ? 'default' : mirrorStatus === 'expired' ? 'destructive' : 'outline'}
                className={cn(
                  'h-7 px-3 font-black uppercase text-[9px] gap-2 shadow-sm',
                  mirrorStatus === 'found'
              ...
              <Badge
                variant={isNetworkDisabled ? 'destructive' : 'outline'}
                className={cn(
                  'h-7 px-3 font-black uppercase text-[9px] gap-2',
              ...
              <div className="p-5 rounded-2xl bg-white border border-primary/20 shadow-sm space-y-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <h4 className="text-xs font-black uppercase text-slate-800 dark:text-slate-200">
                      1. Selective Site Mirroring
                    </h4>
                    <p className="text-[10px] text-muted-foreground italic">
                      Sync: {lastDownload ? format(lastDownload, 'PP p') : 'Never'}
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    ...
                    className="h-8 px-3 font-black uppercase text-[9px] bg-white border-primary/20 text-primary gap-1.5"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    disabled={isDownloading || isNetworkDisabled}
                  >
                ...
                <Button
                  ...
                  className={cn(
                    'w-full h-10 font-black uppercase text-[10px] gap-2',
                    isOnline && mirrorStatus === 'expired' && 'bg-amber-600 hover:bg-amber-700',
                  )}
                >
              ...
              <div className="p-5 rounded-2xl bg-white border border-indigo-100 shadow-sm space-y-4">
                ...
                {isNetworkDisabled ? (
                  <Button
                    ...
                    className="w-full h-10 border-indigo-200 text-indigo-700 font-black uppercase text-[10px] hover:bg-indigo-50"
                  >
                    {isSyncing ? (
                ...
                <div className="p-2 bg-muted/20 rounded-lg border border-dashed text-[9px] text-muted-foreground leading-tight font-medium italic">
                  Forces the portal to rely purely on the local disk cache.
                </div>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-white border border-blue-100 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    disabled={isExporting || mirrorStatus === 'none'}
                    className="h-10 border-blue-200 text-blue-700 font-black uppercase text-[10px] shadow-sm"
                  >
                    <Share2 className="h-3 w-3 mr-1.5" /> EXPORT
                  </Button>
                  <Button
                    variant="outline"
                ...
                <div className="p-2 bg-blue-50/50 rounded-lg border border-blue-100 text-[9px] text-blue-800 italic leading-tight">
                  Transfer your data mirror to a FIELD LAPTOP or DIFFERENT BROWSER via USB.
                </div>
              </div>
            </div>
          </CardContent>
