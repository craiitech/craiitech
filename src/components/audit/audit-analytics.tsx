...
import { CampusStatsPrintTemplate, CampusStatEntry } from './campus-stats-print-template';
...
export function AuditAnalytics({
  ...
  const analytics = useMemo(() => {
    ...
    const campusStatsMap: Record<string, CampusStatEntry> = {};
    campuses.forEach((c) => {
      campusStatsMap[c.id] = {
        campusId: c.id,
        campusName: c.name,
    ...
    if (!campusStatsMap['university-wide']) {
      campusStatsMap['university-wide'] = {
        campusId: 'university-wide',
    ...
    yearSchedules.forEach((s) => {
      ...
      if (!campusStatsMap[cid]) {
        campusStatsMap[cid] = {
          campusId: cid,
          campusName: campusMap.get(cid) || 'Institutional',
          totalAudited: 0,
          completedAudited: 0,
          ofiCount: 0,
          ncCount: 0,
      ...
      campusStatsMap[cid].totalAudited++;
      if (s.status === 'Completed') {
        campusStatsMap[cid].completedAudited++;
      }
    });

    yearFindings.forEach((f) => {
      const s = yearSchedules.find((sch) => sch.id === f.auditScheduleId);
      if (!s) return;
      const cid = s.campusId || 'university-wide';
      if (!campusStatsMap[cid]) return;
      if (f.type === 'Observation for Improvement') {
        campusStatsMap[cid].ofiCount++;
      } else if (f.type === 'Non-Conformance') {
        campusStatsMap[cid].ncCount++;
      } else if (f.type === 'Compliance') {
        if (campusStatsMap[cid].complianceCount !== undefined) {
          campusStatsMap[cid].complianceCount!++;
        }
      }
    });

    const campusStats = Object.values(campusStatsMap)
      .filter((cs) => cs.totalAudited > 0 || cs.ofiCount > 0 || cs.ncCount > 0)
      .sort((a, b) => b.ncCount - a.ncCount || b.ofiCount - a.ofiCount || b.totalAudited - a.totalAudited);
  ...
  const handlePrintAssignments = () => {
    if (!analytics?.auditorData.length) {
      toast({
        ...
        description: 'There are no active auditor assignments for the selected year.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handlePrintReceivingForm = () => {
    if (!analytics?.auditorData.length) {
      toast({
        title: 'No Assignments',
        description: 'There are no active auditors found for this year.',
        variant: 'destructive',
      });
      return;
    }
    try {
      ...
      const reportHtml = renderToStaticMarkup(
        <AuditReceivingPrintTemplate auditors={auditorsList} year={selectedYear} />,
      );
  ...
  const handlePrintAuditorSchedule = () => {
    if (!analytics?.yearSchedules.length) {
      toast({
        title: 'No Schedule',
        description: 'There are no sessions scheduled for the selected year.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handlePrintCampusStats = () => {
    if (!analytics?.campusStats.length) {
      toast({
        title: 'No Data',
        description: 'There are no campus audit statistics found for the selected year.',
        variant: 'destructive',
      });
      return;
    }
    try {
      const reportHtml = renderToStaticMarkup(
        <CampusStatsPrintTemplate
          campusStats={analytics.campusStats}
          year={selectedYear}
          qaoDirector={signatories?.qaoDirector}
          leadAuditorName={analytics.leadAuditorName}
        />,
      );
      triggerPrint(reportHtml, `Campus_Audit_Statistics_AY${selectedYear}`);
    } catch (e) {
      console.error(e);
    }
  };
      ...
      return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-800 p-3 rounded-lg shadow-md text-xs">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 dark:text-slate-100">
              {data.name}
            </span>
          </div>
          <div className="space-y-1 font-semibold text-slate-600 dark:text-slate-400">
            <p className="text-[10px] uppercase">
              Findings:{' '}
              <span className="text-slate-900 dark:text-slate-100 dark:text-white font-black">{data.value}</span>
            </p>
            <p className="text-[10px] uppercase">
              Proportion: <span className="text-slate-900 dark:text-slate-100 dark:text-white font-black">{pct}%</span>
            </p>
          </div>
        </div>
      );
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-800 p-3 rounded-lg shadow-md text-xs space-y-2">
          <p className="font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 dark:text-slate-100 border-b pb-1">
            {label} Processes
          </p>
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 dark:border-slate-800 p-3 rounded-lg shadow-md max-w-[280px] text-xs">
          ...
          <p className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-100 mt-1 leading-normal">
            {data.title}
          </p>
          {data.description && (
            <p className="text-[10px] text-muted-foreground mt-1 leading-normal italic font-medium line-clamp-3">
              {data.description}
            </p>
          )}
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <Card className="shadow-md border-primary/10 overflow-hidden bg-card">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/50 dark:bg-slate-800/20">
                    ...
                    <TableHead className="text-[9px] font-black uppercase text-indigo-700 tracking-wider text-center">
                      Total Findings
                    </TableHead>
                ...
                <TableBody>
                  {yearlyIqaPerformance.map((row) => {
                    ...
                    return (
                      <TableRow
                        key={row.year}
                        className="hover:bg-slate-50/80 transition-all border-b group text-center"
                      >
                        ...
                        <TableCell className="tabular-nums font-black text-sm text-indigo-600">
                          {row.findings}
                        </TableCell>
                        <TableCell>
                          <span
                        ...
                        <TableCell className="tabular-nums font-black text-sm text-emerald-600">
                          {row.compliance}
                        </TableCell>
                        <TableCell className="tabular-nums font-black text-sm text-slate-400">{row.na}</TableCell>
                        <TableCell className="text-right pr-6 tabular-nums font-black text-sm text-slate-700 dark:text-slate-300">
                          {row.auditors}
                        </TableCell>
                      </TableRow>
                    );
          ...
          <CardFooter className="bg-muted/5 border-t py-2.5 px-6">
            <div className="flex items-center gap-2 text-[9px] text-muted-foreground">
              ...
              Metrics aggregated across all audit plans, schedules, and findings. Year-over-year change shown as
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card className="bg-primary/5 border-primary/10 shadow-sm flex flex-col justify-between min-h-[140px] hover:shadow-md transition-shadow">
          ...
          <CardContent className="px-5 pb-4 pt-1 flex items-baseline justify-between">
            ...
            <Badge
              variant="outline"
              className="text-[8px] font-black tracking-widest uppercase bg-white border-primary/20 text-primary"
            >
        ...
        <Card className="bg-emerald-50 border-emerald-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:shadow-md transition-shadow">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-[10px] font-black uppercase text-emerald-800 tracking-widest">
              Audit Completion
            </CardTitle>
        ...
        <Card className="bg-blue-50 border-blue-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:shadow-md transition-shadow">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-[10px] font-black uppercase text-blue-800 tracking-widest">
              Audit Engagement
            </CardTitle>
          ...
          <CardContent className="px-5 pb-4 pt-1 flex items-baseline justify-between">
            ...
            <Badge
              variant="outline"
              className="text-[8px] font-black tracking-widest uppercase bg-white border-blue-200 text-blue-700"
            >
        ...
        <Card className="bg-purple-50 border-purple-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:shadow-md transition-shadow">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-[10px] font-black uppercase text-purple-800 tracking-widest">
              Auditor Sex Balance
            </CardTitle>
          ...
          <CardContent className="px-5 pb-4 pt-1 flex items-baseline justify-between">
            ...
            <Badge
              variant="outline"
              className="text-[8px] font-black tracking-widest uppercase bg-white border-purple-200 text-purple-700"
            >
              {Math.round(
                (analytics.auditorSexCounts.Female /
                  (analytics.auditorSexCounts.Male + analytics.auditorSexCounts.Female || 1)) *
                  100,
              )}
              % Female
            </Badge>
        ...
        <Card className="bg-rose-50 border-rose-100 shadow-sm flex flex-col justify-between min-h-[140px] hover:shadow-md transition-shadow">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-[10px] font-black uppercase text-rose-800 tracking-widest">
              Critical Gaps
            </CardTitle>
          ...
          <CardContent className="px-5 pb-4 pt-1 flex items-baseline justify-between">
            ...
            <Badge className="text-[8px] font-black tracking-widest uppercase bg-rose-600 text-white border-none">
              {analytics.totalFindings > 0 ? Math.round((analytics.counts.NC / analytics.totalFindings) * 100) : 0}%
              Ratio
            </Badge>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Institutional Finding Lifecycle
              </CardTitle>
            </div>
            <CardDescription className="text-xs text-muted-foreground mt-1">
              Proportionate distribution of QMS audit outcomes (Compliance, OFI, Non-Conformance, N/A) for institutional
              parity tracking.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="shadow-lg border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="bg-muted/10 border-b py-4">
            ...
            <CardDescription className="text-xs text-muted-foreground mt-1">
              Process audit profiles across Management, Operations, and Support frameworks to isolate systemic
                ...
                <BarChart data={analytics.categoryFindingsData} margin={{ right: 30 }}>
                  ...
                  <Bar dataKey="NC" stackId="a" fill={COLORS.NC}>
                    <LabelList
                      dataKey="NC"
                      position="center"
                      style={{ fontSize: '9px', fontWeight: '900', fill: 'white' }}
                  ...
                  <Bar dataKey="OFI" stackId="a" fill={COLORS.OFI}>
                    <LabelList
                      dataKey="OFI"
                      position="center"
                      style={{ fontSize: '9px', fontWeight: '900', fill: 'black' }}
                      formatter={(val: number) => (val > 0 ? val : '')}
                    />
                  </Bar>
        ...
        <Card className="shadow-lg border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="bg-primary/5 border-b py-4 flex flex-row items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <CardDescription className="text-xs text-muted-foreground mt-1">
                Comparative audit volume showing completed versus assigned QMS schedules per auditor for workload
                tracking.
              </CardDescription>
            </div>
            <div>
              <Button
                onClick={handlePrintAuditorsList}
                size="sm"
                variant="outline"
                className="h-9 px-4 font-black uppercase text-[10px] tracking-widest bg-white border-primary/20 text-primary gap-2 shadow-sm shrink-0"
              >
          ...
          <CardFooter className="bg-muted/5 border-t py-3 px-6">
            <p className="text-[9px] text-muted-foreground italic text-center w-full">
              Tracking workload distribution and session finalization per auditor.
            </p>
          </CardFooter>
        </Card>
        ...
        <Card className="shadow-lg border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="bg-muted/10 border-b py-4">
            ...
            <CardDescription className="text-xs text-muted-foreground mt-1">
              Standard clause exposure ranking, isolating ISO 21001:2018 clauses with high frequency of
              non-conformances.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-8 flex-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="shadow-lg border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="bg-muted/10 border-b py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <ShieldAlert className="h-5 w-5 text-destructive" />
                <CardTitle className="text-sm font-black uppercase tracking-tight text-destructive">
                  Unit Finding Hotspots & Campus Stats
                </CardTitle>
              </div>
              <CardDescription className="text-xs text-destructive/70">
                Prioritized department directory with open non-conformances needing immediate Corrective Action Request
                (CAR) resolution.
              </CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              <div className="flex border rounded-md p-0.5 bg-background">
                <button
                  type="button"
                  onClick={() => setHotspotTab('hotspots')}
                  className={cn(
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                className="h-8 px-3 font-black uppercase text-[10px] tracking-widest bg-white border-destructive/20 text-destructive hover:bg-destructive/10 gap-1.5 shadow-sm"
              >
        ...
        <Card className="shadow-lg border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="bg-muted/10 border-b py-4 flex flex-row items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 justify-end">
              <Button
                onClick={handlePrintUnitSchedule}
                size="sm"
                variant="outline"
              ...
              <Button
                onClick={handlePrintAuditorSchedule}
                size="sm"
                variant="outline"
                className="h-9 px-4 font-black uppercase text-[10px] tracking-widest bg-white border-indigo-200 text-indigo-700 gap-2 shadow-sm"
              >
                <CalendarDays className="h-4 w-4" /> Auditor Sched.
              </Button>
              <Button
                onClick={handlePrintReceivingForm}
                size="sm"
                variant="outline"
                className="h-9 px-4 font-black uppercase text-[10px] tracking-widest bg-emerald-50 border-emerald-200 text-emerald-700 gap-2 shadow-sm"
              >
                ...
                {analytics.auditorData.map((auditor, aIdx) => (
                  <div key={aIdx} className="p-4 hover:bg-muted/10 transition-colors">
                    <div className="flex items-center justify-between">
                      <h4 className="font-black text-slate-900 dark:text-slate-100 uppercase text-[11px] tracking-tight">
                        {auditor.name}
                      </h4>
                      <Badge variant="secondary" className="h-4 text-[8px] font-black">
                        {auditor.count} ASSIGNMENTS
                      </Badge>
                    </div>
                    <div className="pl-4 mt-2 text-[10px] text-muted-foreground italic truncate">
                      {auditor.assignments.map((a) => a.unitName).join(', ')}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="p-4 bg-muted/5 border rounded-xl">
        <div className="flex items-start gap-4">
          ...
          <p className="text-[9px] text-muted-foreground leading-relaxed italic">
            <strong>Administrative Guide:</strong> This dashboard provides institutional oversight of the Internal
            Quality Audit framework. Use the "Print Assignments" registry to issue official notices to the audit team.
            Non-conformance hotspots should be prioritized for Corrective Action Request (CAR) issuance in the main QA
            module.
          </p>
        </div>
      </div>
