...
export function AdminAuditView() {
  ...
  const auditPlansQuery = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'auditPlans')) : null),
    [firestore],
  );
  const { data: auditPlans, isLoading: isLoadingPlans } = useCollection<AuditPlan>(auditPlansQuery);

  const schedulesQuery = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'auditSchedules')) : null),
    [firestore],
  );
  ...
  const hasEvidence = (scheduleId: string) => {
    ...
    const hasSummary = schedule
      ? !!(schedule.summaryCommendable || schedule.summaryCompliance || schedule.summaryOFI || schedule.summaryNC)
      : false;
  ...
  const handlePrintIndividualTemplate = (schedule: AuditSchedule, withData: boolean = false) => {
    ...
    const scheduleFindings = withData ? findings?.filter((f) => f.auditScheduleId === schedule.id) || [] : [];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground font-medium">
                Analyze results and manage the institutional audit itinerary.
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                disabled={isLoadingClauses && !isoClauses}
                className="h-9 mt-5 shadow-lg shadow-primary/20 font-black uppercase text-[10px] tracking-widest"
              >
          ...
          <ScrollArea className="w-full">
            <TabsList className="bg-muted p-1 border shadow-sm w-fit h-10 animate-tab-highlight rounded-md">
              ...
              <TabsTrigger
                ...
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8 data-[state=active]:bg-purple-600 data-[state=active]:text-white"
              >
              ...
              <TabsTrigger
                value="results"
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8 data-[state=active]:bg-indigo-600 data-[state=active]:text-white"
              >
                <ClipboardCheck className="h-3.5 w-3.5" /> Audit Results & CAR Bridge
              </TabsTrigger>
              <TabsTrigger
                value="reporting"
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8 data-[state=active]:bg-emerald-600 data-[state=active]:text-white"
              >
                <Printer className="h-3.5 w-3.5" /> Audit Reporting
              </TabsTrigger>
              <TabsTrigger
                value="portfolios"
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8 data-[state=active]:bg-[#1B6535] data-[state=active]:text-white"
              >
            ...
            <CardHeader className="bg-muted/30 border-b py-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-black uppercase tracking-widest">
                  University Internal Quality Audit (IQA) Registry
                </CardTitle>
                <Badge variant="outline" className="h-5 text-[10px] font-black bg-white uppercase">
                  AY {selectedYear} Session Log
                </Badge>
              </div>
            </CardHeader>
                  ...
                  <div className="w-full sm:w-[250px]">
                    <Select value={reportUnitFilter} onValueChange={setReportUnitFilter}>
                      ...
                      <SelectContent className="bg-white border-primary/20">
                        ...
                        {units
                          ?.filter((u) => reportCampusFilter === 'all' || u.campusIds?.includes(reportCampusFilter))
                          .map((u) => (
                            <SelectItem
                              key={u.id}
                              value={u.id}
              ...
              {isLoading ? (
                ...
                <div className="py-20 text-center opacity-30 flex flex-col items-center gap-2 bg-white/50 border rounded-xl">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest text-slate-500">
                    No completed IQAs match the filters for AY {selectedYear}
                  </p>
                </div>
              ) : (
                  ...
                  <Table>
                    <TableHeader className="bg-muted/40">
                      <TableRow>
                        ...
                        <TableHead className="text-center text-[10px] font-black uppercase pr-6">
                          Printing Options
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {completedSchedules.map((schedule) => {
                        ...
                        const auditorName = auditorUser
                          ...
                          : schedule.auditorName || 'Assigned Auditor';
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 text-center">
                  ...
                  <h3 className="font-bold text-xs uppercase tracking-wider text-slate-500">No Auditors Registered</h3>
                ...
                <div className="divide-y divide-slate-100">
                  {auditors.map((auditor) => {
                    ...
                    return (
                      <div
                        key={auditor.id}
                        className="p-6 hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors"
                      >
                                ...
                                {auditor.portfolios.map((portfolio) => (
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      asChild
                                    >
                                      <a href={portfolio.googleDriveLink} target="_blank" rel="noopener noreferrer">
      ...
      {/* Dialogs */}
      <AuditPlanDialog
        isOpen={isPlanDialogOpenState}
        onOpenChange={setIsPlanDialogOpenState}
        plan={editingPlan}
        campuses={campuses || []}
      />
      ...
      {isScheduleDialogOpen && selectedPlanForScheduling && (
        <AuditScheduleDialog
          ...
          topManagement={
            users?.filter(
              (u) => u.role?.toLowerCase().includes('president') || u.role?.toLowerCase().includes('director'),
            ) || []
          }
        />
      )}
