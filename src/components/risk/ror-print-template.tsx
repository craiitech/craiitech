 ...
 */
export function RORPrintTemplate({
  risks,
  unitName,
  campusName,
  year,
  ...
  const isFinal = cycle
    ? cycle === 'final'
    : risks.some((r) => r.status === 'Closed' || (r.postTreatment && r.postTreatment.evidence));
  ...
  return (
    ...
    >
      ...
      <div
        style={{ textAlign: 'center', marginBottom: '20px', borderBottom: '2px solid black', paddingBottom: '10px' }}
      >
        <p style={{ margin: '0', fontSize: '10pt', fontWeight: 'bold', textTransform: 'uppercase' }}>
          Republic of the Philippines
        </p>
        <h1 style={{ margin: '5px 0', fontSize: '18pt', fontWeight: '900', textTransform: 'uppercase' }}>
        ...
        <div style={{ marginTop: '15px' }}>
          ...
          >
            RISK AND OPPORTUNITY REGISTER (ROR)
          </h2>
          <div
            style={{
              display: 'flex',
          ...
          >
            <span>
              FISCAL YEAR: <span style={{ textDecoration: 'underline', padding: '0 10px' }}>{year}</span>
            </span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div
                style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                  backgroundColor: !isFinal ? 'black' : 'transparent',
                }}
              >
      ...
      >
        ...
        <div>
          Updated as of:{' '}
          <span style={{ textDecoration: 'underline', marginLeft: '10px' }}>{format(new Date(), 'MMMM d, yyyy')}</span>
        </div>
      </div>
      ...
      >
        <thead>
          <tr style={{ backgroundColor: '#f9fafb' }}>
            <th style={{ border: '1px solid black', padding: '4px', fontSize: '8pt', width: '10%' }}>
              OBJECTIVE & ORIGIN
            </th>
            <th style={{ border: '1px solid black', padding: '4px', fontSize: '8pt', width: '13%' }}>
              RISK (R) / OPPORTUNITY (O) DESCRIPTION AND CAUSES
            </th>
            <th style={{ border: '1px solid black', padding: '4px', fontSize: '8pt', width: '9%' }}>
              CURRENT CONTROLS/ SITUATION
            </th>
        ...
        <tbody style={{ fontSize: '8pt' }}>
          {/* RISKS SECTION */}
          <tr
            style={{ backgroundColor: '#f3f4f6', fontWeight: '900', textTransform: 'uppercase', textAlign: 'center' }}
          >
            <td colSpan={16} style={{ border: '1px solid black', padding: '4px' }}>
              I. Risks
            </td>
          </tr>
          {riskEntries.map((r) => {
              ...
              <tr key={r.id}>
                <td style={{ border: '1px solid black', padding: '4px', verticalAlign: 'top' }}>
                  {(rowCampus || rowUnit) && (
                    ...
                    >
                      {rowCampus ? `🏛️ ${rowCampus}` : ''}
                      {rowCampus && rowUnit ? ' • ' : ''}
                      {rowUnit ? `🏢 ${rowUnit}` : ''}
                    </div>
                  )}
                  <div style={{ fontWeight: 'bold', color: '#0f172a' }}>{r.objective}</div>
                </td>
                <td style={{ border: '1px solid black', padding: '4px', verticalAlign: 'top', fontWeight: 'bold' }}>
                  {r.description}
                </td>
                ...
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {r.preTreatment.likelihood}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {r.preTreatment.consequence}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center', fontWeight: '900' }}>
                  {r.preTreatment.magnitude}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center', fontWeight: '900' }}>
                  {r.preTreatment.rating?.charAt(0)}
                </td>
                ...
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {safeDate(r.targetDate)}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {isFinal ? r.postTreatment?.dateImplemented || '' : ''}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', verticalAlign: 'top', fontSize: '7pt' }}>
                  {isFinal ? r.monitoringScore || '' : ''}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {isFinal ? r.postTreatment?.likelihood || '' : ''}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center' }}>
                  {isFinal ? r.postTreatment?.consequence || '' : ''}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center', fontWeight: '900' }}>
                  {isFinal ? r.postTreatment?.magnitude || '' : ''}
                </td>
                <td style={{ border: '1px solid black', padding: '2px', textAlign: 'center', fontWeight: '900' }}>
                  {isFinal ? r.postTreatment?.rating?.charAt(0) || '' : ''}
                </td>
              </tr>
            );
          })}
          ...
          {opportunityEntries.length === 0 && (
            <tr>
              ...
              >
                No Opportunity entries recorded.
              </td>
            </tr>
          )}
        </tbody>
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ borderBottom: '2px solid black', paddingBottom: '2px' }}>&nbsp;</div>
