...
export function RiskDashboard({ risks, isLoading, selectedYear }: RiskDashboardProps) {
  const analytics = useMemo(() => {
    ...
    const now = new Date();
    const highActive = risks.filter(
      (r) => r.type === 'Risk' && r.status !== 'Closed' && r.preTreatment.rating === 'High',
    ).length;
    const mediumActive = risks.filter(
      (r) => r.type === 'Risk' && r.status !== 'Closed' && r.preTreatment.rating === 'Medium',
    ).length;
    const lowActive = risks.filter(
      (r) => r.type === 'Risk' && r.status !== 'Closed' && r.preTreatment.rating === 'Low',
    ).length;
    ...
    const overdueCount = risks.filter(
      (r) =>
        r.status !== 'Closed' &&
        r.targetDate &&
        (r.targetDate instanceof Timestamp ? r.targetDate.toDate() : new Date(r.targetDate)) < now,
    ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <Chart3DDefs idPrefix="riskdash3d" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 animate-in fade-in duration-500">
        <Card className="bg-violet-50 border-violet-100 shadow-sm relative overflow-hidden">
          <CardHeader className="pb-1.5 pt-3 px-4">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-violet-700">
              Weighted Exposure
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
            <div className="text-2xl font-black text-violet-700 tabular-nums">{analytics.weightedScore}</div>
            <p className="text-[7.5px] font-bold text-violet-500 uppercase tracking-tighter mt-1">
              High (3pt) + Med (2pt) + Low (1pt)
            </p>
          </CardContent>
        </Card>
        <Card className="bg-rose-50 border-rose-100 shadow-sm relative overflow-hidden">
          <CardHeader className="pb-1.5 pt-3 px-4">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-rose-700">
              Open Critical Risks
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
            <div className="text-2xl font-black text-rose-600 tabular-nums">{analytics.openCount}</div>
            <p className="text-[7.5px] font-bold text-rose-400 uppercase tracking-tighter mt-1">
              Staged in Active analysis
            </p>
          </CardContent>
        </Card>
        <Card
          ...
          <CardHeader className="pb-1.5 pt-3 px-4">
            <CardTitle
              className={cn(
                ...
                analytics.overdueCount > 0 ? 'text-red-700' : 'text-slate-600 dark:text-slate-400',
              )}
            >
              Overdue Actions
            </CardTitle>
          ...
          <CardContent className="pb-3 px-4">
            <div
              className={cn(
                ...
                analytics.overdueCount > 0 ? 'text-red-600' : 'text-slate-500',
              )}
            >
            ...
            <p
              className={cn(
                'text-[7.5px] font-bold uppercase tracking-tighter mt-1',
                analytics.overdueCount > 0 ? 'text-red-500' : 'text-slate-400',
              )}
            >
              Stalled past target date
            </p>
          </CardContent>
        </Card>
        <Card className="bg-emerald-50 border-emerald-100 shadow-sm relative overflow-hidden">
          ...
          <CardContent className="pb-3 px-4">
            ...
            <p className="text-[7.5px] font-bold text-emerald-500 uppercase tracking-tighter mt-1">
        ...
        <Card className="bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
          <CardHeader className="pb-1.5 pt-3 px-4">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-slate-700 dark:text-slate-300">
              Total Entries
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col w-full bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              <Target className="h-5 w-5 text-primary" />
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Institutional Objective Vulnerability (3D)
              </CardTitle>
            </div>
          </CardHeader>
          <CardContent className="pt-6 flex-1">
            <ChartContainer
              config={{
                risks: { label: 'Risks', color: 'hsl(var(--destructive))' },
                opportunities: { label: 'Opportunities', color: 'hsl(var(--chart-2))' },
              }}
              className="h-[300px] w-full"
            >
              <ResponsiveContainer>
                <BarChart data={analytics.objectiveData} layout="vertical" margin={{ left: 20, right: 35 }}>
                  ...
                  <Bar
                    dataKey="risks"
                    stackId="a"
                    fill="url(#riskdash3d-grad-rose)"
                    filter="url(#riskdash3d-soft-depth)"
                  >
                    <LabelList
                      dataKey="risks"
                      position="center"
                  ...
                  <Bar
                    ...
                    stackId="a"
                    fill="url(#riskdash3d-grad-emerald)"
                    radius={[0, 6, 6, 0]}
                    filter="url(#riskdash3d-soft-depth)"
                  >
                    <LabelList
              ...
              {analytics.upcomingDeadlines.map((r, idx) => (
                <div key={idx} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate" title={r.description}>
                      {r.description}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      <Badge
                        variant="secondary"
                        className="h-4 text-[8px] font-black uppercase bg-primary/5 text-primary border-none"
                      >
                        {r.responsiblePersonName || 'No Lead'}
                      </Badge>
        ...
        <Card className="shadow-md border-blue-100 bg-blue-50/5 flex flex-col">
          <CardHeader className="bg-blue-50/20 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight text-blue-800">
                Low-Risk Vigilance Feed
              </CardTitle>
            </div>
            <CardDescription className="text-[10px]">
              Monitoring triggers to prevent low risks from escalating.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0 flex-1">
            <div className="divide-y">
              {analytics.watchlist.map((r, idx) => (
                <div key={idx} className="p-4 space-y-2 hover:bg-blue-50/10 transition-colors">
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate block max-w-[200px]"
                      title={r.description}
                    >
                      {r.description}
                    </span>
                    <Badge
                      variant="outline"
                      className="h-4 text-[7px] font-black bg-blue-50 border-blue-200 text-blue-700 uppercase tracking-tighter shrink-0"
                    >
      ...
      <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-emerald-100 bg-emerald-50/10 overflow-hidden">
        <CardHeader className="bg-emerald-50 border-b py-4">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-black uppercase tracking-tight text-emerald-800">
              Verified Risk Reduction Profile (3D)
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent className="pt-8">
          {analytics.closureEfficiencyData.length > 0 ? (
            <ChartContainer config={{}} className="h-[350px] w-full">
              <ResponsiveContainer>
                <BarChart data={analytics.closureEfficiencyData} margin={{ bottom: 40, top: 10 }}>
                  ...
                  <Bar
                    dataKey="Initial"
                    fill="url(#riskdash3d-grad-indigo)"
                    radius={[6, 6, 0, 0]}
                    barSize={22}
                    filter="url(#riskdash3d-soft-depth)"
                  >
