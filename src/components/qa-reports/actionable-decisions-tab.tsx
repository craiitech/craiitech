...
export function ActionableDecisionsTab({ campuses, units }: ActionableDecisionsTabProps) {
  ...
  const handleNotify = async (output: ManagementReviewOutput) => {
    if (!firestore) return;
    setNotifyingId(output.id);
    try {
      const targetUnitIds: string[] = [];
      const unitNamesList: string[] = [];

      (output.assignments || []).forEach((a) => {
        if (a.unitId && !targetUnitIds.includes(a.unitId)) {
          targetUnitIds.push(a.unitId);
        }
        const unitName = unitMap.get(a.unitId) || a.unitId;
        const campusName = campusMap.get(a.campusId) || a.campusId;
        unitNamesList.push(`${unitName} (${campusName})`);
      });
      ...
      toast({
        ...
        description: err?.message || 'Could not send notification to responsible units.',
        variant: 'destructive',
      });
    } finally {
      setNotifyingId(null);
  ...
  const reviewsQuery = useMemoFirebase(
    () => (firestore ? collection(firestore, 'managementReviews') : null),
    [firestore],
  );
  ...
  const actionEntrySchema = z.object({
    description: z
      .string()
      .min(10, 'Please provide a detailed description of the action taken (minimum 10 characters).'),
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Monitoring and implementation of institutional management directives.
          </p>
        </div>
                  ...
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow>
                        ...
                        <TableHead>
                          <Button
                            variant="ghost"
                            className="p-0 h-auto text-[10px] font-black uppercase hover:bg-transparent"
                        ...
                        <TableHead>
                          <Button
                            variant="ghost"
                            className="p-0 h-auto text-[10px] font-black uppercase hover:bg-transparent"
                            onClick={() => requestSort('responsibility')}
                          >
                        ...
                        <TableHead className="text-center">
                          <Button
                            variant="ghost"
                            className="p-0 h-auto text-[10px] font-black uppercase hover:bg-transparent mx-auto"
                            onClick={() => requestSort('followUpDate')}
                          >
                        ...
                        <TableHead className="text-right">
                          <Button
                            variant="ghost"
                            className="p-0 h-auto text-[10px] font-black uppercase hover:bg-transparent ml-auto"
                    ...
                    <TableBody>
                      {processedOutputs.map((output, index) => (
                        ...
                        >
                          <TableCell className="text-[10px] font-black text-muted-foreground text-center pl-6">
                            {index + 1}
                          </TableCell>
                          <TableCell>
                            <div className="flex flex-col gap-1 max-w-xs">
                              <span className="font-bold text-sm text-slate-900 dark:text-slate-100 leading-snug group-hover:text-primary transition-colors">
                                {output.description}
                              </span>
                            ...
                            <div className="flex flex-col gap-1">
                              {(output.assignments || []).map((a, i) => (
                                <div key={i} className="flex items-center gap-1">
                                  <Badge
                                    variant="secondary"
                                    className="text-[8px] h-4 font-black bg-primary/5 text-primary border-none"
                                  >
            ...
            <>
              <DialogHeader className="p-6 bg-slate-50 dark:bg-slate-800/50 border-b shrink-0">
                ...
                <DialogDescription className="text-xs">
                  Comprehensive view of the institutional requirement and assigned responsibilities.
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="p-8 space-y-8">
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                            {previewOutput.initiator}
                          </p>
                        </div>
                        <div>
                          <p className="text-[9px] font-bold text-slate-400 uppercase">Source Session</p>
                          <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                            {reviewMap.get(previewOutput.mrId)?.title || 'Management Review'}
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          <Badge
                            className={cn(
                              ...
                              previewOutput.status === 'Open'
                                ? 'bg-rose-600 text-white'
                                : previewOutput.status === 'On-going'
                                  ? 'bg-amber-500 text-amber-950'
                                  : previewOutput.status === 'Submit for Closure Verification'
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                              <span className="text-[10px] font-black uppercase text-primary leading-none mb-1">
                                {campusMap.get(a.campusId)}
                              </span>
                              <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate max-w-[180px]">
                                {unitMap.get(a.unitId)}
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  {previewOutput.followUpRemarks && (
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-700 dark:text-slate-300 mb-3">
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase">
                          Action by: {previewOutput.actionTakenBy}
                        </span>
                        <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase">
                          {safeFormatDateLocal(previewOutput.actionDate)}
                        </span>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="bg-emerald-50 rounded-xl p-6 border border-emerald-100">
                      ...
                      <div className="mt-4 pt-4 border-t border-emerald-200 flex items-center justify-between">
                        <span className="text-[10px] font-bold text-emerald-700 uppercase">
                          Verified by: {previewOutput.verifiedBy}
                        </span>
                        <span className="text-[10px] font-bold text-emerald-700 uppercase">
                          {safeFormatDateLocal(previewOutput.verificationDate)}
                        </span>
                      </div>
                    </div>
                  )}
      ...
      <Dialog open={isUpdateDialogOpen} onOpenChange={setIsUpdateDialogOpen}>
        <DialogContent className="sm:max-w-lg h-[80dvh] flex flex-col p-0">
          <DialogHeader className="p-6 border-b shrink-0">
            ...
            <DialogDescription className="text-xs">
              Update the status and provide progress notes for this assigned review output.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="p-4 bg-muted/30 rounded-lg border space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {selectedOutput?.lineNumber && (
                    <Badge
                      variant="outline"
                      className="text-[9px] h-4 font-bold border-primary/30 text-primary uppercase"
                    >
                      Minutes Line: {selectedOutput.lineNumber}
                    </Badge>
                  )}
                </div>
                ...
                <form onSubmit={updateForm.handleSubmit(handleUpdateStatusSubmit)} className="space-y-6">
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <Input
                                ...
                                className="bg-slate-50 dark:bg-slate-800/50 h-9 text-xs font-bold"
                              />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-[10px] font-black uppercase">
                            Action Taken / Unit Progress Summary
                          </FormLabel>
                          <FormControl>
                            <Textarea
                              {...field}
                              placeholder="Describe the steps taken by your unit to address this MR decision..."
                              rows={4}
                              className="bg-slate-50 dark:bg-slate-800/50 text-xs"
                            />
                          </FormControl>
                          <FormDescription className="text-[9px]">
                            Provide evidence of completion or reasons for ongoing status.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value}>
                            <FormControl>
                              <SelectTrigger className="bg-primary/5 border-primary/20 font-black h-10">
                                <SelectValue />
                              </SelectTrigger>
                            </FormControl>
                            <SelectContent modal={false}>
                              ...
                              <SelectItem value="Submit for Closure Verification" className="font-bold text-blue-600">
                                Submit for Closure Verification
                              </SelectItem>
                              {isAdmin && (
                                <SelectItem value="Closed" className="font-bold text-emerald-600">
                                  Closed (Institutional Verification Complete)
                                </SelectItem>
                              )}
                            </SelectContent>
                          </Select>
                          <FormDescription className="text-[9px]">
                            {isAdmin
                              ? "Only administrators can move an item to 'Closed' status."
                              : "Select 'Submit for Closure Verification' once your unit has completed the action."}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      <Card className="border-emerald-200 bg-emerald-50/20">
                        <CardContent className="p-4 space-y-4">
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-[10px] font-black uppercase text-emerald-700">
                                  Verification Date
                                </FormLabel>
                                <FormControl>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-[10px] font-black uppercase text-emerald-700">
                                  Verification Findings / Description
                                </FormLabel>
                                <FormControl>
                                  <Textarea
                                    {...field}
                                    placeholder="Record findings from closure verification audit..."
                                    rows={3}
                                    className="bg-white text-xs border-emerald-100"
                                  />
                                </FormControl>
                                <FormDescription className="text-[9px] text-emerald-600/70">
                                  Required for official closure of institutional decision items.
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                            )}
                  ...
                  <DialogFooter className="p-6 border-t bg-slate-50 dark:bg-slate-800/50 shrink-0 gap-2 sm:gap-0">
                    ...
                    <Button
                      type="submit"
                      disabled={isSubmitting}
                      className="min-w-[150px] shadow-xl shadow-primary/20 font-black"
                    >
                      {isSubmitting ? (
          ...
          <div className="p-2">
            ...
            <Form {...actionEntryForm}>
              <form onSubmit={actionEntryForm.handleSubmit(handleAddActionEntry)} className="space-y-5">
                <FormField
                  control={actionEntryForm.control}
                  name="description"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          {...field}
                          placeholder="Describe the specific action taken by your unit..."
                          rows={4}
                          className="bg-slate-50 dark:bg-slate-800/50 text-xs"
                        />
                      </FormControl>
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            ...
                            className="bg-slate-50 dark:bg-slate-800/50 h-9 text-xs"
                          />
                        </FormControl>
                ...
                <DialogFooter className="gap-2 pt-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setIsActionEntryDialogOpen(false)}
            ...
            <div className="space-y-4 py-2">
              <div className="border rounded-lg p-4 bg-slate-50 dark:bg-slate-800/50 space-y-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    {selectedActionEntry.description}
                  </p>
                </div>
                <div className="flex flex-wrap gap-4 text-[10px] font-bold text-muted-foreground">
                  ...
                  {selectedActionEntry.googleDriveLink && (
                    <a
                      href={selectedActionEntry.googleDriveLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-primary hover:underline font-black"
                    >
              ...
              <div className="space-y-2">
                ...
                <p className="text-[9px] text-muted-foreground">
                  This feedback will be visible to the submitting unit.
                </p>
              </div>
            </div>
          )}
