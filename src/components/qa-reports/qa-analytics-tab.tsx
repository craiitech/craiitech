...
export function QaAnalyticsTab() {
  ...
  const analytics = useMemo(() => {
    ...
    const outputs = isInstitutionalViewer
      ? rawMrOutputs
      : rawMrOutputs.filter((o) =>
          o.assignments?.some((a: any) => a.unitId === userProfile.unitId || a.campusId === userProfile.campusId),
        );
    ...
    const topUnits = Object.values(unitCounts)
    ...
    (selectedYear === 'all' ? cars : yearCars)
    ...
    type CampusRecord = {
      ...
      Open: number;
      'In Progress': number;
      'Awaiting Response/Update': number;
      'For Final Verification': number;
      Closed: number;
    };
    ...
    mrs.forEach((mr) => {
      const date =
        mr.startDate instanceof Timestamp ? mr.startDate.toDate() : mr.startDate ? new Date(mr.startDate) : null;
    ...
    if (byCampus.length > 0) {
      ...
      const totalCampusOpen =
        highestCampus.Open +
        highestCampus['In Progress'] +
        highestCampus['Awaiting Response/Update'] +
        highestCampus['For Final Verification'];
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center border rounded-xl border-dashed bg-muted/5">
        ...
        <p className="text-sm text-muted-foreground">
          Analytics will synchronize once reports, MR sessions, or CARs are registered in the system.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Chart3DDefs idPrefix="qaanalytics3d" />
        ...
        <div className="flex flex-col">
          <div className="bg-emerald-50 px-6 py-3 border-b flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-700">
              Institutional Strength Registry
            </span>
          </div>
          <div className="p-6 space-y-4">
            {analytics.strengths.length > 0 ? (
              ...
              <p className="text-[10px] text-muted-foreground italic opacity-50 py-10 text-center">
                Calibrating institutional strengths...
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <Card className="bg-primary/5 border-primary/10 shadow-sm relative overflow-hidden flex flex-col">
          ...
          <CardContent className="flex-1">
            ...
            <p className="text-[8px] font-bold text-muted-foreground mt-0.5 uppercase tracking-tighter">
              Institutional Registry ({selectedYear === 'all' ? 'All Time' : `AY ${selectedYear}`})
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-rose-50 border-rose-100 shadow-sm relative overflow-hidden flex flex-col">
          ...
          <CardContent className="flex-1">
            ...
            <p className="text-[8px] font-bold text-rose-600/70 mt-0.5 uppercase tracking-tighter">
        ...
        <Card className="bg-orange-50 border-orange-100 shadow-sm relative overflow-hidden flex flex-col">
          <CardHeader className="pb-1.5">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-orange-700">
              In Progress
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
            <div className="text-2xl font-black text-orange-600 tabular-nums">{analytics.progressCars}</div>
            <p className="text-[8px] font-bold text-orange-600/70 mt-0.5 uppercase tracking-tighter">
              Unit Treatment Stage
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-amber-50 border-amber-100 shadow-sm relative overflow-hidden flex flex-col">
          <CardHeader className="pb-1.5">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-amber-700">
              Awaiting Response
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
            <div className="text-2xl font-black text-amber-600 tabular-nums">{analytics.awaitingCars}</div>
            <p className="text-[8px] font-bold text-amber-600/70 mt-0.5 uppercase tracking-tighter">
              Bottleneck — Unit Action Needed
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-indigo-50 border-indigo-100 shadow-sm relative overflow-hidden flex flex-col">
          <CardHeader className="pb-1.5">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-indigo-700">
              For Verification
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
            <div className="text-2xl font-black text-indigo-600 tabular-nums">{analytics.verificationCars}</div>
            <p className="text-[8px] font-bold text-indigo-600/70 mt-0.5 uppercase tracking-tighter">
              Awaiting QMS Closure
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-emerald-50 border-emerald-100 shadow-sm relative overflow-hidden flex flex-col">
          <CardHeader className="pb-1.5">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-700">
              Resolution Rate
            </CardTitle>
          </CardHeader>
          <CardContent className="flex-1">
            ...
            <p className="text-[8px] font-bold text-emerald-600/70 mt-0.5 uppercase tracking-tighter">
              {analytics.closedCars} of {analytics.totalCars} Closed
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-start gap-3 shadow-sm">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider">
              Executive Summary — Corrective Action Pipeline
            </p>
            <p className="text-sm font-bold text-slate-700 dark:text-slate-300 mt-1 leading-relaxed">
              {analytics.pipelineText}
            </p>
          </div>
        </div>
      )}
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">CAR Lifecycle Profile (3D)</CardTitle>
            </div>
                      ...
                      >
                        {analytics.carStatusData.map((entry, index) => (
                          <Cell
                            ...
                            fill={
                              entry.statusId === 'Open'
                                ? 'url(#qaanalytics3d-grad-rose)'
                                : entry.statusId === 'In Progress'
                                  ? 'url(#qaanalytics3d-grad-amber)'
                                  : entry.statusId === 'Awaiting Response/Update'
                                    ? 'url(#qaanalytics3d-grad-amber)'
                                    : entry.statusId === 'For Final Verification'
                                      ? 'url(#qaanalytics3d-grad-indigo)'
                                      : 'url(#qaanalytics3d-grad-emerald)'
                            }
                            filter="url(#qaanalytics3d-soft-depth)"
                          />
                        ))}
                      </Pie>
                    </PieChart>
                ...
                <div className="space-y-2">
                  {analytics.carStatusData.map((d) => {
                    ...
                    return (
                      <div
                        key={d.name}
                        className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-1.5 last:border-0 last:pb-0"
                      >
                        <div className="flex items-center gap-2">
                          <div
                            className="h-2.5 w-2.5 rounded-full shrink-0 shadow-sm"
                            style={{ background: CAR_STATUS_COLORS[d.name] || '#94a3b8' }}
                          />
                        ...
                        <span className="text-[11px] font-black text-slate-900 dark:text-slate-100">
                          {d.value} <span className="text-slate-400 font-medium font-mono text-[9px]">({pct}%)</span>
                        </span>
                      </div>
                    );
                  })}
          ...
          <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
            <CardHeader className="bg-muted/10 border-b py-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-sm font-black uppercase tracking-tight">NC vs OFI Split (3D)</CardTitle>
              </div>
            ...
            <CardContent className="pt-4">
                    ...
                    >
                      {analytics.findingData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            entry.name === 'NC' ? 'url(#qaanalytics3d-grad-rose)' : 'url(#qaanalytics3d-grad-amber)'
                          }
                          filter="url(#qaanalytics3d-soft-depth)"
                        />
                      ))}
                  ...
                  {analytics.findingData.map((d) => (
                    <div key={d.name} className="flex items-center gap-3">
                      <div
                        ...
                        style={{ background: NATURE_COLORS[d.name] }}
                      />
                      <div>
                        <p className="text-sm font-black text-slate-800 dark:text-slate-200">{d.value}</p>
                        <p className="text-[9px] font-bold text-muted-foreground uppercase leading-none mt-0.5">
                          {d.name === 'NC' ? 'Non-Conformance' : 'Opportunity for Improvement'}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              {analytics.totalCars > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-2.5 flex items-start gap-2">
          ...
          <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
            <CardHeader className="bg-muted/10 border-b py-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-sm font-black uppercase tracking-tight">CARs by Source (3D)</CardTitle>
              </div>
              <CardDescription className="text-[9px] font-bold uppercase tracking-widest text-slate-400">
                ...
                <>
                  <ResponsiveContainer width="100%" height={130}>
                    ...
                    >
                      ...
                      />
                      <RechartsTooltip />
                      <Bar dataKey="value" name="CARs" radius={[0, 6, 6, 0]} filter="url(#qaanalytics3d-soft-depth)">
                  ...
                  <div className="mt-2 pt-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-2.5 flex items-start gap-2">
                    <Info className="h-3.5 w-3.5 text-primary shrink-0 mt-0.5" />
                    <div>
                      <p className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Analysis</p>
                      <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">
                        {analytics.sourceText}
                      </p>
                    </div>
                  </div>
                </>
            ...
            {analytics.openCars > 0 ? (
              <>
                <ResponsiveContainer width="100%" height={210}>
                  <BarChart data={analytics.agingData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 flex items-start gap-2">
                  <Info className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                  <div>
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider">
                      Analysis & Action Plan
                    </p>
                    <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">
                      {analytics.agingText}
                    </p>
                  </div>
                </div>
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">Top Units by Open CARs (3D)</CardTitle>
            </div>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
              ...
              <>
                <ResponsiveContainer width="100%" height={210}>
                  ...
                  >
                    ...
                    />
                    <RechartsTooltip content={<CustomTooltip />} />
                    <Bar dataKey="open" name="Open CARs" radius={[0, 6, 6, 0]} filter="url(#qaanalytics3d-soft-depth)">
                      {analytics.topUnits.map((entry: any, i: number) => (
                        <Cell
                          key={i}
                          fill={entry.nc > 0 ? 'url(#qaanalytics3d-grad-rose)' : 'url(#qaanalytics3d-grad-indigo)'}
                        />
                      ))}
                      <LabelList content={<RenderBar3DLabel />} />
                    </Bar>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-indigo-600" />
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Monthly CAR Trend (3D Depth) — {selectedYear === 'all' ? 'All Years' : `AY ${selectedYear}`}
              </CardTitle>
            </div>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
          ...
          <CardContent className="pt-6">
            {analytics.monthlyTrend.some((m) => m.Created > 0 || m.Closed > 0) ? (
              <>
                <ResponsiveContainer width="100%" height={210}>
                  <LineChart data={analytics.monthlyTrend} margin={{ top: 10, right: 30, left: 0, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" opacity={0.15} />
          ...
          <CardContent className="pt-6 flex-1">
            <div className="flex items-center justify-center h-[120px]">
              <div className="text-center">
                ...
                <p className="text-[10px] font-bold text-blue-600/70 mt-1 uppercase tracking-tighter">
                  Resolution Rate
                </p>
                <p className="text-[9px] font-bold text-muted-foreground mt-1">
                  {analytics.totalDecisions} total decisions
                </p>
              </div>
            </div>
            {analytics.totalDecisions > 0 && analytics.mrResolutionRate < 70 && (
              <div className="mt-2 pt-3 border-t border-slate-100 dark:border-slate-700 bg-rose-50 rounded-xl p-2.5 flex items-start gap-2">
                <AlertTriangle className="h-3.5 w-3.5 text-rose-600 shrink-0 mt-0.5" />
                <p className="text-[10px] font-bold text-rose-700 leading-relaxed">
                  Decision backlog needs attention — less than 70% of management review outputs have been closed.
                </p>
              </div>
            )}
          </CardContent>
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Corrective Actions by Campus (3D)
              </CardTitle>
            </div>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <BarChart data={analytics.byCampus} margin={{ top: 10, right: 20, left: 0, bottom: 35 }}>
                    ...
                    <Bar
                      dataKey="Open"
                      stackId="a"
                      fill="url(#qaanalytics3d-grad-rose)"
                      filter="url(#qaanalytics3d-soft-depth)"
                    >
                      <LabelList
                        dataKey="Open"
                        position="inside"
                        style={{ fontSize: 8, fontWeight: 900, fill: '#fff' }}
                    ...
                    <Bar
                      dataKey="In Progress"
                      stackId="a"
                      fill="url(#qaanalytics3d-grad-amber)"
                      filter="url(#qaanalytics3d-soft-depth)"
                    >
                      <LabelList
                        dataKey="In Progress"
                        position="inside"
                        style={{ fontSize: 8, fontWeight: 900, fill: '#000' }}
                        formatter={(v: number) => (v > 0 ? v : '')}
                      />
                    </Bar>
                    <Bar
                      dataKey="Awaiting Response/Update"
                      stackId="a"
                      fill="url(#qaanalytics3d-grad-amber)"
                      filter="url(#qaanalytics3d-soft-depth)"
                    >
                    ...
                    <Bar
                      dataKey="For Final Verification"
                      stackId="a"
                      fill="url(#qaanalytics3d-grad-indigo)"
                      filter="url(#qaanalytics3d-soft-depth)"
                    >
                      <LabelList
                        dataKey="For Final Verification"
                    ...
                    <Bar
                      dataKey="Closed"
                      stackId="a"
                      fill="url(#qaanalytics3d-grad-emerald)"
                      radius={[6, 6, 0, 0]}
                      filter="url(#qaanalytics3d-soft-depth)"
                    >
                      <LabelList
        ...
        <Card className="shadow-lg hover:shadow-xl transition-all rounded-2xl border-primary/10 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                MR Decision Trends by Review Year (3D)
              </CardTitle>
            </div>
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
