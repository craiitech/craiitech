...
export function ProgramPerformanceView({
  program,
  record,
  selectedYear,
  onResolveDeficiency,
}: ProgramPerformanceViewProps) {
  ...
  const analyticsData = useMemo(() => {
    ...
    const facultyPieData = [
      ...
      {
        name: 'Unqualified',
        value: Math.max(0, totalFaculty - alignedFaculty - othersFaculty),
        fill: STATUS_COLORS['Needs Correction'],
      },
    ].filter((d) => d.value > 0);

    const latestBoard =
      record.boardPerformance && record.boardPerformance.length > 0
    ...
    const pillarScores = {
      ...
      accreditation: program.isNewProgram
        ? 20
        : latestAccreditation?.level && latestAccreditation.level !== 'Non Accredited'
          ? 20
          : 0,
    ...
    if (record.ched?.copcStatus !== 'With COPC')
      gaps.push({
        type: 'Institutional Authority',
        msg: 'Program is operating without an active COPC.',
        priority: 'High',
        target: 'ched',
      });
    if (!latestAccreditation || latestAccreditation.level === 'Non Accredited') {
      if (!program.isNewProgram)
        gaps.push({
          ...
          msg: 'No active accreditation status recorded.',
          priority: 'Medium',
    ...
    if (record.ched?.copcLink)
      evidenceRegistry.push({ title: 'CHED COPC Certificate', url: record.ched.copcLink, category: 'Regulatory' });
    if (record.ched?.programCmoLink)
      evidenceRegistry.push({
        title: 'CHED Memorandum Order (CMO)',
        url: record.ched.programCmoLink,
        category: 'Regulatory',
      });
    if (record.ched?.boardApprovalLink)
      evidenceRegistry.push({
        title: 'Board Approval (BOR Resolution)',
    ...
    curriculumRecords.forEach((cr) => {
      if (cr.notationProofLink) {
        const majorName =
          cr.majorId === 'General' || cr.majorId === 'general' ? 'General' : specMap.get(cr.majorId) || cr.majorId;
        evidenceRegistry.push({
    ...
    milestones.forEach((m) => {
      if (m.certificateLink)
        evidenceRegistry.push({
          title: `${m.level} Accreditation Certificate`,
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center border border-dashed rounded-2xl bg-muted/5">
        ...
        <p className="text-sm text-muted-foreground">
          Log program data in the compliance modules to activate decision support.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <Chart3DDefs idPrefix="progperf3d" />

      {/* 1. EXECUTIVE PERFORMANCE SUMMARY */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 shadow-lg hover:shadow-xl transition-all border-primary/10 rounded-2xl overflow-hidden flex flex-col relative bg-white dark:bg-slate-900">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <TrendingUp className="h-16 w-16 text-primary" />
          </div>
          <CardHeader className="bg-muted/10 border-b">
            <CardTitle className="text-sm font-black uppercase tracking-widest text-primary">
              Strategic Maturity Index (3D)
            </CardTitle>
          </CardHeader>
              ...
              <ResponsiveContainer>
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={analyticsData.radarData}>
                  ...
                  <Radar
                    ...
                    stroke="#10b981"
                    strokeWidth={2}
                    fill="url(#progperf3d-grad-emerald)"
                    fillOpacity={0.6}
                    filter="url(#progperf3d-soft-depth)"
                  />
                </RadarChart>
              </ResponsiveContainer>
            </ChartContainer>
          ...
          <CardFooter className="bg-muted/5 border-t py-3">
            <div className="flex items-start gap-3">
              ...
              <p className="text-[9px] text-muted-foreground leading-relaxed italic">
                <strong>Insight:</strong> Measures balance across 5 compliance pillars. A symmetrical radar indicates
                consistent quality assurance across all modules.
              </p>
            </div>
          </CardFooter>
        </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-2xl font-black text-slate-900 dark:text-slate-100 uppercase">
                    {program.isNewProgram
                      ? 'NEW PROGRAM'
                      : analyticsData.latestAccreditation?.statusValidityDate || 'TBA'}
                  </p>
                </div>
            ...
            <Card className="border-blue-200 shadow-sm bg-blue-50/10">
              <CardHeader className="pb-2">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-blue-700">
                  Enrollment Baseline
                </CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <div className="p-6 space-y-4">
                  {analyticsData.gaps.length > 0 ? (
                    analyticsData.gaps.map((gap, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-4 bg-white p-4 rounded-xl border border-destructive/10 shadow-sm transition-all hover:border-destructive/30 group cursor-pointer"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <p className="text-[10px] font-black text-destructive uppercase tracking-[0.1em]">
                              {gap.type}
                            </p>
                            <Badge
                              variant="outline"
                              className="h-4 text-[8px] border-destructive/20 text-destructive font-black uppercase"
                            >
                              {gap.priority}
                            </Badge>
                          </div>
                    ...
                    <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground">
                        All quality criteria verified for AY {selectedYear}.
                      </p>
                    </div>
                  )}
                </div>
          ...
          <CardHeader className="bg-muted/10 border-b py-4">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-black uppercase tracking-tight">
                Enrollment Velocity Trend (3D)
              </CardTitle>
            </div>
            <CardDescription className="text-xs">
              Headcount distribution across Year Levels for AY {selectedYear}.
            </CardDescription>
          </CardHeader>
            ...
            <ChartContainer config={{}} className="h-[300px] w-full">
              <ResponsiveContainer>
                <BarChart data={analyticsData.enrollmentData}>
                  ...
                  <Bar
                    dataKey="1st Sem"
                    fill="url(#progperf3d-grad-indigo)"
                    radius={[6, 6, 0, 0]}
                    filter="url(#progperf3d-soft-depth)"
                  >
          ...
          <CardFooter className="bg-muted/5 border-t py-3 px-6">
            <div className="flex items-start gap-3">
              ...
              <p className="text-[9px] text-muted-foreground italic leading-tight">
                <strong>Insight:</strong> Tracks population retention and growth. Disparities between semesters or years
                indicate potential high-risk attrition points.
              </p>
            </div>
          </CardFooter>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="shadow-lg hover:shadow-xl transition-all border-primary/10 rounded-2xl overflow-hidden flex flex-col bg-white dark:bg-slate-900">
            <CardHeader className="p-4 bg-muted/10 border-b">
              <CardTitle className="text-[10px] font-black uppercase text-center">
                Faculty Maturity Profile (3D)
              </CardTitle>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col items-center justify-center p-6">
              <ChartContainer config={{}} className="h-[185px] w-[185px]">
                <ResponsiveContainer>
                  <PieChart>
                    <Pie
                      data={analyticsData.facultyPieData}
                      cx="50%"
                      cy="50%"
                      innerRadius={40}
                      outerRadius={65}
                      ...
                      labelLine={false}
                    >
                      {analyticsData.facultyPieData.map((e, i) => (
                        <Cell
                          key={i}
                          fill={i === 0 ? 'url(#progperf3d-grad-emerald)' : 'url(#progperf3d-grad-amber)'}
                          filter="url(#progperf3d-soft-depth)"
                        />
                      ))}
                    </Pie>
            ...
            <CardFooter className="bg-muted/5 border-t py-3">
              <p className="text-[8px] text-center w-full text-muted-foreground italic">
                Percent Aligned with Program Standards (CMO)
              </p>
            </CardFooter>
          </Card>
          ...
          <Card className="shadow-lg hover:shadow-xl transition-all border-primary/10 rounded-2xl overflow-hidden flex flex-col bg-white dark:bg-slate-900">
            ...
            <CardContent className="flex-1 flex flex-col items-center justify-center p-6">
              {program.isBoardProgram && analyticsData.latestBoard ? (
                <>
                  <ChartContainer config={{}} className="h-[185px] w-[185px]">
                    <ResponsiveContainer>
                      <BarChart data={analyticsData.boardComparisonData} margin={{ top: 25 }}>
                        <XAxis
                          dataKey="name"
                        ...
                        <RechartsTooltip content={<ChartTooltipContent />} />
                        <Bar dataKey="rate" radius={[6, 6, 0, 0]} barSize={32} filter="url(#progperf3d-soft-depth)">
                          {analyticsData.boardComparisonData.map((e, i) => (
                            <Cell
                              key={i}
                              fill={
                                e.name.includes('Institutional') ||
                                e.name.includes('School') ||
                                e.name.includes('Campus')
                                  ? 'url(#progperf3d-grad-emerald)'
                                  : 'url(#progperf3d-grad-indigo)'
                              }
                            />
                          ))}
                          <LabelList content={<RenderBar3DLabel />} />
                  ...
                  <p className="text-[9px] font-black text-primary mt-4 uppercase">
                    Latest Exam: {analyticsData.latestBoard.examDate}
                  </p>
                </>
              ) : (
            ...
            <CardFooter className="bg-muted/5 border-t py-3">
              <p className="text-[8px] text-center w-full text-muted-foreground italic">
                Source: PRC Official Board Results Ledger
              </p>
            </CardFooter>
          </Card>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <CardDescription className="text-xs font-medium">
                Consolidated registry of mandatory and enhancement requirements from all active milestones.
              </CardDescription>
            </div>
                ...
                {analyticsData.recommendations.map((reco, idx) => (
                  <TableRow key={reco.id} className="hover:bg-muted/20 transition-colors">
                    <TableCell className="pl-8 py-5">
                      <Badge
                        variant="secondary"
                        className="bg-primary/10 text-primary border-none font-black text-[9px] h-5"
                      >
                        {reco.milestoneLevel}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant={reco.type === 'Mandatory' ? 'destructive' : 'secondary'}
                        ...
                        className={cn(
                          ...
                          reco.status === 'Open'
                            ...
                            : reco.status === 'In Progress'
                              ...
                              : reco.status === 'Move to the Official Current Level'
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed italic">
                This registry tracks the specific improvements mandated by external accreditors. Units identified in the
                "Accountable Units" column are responsible for submitting evidence logs of implementation through the
      ...
      <Card className="border-primary/10 shadow-lg overflow-hidden flex flex-col">
        <CardHeader className="bg-primary/5 border-b py-4">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="bg-white text-primary border-primary/20 h-5 px-2 font-black text-[9px] uppercase"
            >
                    ...
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant="secondary"
                            className="h-3 text-[7px] font-black uppercase bg-emerald-100 text-emerald-700 border-none"
                          >
