        ...
        <Card className="shadow-sm border-primary/10">
          <CardHeader className="bg-muted/10 border-b py-4">
            ...
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              CHED Recognition and Operating Standards.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6 pt-6">
            <FormField
              ...
              render={({ field: inputField }) => (
                <FormItem>
                  <FormLabel className="text-[10px] font-black uppercase text-primary tracking-widest">
                    COPC Status
                  </FormLabel>
                  <Select onValueChange={inputField.onChange} value={inputField.value} disabled={!canEdit || !isAdmin}>
                    <FormControl>
                      <SelectTrigger className="h-11 font-bold bg-primary/5 border-primary/20">
                        <SelectValue />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
        ...
        <Card className="shadow-sm border-primary/10">
          <CardHeader className="bg-muted/10 border-b py-4">
            ...
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest">
              Authority granted via official RSU Board Resolutions.
            </CardDescription>
          </CardHeader>
                  ...
                  <FormControl>
                    ...
                    >
                      <div className="flex items-center space-x-2 p-3 rounded-xl border bg-muted/5 group cursor-pointer hover:border-primary/20">
                        ...
                        <Label htmlFor="mode-sole" className="text-xs font-bold cursor-pointer">
                          Institutional Program Approval
                        </Label>
                      </div>
                      <div
                        className={cn(
                          'flex items-center space-x-2 p-3 rounded-xl border bg-muted/5 group cursor-pointer hover:border-primary/20',
                          !hasSpecializations && 'opacity-50 pointer-events-none',
                        )}
                      >
                        <RadioGroupItem value="per-major" id="mode-major" disabled={!hasSpecializations} />
                        <Label htmlFor="mode-major" className="text-xs font-bold cursor-pointer">
                          Specialization-Level Approvals
                        </Label>
                      </div>
                    </RadioGroup>
                  </FormControl>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-primary">
                    Specialization Resolutions Registry
                  </p>
                </div>
                  ...
                  {program.specializations?.map((spec, idx) => (
                    <div key={spec.id} className="p-4 rounded-xl border bg-muted/5 space-y-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        ...
                        {majorApprovals[idx]?.link && (
                          <Badge
                            variant="outline"
                            className="bg-emerald-50 text-emerald-700 border-emerald-200 h-4 text-[8px] font-black"
                          >
                            LINKED
                          </Badge>
                        )}
                    ...
                    render={({ field: inputField }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2">
                          <Hash className="h-3 w-3" /> Referendum No.
                        </FormLabel>
                        <FormControl>
                          <Input
                            {...inputField}
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <Input
                            ...
                            value={inputField.value || ''}
                            placeholder="https://drive.google.com/..."
                            className="pl-9 h-11 text-xs bg-white border-destructive/20"
                            disabled={!canEdit || !isAdmin}
                          />
                        </div>
                      </FormControl>
          ...
          <CardHeader className="flex flex-row items-center justify-between py-4 bg-primary/5 border-b">
            <div className="space-y-1">
              <CardTitle className="flex items-center gap-2 text-primary text-sm uppercase font-black tracking-tight">
                <Calendar className="h-4 w-4" /> RQAT Monitoring & Site Visits
              </CardTitle>
              <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                RQAT Audit Trail.
              </CardDescription>
            ...
            {canEdit && isAdmin && (
              <Button
                ...
                className="h-8 gap-1.5 text-[10px] font-black uppercase tracking-widest shadow-md"
              >
          ...
          <CardContent className="pt-6 flex-1 overflow-hidden">
            <div className="space-y-6">
              {rqatFields.map((field, index) => (
                <div
                  key={field.id}
                  className="relative p-5 rounded-2xl border bg-muted/10 space-y-4 group hover:border-primary/30 transition-all"
                >
                  {canEdit && isAdmin && (
                    <Button
                      type="button"
                    ...
                    render={({ field: inputField }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase text-primary flex items-center gap-2">
                          <LinkIcon className="h-3 w-3" /> Monitoring Report Link (GDrive){' '}
          ...
          <CardFooter className="bg-primary/5 border-t py-3 px-6">
            <div className="flex items-start gap-3">
              ...
              <p className="text-[10px] text-muted-foreground italic leading-relaxed">
                RQAT visit records are institutional evidence of compliance with CHED Quality Assurance frameworks.
              </p>
            </div>
          </CardFooter>
        </Card>
