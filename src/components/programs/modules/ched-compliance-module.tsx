  ...
  return (
    <div className="mt-4 space-y-2 animate-in fade-in slide-in-from-top-2 duration-500">
      ...
      <div className="aspect-[16/10] w-full rounded-lg border bg-muted overflow-hidden shadow-inner relative group">
        <iframe src={embedUrl} className="absolute inset-0 h-full w-full border-none" allow="autoplay" title={`${title} Preview`} />
      </div>
    </div>
  );
}
      ...
      <div className="space-y-6">
        <Card className="shadow-sm border-primary/10">
          <CardHeader className="bg-muted/10 border-b py-4">
            ...
            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">CHED Recognition and Operating Standards.</CardDescription>
          </CardHeader>
        ...
        <Card className="shadow-sm border-primary/10">
            ...
            <CardContent className="pt-6 space-y-6">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="space-y-3">
                            ...
                            <FormControl>
                                <RadioGroup onValueChange={field.onChange} value={field.value || 'sole'} className="flex flex-col space-y-1" disabled={!canEdit}>
                                    ...
                                    <div className={cn("flex items-center space-x-2 p-3 rounded-xl border bg-muted/5 group cursor-pointer hover:border-primary/20", !hasSpecializations && "opacity-50 pointer-events-none")}>
                                        ...
                                        <Label htmlFor="mode-major" className="text-xs font-bold cursor-pointer">Specialization-Level Approvals</Label>
                                    </div>
                                </RadioGroup>
                            </FormControl>
                        </FormItem>
                ...
                {boardApprovalMode === 'sole' ? (
                    <div className="space-y-4">
                        <FormField control={control} name="ched.boardApprovalLink" render={({ field }) => (
                            <FormItem><FormLabel className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest flex items-center gap-2">BOR Resolution Link (GDrive) {boardApprovalLinkVal && <CheckCircle2 className="h-3 w-3 text-green-500" />}</FormLabel>
                                <FormControl><div className="relative"><LinkIcon className="absolute left-3 top-3 h-3.5 w-3.5 text-muted-foreground" /><Input {...field} value={field.value || ''} placeholder="https://drive.google.com/..." className="pl-9 h-9 text-xs" disabled={!canEdit} /></div></FormControl>
                            </FormItem>
                        )} />
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2"><Layers className="h-4 w-4 text-primary" /><p className="text-[10px] font-black uppercase tracking-widest text-primary">Specialization Resolutions Registry</p></div>
                    ...
                    <div className="pt-6 border-t mt-6 space-y-4 border-destructive/20 bg-destructive/5 p-5 rounded-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField control={control} name="ched.closureReferendumNumber" render={({ field }) => (
                                <FormItem><FormLabel className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2"><Hash className="h-3 w-3" /> Referendum No.</FormLabel><FormControl><Input {...field} value={field.value || ''} placeholder="e.g. 2024-042" className="h-9 text-xs bg-white font-mono font-bold" disabled={!canEdit} /></FormControl></FormItem>
                            )} />
                            <FormField control={control} name="ched.closureApprovalDate" render={({ field }) => (
                                <FormItem><FormLabel className="text-[10px] font-black uppercase text-muted-foreground">Approval Date</FormLabel><FormControl><Input {...field} value={field.value || ''} type="date" className="h-9 text-xs bg-white" disabled={!canEdit} /></FormControl></FormItem>
                            )} />
                        </div>
                        <FormField control={control} name="ched.closureResolutionLink" render={({ field }) => (
                            <FormItem><FormLabel className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2">Evidence: Closure Resolution (PDF) {closureLinkVal && <CheckCircle2 className="h-3 w-3 text-green-500" />}</FormLabel>
                                <FormControl><div className="relative"><LinkIcon className="absolute left-3 top-3 h-3.5 w-3.5 text-muted-foreground opacity-50" /><Input {...field} value={field.value || ''} placeholder="https://drive.google.com/..." className="pl-9 h-11 text-xs bg-white border-destructive/20" disabled={!canEdit} /></div></FormControl>
                            </FormItem>
                        )} />
        ...
        <Card className="border-primary/20 shadow-lg overflow-hidden flex flex-col h-full">
            <CardHeader className="flex flex-row items-center justify-between py-4 bg-primary/5 border-b">
                <div className="space-y-1">
                    <CardTitle className="flex items-center gap-2 text-primary text-sm uppercase font-black tracking-tight"><Calendar className="h-4 w-4" /> RQAT Monitoring & Site Visits</CardTitle>
                    <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">RQAT Audit Trail.</CardDescription>
                </div>
                {canEdit && (
                    <Button type="button" size="sm" onClick={() => appendRqat({ date: '', result: '', nonCompliances: '', comments: '', reportLink: '' })} className="h-8 gap-1.5 text-[10px] font-black uppercase tracking-widest shadow-md">
            ...
            <CardContent className="pt-6 flex-1 overflow-hidden">
                <div className="space-y-6">
                    {rqatFields.map((field, index) => (
                        <div key={field.id} className="relative p-5 rounded-2xl border bg-muted/10 space-y-4 group hover:border-primary/30 transition-all">
                            {canEdit && <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 text-destructive h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => removeRqat(index)}><Trash2 className="h-4 w-4" /></Button>}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField control={control} name={`ched.rqatVisits.${index}.date`} render={({ field: inputField }) => (
                                    <FormItem><FormLabel className="text-[10px] font-black uppercase text-muted-foreground">Visit Timeline</FormLabel><FormControl><Input {...inputField} placeholder="e.g., October 2024" className="h-9 text-xs bg-white font-bold" disabled={!canEdit} /></FormControl></FormItem>
                                )} />
                                <FormField control={control} name={`ched.rqatVisits.${index}.result`} render={({ field: inputField }) => (
                                    <FormItem><FormLabel className="text-[10px] font-black uppercase text-muted-foreground">Outcome</FormLabel><FormControl><Input {...inputField} placeholder="e.g., Compliant" className="h-9 text-xs bg-white font-bold" disabled={!canEdit} /></FormControl></FormItem>
                                )} />
                            </div>
                            ...
                            <GDrivePreview url={rqatVisits[index]?.reportLink} title={`RQAT: ${rqatVisits[index]?.date || 'Visit Report'}`} />
                        </div>
                    ))}
                    {rqatFields.length === 0 && (
            ...
            <CardFooter className="bg-primary/5 border-t py-3 px-6">
                <div className="flex items-start gap-3"><Info className="h-4 w-4 text-blue-600 shrink-0 mt-0.5" />
                    <p className="text-[10px] text-muted-foreground italic leading-relaxed">RQAT visit records are institutional evidence of compliance with CHED Quality Assurance frameworks.</p>
                </div>
            </CardFooter>
        </Card>
      </div>
