 ...
 */
export function AccreditationRecommendationReport({
  items,
  unitMap,
  scope,
  year,
  unitName,
}: AccreditationRecommendationReportProps) {
  const currentTitle =
    scope === 'institutional' || scope === 'unit'
      ? 'Institutional Accreditation Gaps Registry'
      : 'Program Accreditation Recommendations';

  return (
    <div
      className="p-0 text-black dark:text-white bg-white mx-auto font-serif leading-tight"
    ...
    >
      ...
      <div className="text-center mb-10 border-b-2 border-black pb-6">
        ...
        <h2 className="font-semibold uppercase tracking-tight mt-1" style={{ fontSize: '12pt' }}>
          Quality Assurance Office
        </h2>
        <div className="mt-8">
          <h2 className="font-black uppercase underline decoration-2 underline-offset-4" style={{ fontSize: '13pt' }}>
            {currentTitle}
          </h2>
          {unitName && (
            <h3
              className="font-black uppercase text-slate-900 dark:text-slate-100 mt-4 tracking-tight"
              style={{ fontSize: '16pt' }}
            >
              {unitName}
            </h3>
          ...
          <p className="font-bold mt-2 uppercase tracking-widest" style={{ fontSize: '10pt' }}>
            ACADEMIC YEAR: {year === 'all' ? 'ALL YEARS' : year}
          </p>
        </div>
      </div>

      <p className="mb-6 text-justify italic" style={{ fontSize: '10pt' }}>
        In alignment with the institutional Quality Management System (ISO 21001:2018) and AACCUP accreditation
        standards, the following items have been identified as mandatory requirements or enhancement areas. The
        identified accountable units are directed to implement necessary corrective or improvement actions.
      </p>
          ...
          {items.map((item, i) => (
            <tr key={i} className="border-b border-black">
              <td className="border border-black p-2 align-top">
                <p
                  className="font-black leading-tight uppercase text-slate-900 dark:text-slate-100"
              ...
              <td
                className="border border-black p-2 text-center align-top font-bold uppercase"
                style={{ fontSize: '8pt' }}
              >
                {item.noEntry ? '—' : item.recommendation.type}
              </td>
              <td className="border border-black p-2 align-top italic leading-relaxed" style={{ fontSize: '9pt' }}>
                {item.noEntry ? (
                  <span className="font-black uppercase italic text-slate-500">No Entry Yet</span>
                ) : (
                  <>
                    "{item.recommendation.text}"
                    {item.recommendation.additionalInfo && (
                      <div
                        className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 not-italic font-bold"
                        style={{ fontSize: '8pt' }}
                      >
              ...
              <td
                className="border border-black p-2 align-top font-bold uppercase text-slate-800 dark:text-slate-200"
                style={{ fontSize: '8pt' }}
              >
                {item.noEntry ? (
      ...
      <div className="mt-24 text-[9pt] text-gray-400 italic text-center border-t pt-4 space-y-1">
        ...
        <p className="font-bold">
          Institutional Accountability Registry {year === 'all' ? 'All Years' : `AY ${year}`}.
        </p>
      </div>
    </div>
  );
