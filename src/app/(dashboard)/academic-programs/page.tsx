...
export default function AcademicProgramsPage() {
  ...
  const isUnitViewer =
    (userRole === 'Unit Coordinator' || userRole === 'Unit ODIMO' || userRole === 'Faculty') && !isDoi;
  ...
  const { data: rawCompliances, isLoading: isLoadingCompliances } =
              ...
              {isAdmin && (
                <div className="pt-4">
                  <Button
                    ...
                    size="sm"
                    className="h-9 font-bold uppercase tracking-tight shadow-lg shadow-primary/20"
                  >
          ...
          <Card className="shadow-md border-primary/10">
            <CardContent className="p-4 flex flex-col md:flex-row items-end gap-4 bg-muted/10">
              ...
              <div className="w-full md:w-64 space-y-1.5">
                ...
                <Select value={unitFilter} onValueChange={setUnitFilter} disabled={!isGlobalViewer && !isCampusViewer}>
                  <SelectTrigger className="h-9 text-xs bg-white">
          ...
          <Tabs defaultValue="active-registry" className="space-y-4">
            <div className="bg-muted/30 px-4 py-1 border rounded-lg flex items-center justify-between">
              <TabsList className="bg-transparent h-8 p-0 gap-2">
                <TabsTrigger
                  value="active-registry"
                  className="h-7 text-[9px] font-black uppercase tracking-widest px-4"
                >
                  Active Portfolio
                </TabsTrigger>
                <TabsTrigger
                  value="closed-registry"
                  className="h-7 text-[9px] font-black uppercase tracking-widest px-4 data-[state=active]:bg-destructive data-[state=active]:text-white"
                >
                  Phased-Out Programs
                </TabsTrigger>
              </TabsList>
