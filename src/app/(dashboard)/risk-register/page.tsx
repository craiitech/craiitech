          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-xs font-bold uppercase tracking-widest">
                Centralized module for institutional risk management.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2 md:justify-end">
              <div className="space-y-1 w-full sm:w-auto">
                <label className="text-[10px] font-bold uppercase text-muted-foreground block sm:text-right">
                  Monitoring Year
                </label>
              ...
              <div className="flex items-center gap-2 pt-0 sm:pt-5 w-full sm:w-auto">
                ...
                >
                  ...
                  {duplicateGroups.length > 0 && (
                    <Badge
                      variant="destructive"
                      className="ml-1 h-4 px-1 min-w-[1.25rem] flex items-center justify-center text-[9px] font-black"
                    >
                ...
                <Button
                  ...
                  className="flex-1 sm:flex-none h-9 bg-white dark:bg-slate-900 border-primary/30 text-primary shadow-sm font-bold uppercase text-[10px] tracking-widest gap-2 hover:bg-primary/5"
                >
                  <Printer className="h-4 w-4 text-primary" />
                  Reports & Decision Support
                </Button>
                {(!isSupervisor || isUnitOdimo) && (
                  <Button
                    onClick={handleNewRisk}
                    className="flex-1 sm:flex-none h-9 shadow-lg shadow-primary/20 font-bold uppercase text-[10px] tracking-widest"
                  >
          ...
          <CardContent className="p-4 space-y-4">
            <div className="flex-1 w-full space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Search by description, objective, or personnel..."
                  value={searchTerm}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  disabled={!isAdmin && (!isSupervisor || isUnitOdimo)}
                >
                  <SelectTrigger className="h-9 text-xs bg-white">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase text-muted-foreground ml-1 flex items-center gap-1.5">
                  <Shield className="h-2.5 w-2.5" /> Type
                </label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase text-muted-foreground ml-1 flex items-center gap-1.5">
                  <TrendingUp className="h-2.5 w-2.5" /> Magnitude Rating
                </label>
          ...
          <Card className="shadow-md border-primary/10 overflow-hidden bg-card">
            <CardHeader className="bg-primary/5 border-b py-3 px-6">
              <CardTitle className="text-xs font-black uppercase tracking-widest text-primary">
                Yearly Risk & Opportunity Summary Profile
              </CardTitle>
              <CardDescription className="text-[10px] uppercase font-bold text-muted-foreground">
                Historical aggregate distributions by year and magnitude status.
              </CardDescription>
            </CardHeader>
                ...
                <Table className="w-full">
                  <TableHeader>
                    <TableRow className="bg-slate-50/50 dark:bg-slate-800/20">
                      ...
                      <TableHead className="text-[10px] font-black uppercase text-center bg-blue-50/30 dark:bg-blue-950/10">
                        Completion Rate{' '}
                        <span className="text-[8px] font-medium text-blue-600 block">% Closed of Total</span>
                      </TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-center bg-rose-50/30 dark:bg-rose-950/10">
                        High{' '}
                        <span className="text-[8px] font-medium text-rose-600 block">
                          (Open | In Progress | Closed)
                        </span>
                      </TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-center bg-amber-50/30 dark:bg-amber-950/10">
                        Medium{' '}
                        <span className="text-[8px] font-medium text-amber-600 block">
                          (Open | In Progress | Closed)
                        </span>
                      </TableHead>
                      <TableHead className="text-[10px] font-black uppercase text-center bg-emerald-50/30 dark:bg-emerald-950/10">
                        Low{' '}
                        <span className="text-[8px] font-medium text-emerald-600 block">
                          (Open | In Progress | Closed)
                        </span>
                      </TableHead>
                    </TableRow>
                  ...
                  <TableBody>
                    {yearlySummaries.map((summary) => (
                      <TableRow
                        key={summary.year}
                        className="hover:bg-muted/10 transition-colors text-center font-bold text-xs"
                      >
                        ...
                        <TableCell className="bg-rose-50/10 dark:bg-rose-950/5 text-[11px] tabular-nums">
                          <div className="flex items-center justify-center gap-1.5">
                            <span className="text-rose-600" title="Open">
                              {summary.high.open}
                            </span>
                            <span className="text-slate-400">/</span>
                            <span className="text-amber-500 font-bold" title="In Progress">
                              {summary.high.inProgress}
                            </span>
                            <span className="text-slate-400">/</span>
                            <span className="text-emerald-600" title="Closed">
                              {summary.high.closed}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell className="bg-amber-50/10 dark:bg-amber-950/5 text-[11px] tabular-nums">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <span className="text-slate-400">/</span>
                            <span className="text-amber-500 font-bold" title="In Progress">
                              {summary.medium.inProgress}
                            </span>
                            <span className="text-slate-400">/</span>
                            <span className="text-emerald-600" title="Closed">
                              {summary.medium.closed}
                            </span>
                          </div>
                        </TableCell>
            ...
            <Tabs value={detailedTab} onValueChange={(val) => setDetailedTab(val as any)} className="w-full">
              <div className="flex items-center justify-between border-b bg-slate-50/50 dark:bg-slate-800/50 px-6 py-2">
                <TabsList className="bg-slate-100 dark:bg-slate-700 p-0.5 border rounded-lg h-8 w-max shadow-sm">
                  <TabsTrigger
                    value="all"
                    className="gap-1.5 text-[9px] font-black uppercase tracking-widest px-4 h-6 rounded-md"
                  >
                    <Layers className="h-3 w-3" /> All Entries ({filteredRisks.length})
                  </TabsTrigger>
                  <TabsTrigger
                    value="risks"
                    className="gap-1.5 text-[9px] font-black uppercase tracking-widest px-4 h-6 rounded-md text-rose-600 data-[state=active]:text-rose-700"
                  >
                    <Shield className="h-3 w-3" /> Risks ({risksOnly.length})
                  </TabsTrigger>
                  <TabsTrigger
                    value="opportunities"
                    className="gap-1.5 text-[9px] font-black uppercase tracking-widest px-4 h-6 rounded-md text-emerald-600 data-[state=active]:text-emerald-700"
                  >
                ...
                <p className="text-[9px] text-muted-foreground italic leading-tight">
                  ...
                  <strong>
                    {detailedTab === 'all'
                      ...
                      : detailedTab === 'risks'
                        ? risksOnly.length
                        : opportunitiesOnly.length}
                  </strong>{' '}
                  {detailedTab === 'all' ? 'total entries' : detailedTab === 'risks' ? 'risks' : 'opportunities'}{' '}
                  matching the current criteria. Use the filters above to refine the list by campus, unit, risk
                  magnitude, or type.
                </p>
              </div>
            </CardFooter>
        ...
        <DialogContent className="max-w-3xl h-[80dvh] flex flex-col p-0 overflow-hidden shadow-2xl border-none">
          <div className="p-6 border-b bg-slate-50 dark:bg-slate-800/50 shrink-0">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Identified identical descriptions. Use the inline confirmation to resolve redundant records.
            </p>
          </div>
            ...
            <div className="p-8 space-y-8">
              {duplicateGroups.length > 0 ? (
                              ...
                              <div className="flex items-center gap-3">
                                <Badge
                                  ...
                                  className={cn(
                                    ...
                                    isConfirming ? 'bg-white/20 text-white' : 'bg-primary/5 text-primary',
                                  )}
                                >
                                  {risk.type}
                                </Badge>
                                <div className="flex flex-col">
                                  <span
                                    className={cn(
                                      'text-[10px] font-black uppercase tracking-tighter tabular-nums',
                                      isConfirming ? 'text-white' : 'text-slate-700 dark:text-slate-300',
                                    )}
                                  >
                                    LOG ID: {risk.id.substring(0, 8)}
                                  </span>
                                  <span
                                    className={cn(
                                      'text-[9px] font-medium italic',
                                      isConfirming ? 'text-white/80' : 'text-muted-foreground',
                                    )}
                                  >
                                    Logged by: {risk.responsiblePersonName || 'Personnel'}
                                  </span>
                                </div>
                              </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <h4 className="text-lg font-black uppercase text-slate-800 dark:text-slate-200">
                      Registry Integrity Verified
                    </h4>
                    <p className="text-sm text-muted-foreground max-w-xs font-medium">
                      No redundant descriptions detected for the current unit and fiscal year.
                    </p>
                  </div>
                </div>
              )}
          ...
          <DialogFooter className="p-6 border-t bg-slate-50 dark:bg-slate-800/50 shrink-0">
            <div className="flex w-full items-center justify-between">
              ...
              <Button
                variant="outline"
                size="sm"
                className="h-9 px-6 font-black uppercase text-[10px] tracking-widest bg-white"
                onClick={() => setIsDuplicateAuditOpen(false)}
              >
                Close Audit
              </Button>
            </div>
