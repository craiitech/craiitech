...
export default function GadCornerPage() {
  ...
  const gadSettingsRef = useMemoFirebase(
    () => (firestore && !isUserLoading && userProfile ? doc(firestore, 'system', 'gadSettings') : null),
  ...
  const { data: compliances, isLoading: isLoadingCompliances } =
  ...
  return (
    <div className="space-y-6">
      <Tabs value={currentTab} onValueChange={handleTabChange} className="space-y-6">
        ...
        <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-md pt-2 pb-4 -mx-4 px-4 lg:-mx-8 lg:px-8 border-b space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className="bg-primary/5 text-primary border-primary/20 font-black uppercase text-[10px] max-w-[200px] truncate"
                >
                  {selectedUnitId === 'all'
          ...
          <ScrollArea className="w-full">
            <TabsList className="bg-muted p-1 border shadow-sm flex lg:inline-flex animate-tab-highlight rounded-md whitespace-nowrap min-w-max w-max">
              <TabsTrigger value="overview" className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8">
              ...
              >
                <CalendarCheck className="h-4 w-4" /> Event Registry
              </TabsTrigger>
              <TabsTrigger value="gpb" className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8">
                <Target className="h-4 w-4" /> GAD Plan & Budget (GPB)
              </TabsTrigger>
              <TabsTrigger value="ar" className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8">
                <FileText className="h-4 w-4" /> Accomplishment Report (AR)
              </TabsTrigger>
              <TabsTrigger value="sdd" className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8">
              ...
              <TabsTrigger
                ...
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8"
              >
                <Target className="h-4 w-4" /> Projects Registry
              </TabsTrigger>
              <TabsTrigger
                value="mainstreaming"
                className="gap-2 text-[10px] font-black uppercase tracking-widest px-6 h-8"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-0.5">
                  <p className="text-[10px] font-black uppercase text-primary tracking-widest">
                    Institutional GAD Registry Settings
                  </p>
                  <p className="text-[9px] text-muted-foreground font-medium italic">
                    Configure the university-wide annual budget baseline.
                  </p>
                </div>
              </div>
              <div className="flex-1 max-w-sm space-y-1">
                <Label className="text-[10px] font-bold uppercase text-slate-600 dark:text-slate-400">
                  Total University Appropriations (Annual)
                </Label>
                <div className="flex gap-2">
                  <Input
                    ...
                    onBlur={(e) =>
                      isAdmin &&
                      gadSettingsRef &&
                      setDoc(gadSettingsRef, { institutionalTotalBudget: Number(e.target.value) }, { merge: true })
                    }
                  />
                  <Button
                    disabled
                    variant="outline"
                    className="h-11 px-6 font-black uppercase text-[10px] border-none bg-white"
                  >
