...
export default function ProfilePage() {
  ...
  const computedPointsData = useMemo(() => {
    ...
    const firstRorSub = yearSubmissions.find(
      (s) => s.cycleId === 'first' && s.reportType === 'Risk and Opportunity Registry',
    );
    const finalRorSub = yearSubmissions.find(
      ...
      return submissionTypes.map((type) => {
        ...
        return {
          ...
          deadlineDate: cycleDeadline
            ? cycleDeadline.endDate?.toDate
              ? cycleDeadline.endDate.toDate()
              : cycleDeadline.endDate
            : null,
        };
      });
    };
    ...
    for (let i = 0; i < 12; i++) {
      if (i < fullStars) {
        stars.push(
          <Star
            key={i}
            className="h-5 w-5 text-yellow-400 fill-yellow-400 drop-shadow-sm transition-transform hover:scale-110 duration-200"
          />,
        );
        ...
        stars.push(<Star key={i} className="h-5 w-5 text-slate-200 fill-slate-200" />);
      }
    }
    return (
      <div className="flex gap-1.5 items-center justify-center py-4 bg-slate-50 dark:bg-slate-800/50 border rounded-2xl shadow-inner">
        {stars}
      </div>
    );
  };
  ...
  const hasPendingChanges = form.formState.isDirty || passwordForm.formState.isDirty || deleteForm.formState.isDirty;
  ...
  const handleDeleteAccount = async (values: z.infer<typeof deleteAccountSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Your personal data and access have been removed. Registry logs were preserved for audit.',
      });
      router.push('/');
    } catch (error: any) {
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-[9px]">
                        Used for institutional Gender and Development (GAD) reporting.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Card id="accessibility" className="shadow-md border-primary/10 flex flex-col scroll-mt-20">
              <CardHeader className="bg-primary/5 border-b">
                ...
                <CardDescription className="text-xs">
                  Customize the interface to suit your visual and cognitive needs.
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-4 p-4 rounded-lg border bg-muted/5 shadow-sm">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Adjust the system-wide text size for optimal readability.
                    </p>
                  </div>
                  <div className="pt-2 px-2">
                    <Slider
                      ...
                      onValueChange={(vals) =>
                        form.setValue('accessibility.fontSize', fontSizeMap[vals[0]], { shouldDirty: true })
                      }
                      disabled={!canEdit}
                    />
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3 p-4 rounded-lg border bg-muted/5 shadow-sm">
                      ...
                      <FormDescription className="text-[10px]">
                        Choose a primary color scheme for your dashboard experience.
                      </FormDescription>
                      <FormControl>
                        <RadioGroup
                          onValueChange={field.onChange}
                        ...
                        >
                          {colorPalettes.map((palette) => (
                            <div key={palette.id} className="flex items-center space-x-2">
                              ...
                              >
                                <div
                                  className={cn('h-4 w-4 rounded-full border border-white/20 shadow-sm', palette.color)}
                                />
                                <span
                                  className={cn(
                                    'text-[10px] font-black uppercase tracking-tight',
                                    field.value === palette.id ? 'text-primary' : 'text-muted-foreground',
                                  )}
                                >
                                  {palette.label}
                                </span>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between rounded-lg border p-4 shadow-sm bg-muted/5">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription className="text-[10px]">
                          Increases readability by sharpening colors and border definitions.
                        </FormDescription>
                      </div>
                      <FormControl>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between rounded-lg border p-4 shadow-sm bg-muted/5">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription className="text-[10px]">
                          Optimizes character spacing and line height for improved cognitive flow.
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch checked={field.value} onCheckedChange={field.onChange} disabled={!canEdit} />
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between rounded-lg border p-4 shadow-sm bg-muted/5">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription className="text-[10px]">
                          Disables UI animations and transitions to prevent vestibular triggers.
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch checked={field.value} onCheckedChange={field.onChange} disabled={!canEdit} />
                ...
                <div className="p-4 rounded-lg bg-blue-50 border border-blue-100 mt-4">
                  <div className="flex gap-3">
                    ...
                    <p className="text-[10px] text-blue-800 leading-relaxed italic">
                      <strong>Inclusivity Standard:</strong> These settings are persistent across devices and sessions,
                      ensuring a consistent and accessible environment for all institutional users as mandated by RSU
                      EOMS accessibility protocols.
                    </p>
                  </div>
                </div>
              </CardContent>
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <CardDescription className="text-[10px] font-bold uppercase text-muted-foreground">
                    Compliance stars performance registry
                  </CardDescription>
                </div>
              </div>
                    ...
                    <div className="border rounded-xl bg-white overflow-hidden shadow-sm">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant="secondary"
                            className="h-5 text-[9px] font-black bg-primary/10 text-primary border-none"
                          >
                            {computedPointsData.breakdown.first
                      ...
                      {isFirstCycleExpanded && (
                        <div className="border-t divide-y bg-slate-50/[0.3] p-2 space-y-1">
                          {computedPointsData.breakdown.first.map((item: any, idx: number) => (
                            <div
                              key={idx}
                              className="p-3 bg-white rounded-lg border shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3 transition-all hover:border-slate-300"
                            >
                              <div className="space-y-1 flex-1 min-w-0">
                                ...
                                <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-muted-foreground">
                                  <span className="flex items-center gap-1">
                                    <Calendar className="h-3 w-3" /> Submitted: {formatDate(item.submissionDate)}
                                  </span>
                                  <span className="flex items-center gap-1">
                                    <Calendar className="h-3 w-3" /> Deadline: {formatDate(item.deadlineDate)}
                                  </span>
                                </div>
                              </div>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            className="h-5 text-[9px] font-black bg-primary/10 text-primary border-none"
                          >
                            {computedPointsData.breakdown.final
                              .reduce((sum: number, item: any) => sum + item.points, 0)
                              .toFixed(1)}{' '}
                            / 6.0 Stars
                          </Badge>
          ...
          <Card className="shadow-md border-primary/10 h-fit">
            <CardHeader className="bg-primary/5 border-b">
              ...
              <CardDescription className="text-xs">
                Establish a new secure password for your institutional portal access.
              </CardDescription>
            </CardHeader>
                ...
                <form onSubmit={passwordForm.handleSubmit(handlePasswordUpdate)} className="space-y-5">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase text-slate-500">
                          Confirm Identity: Current Password
                        </FormLabel>
                        <FormControl>
                  ...
                  <div className="p-4 rounded-xl border border-dashed bg-muted/5 space-y-2">
                    ...
                    <p className="text-[10px] text-slate-500 leading-relaxed italic">
                      Updating your password requires a recent login. If you encounter a security timeout, please sign
                      out and sign back in before attempting this update again.
                    </p>
                  </div>
          ...
          <Card className="border-amber-200 bg-amber-50/10">
            ...
            <CardContent className="space-y-4">
              <p className="text-xs text-amber-800 leading-relaxed font-medium">
                Your account is currently verified for the <strong>{unitName}</strong> office at the{' '}
                <strong>{campusName}</strong>.
              </p>
              <p className="text-[10px] text-amber-700/70 italic">
                If you need to change your assigned Unit or Role, please contact the Quality Assurance Office directly
                as these details are locked for audit integrity.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-destructive/20 bg-destructive/5 overflow-hidden">
            <CardHeader className="bg-destructive/10 border-b py-4">
              ...
              <CardDescription className="text-xs">
                Irreversible actions for your institutional account.
              </CardDescription>
            </CardHeader>
              ...
              <div className="flex flex-col gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-muted-foreground leading-relaxed">
                    Once deleted, you will no longer have access to the RSU EOMS Portal. Your personal profile data and
                    login credentials will be removed.
                  </p>
                </div>

                <Alert className="bg-white border-primary/20">
                  <ShieldCheck className="h-4 w-4 text-primary" />
                  <AlertTitle className="text-[10px] font-black uppercase tracking-tight text-primary">
                    Data Privacy Provision (RA 10173)
                  </AlertTitle>
                  <AlertDescription className="text-[10px] font-medium leading-tight text-slate-600 dark:text-slate-400">
                    In accordance with the <strong>Data Privacy Act of 2012 (RA 10173)</strong>, specifically the{' '}
                    <em>Right to Erasure or Blocking</em>, you may request the removal of your personal identity data
                    from this system. Note that institutional evidence (submissions) is retained for legal and auditing
                ...
                <Alert variant="destructive" className="bg-white border-destructive/20">
                  ...
                  <AlertTitle className="text-[10px] font-black uppercase tracking-tight">
                    Institutional Persistence Note
                  </AlertTitle>
                  <AlertDescription className="text-[10px] font-medium leading-tight">
                    To maintain institutional audit integrity, all documents you have <strong>submitted</strong> or{' '}
                    <strong>approved</strong> will remain in the university registry linked to your institutional
                    identity string.
                  </AlertDescription>
                </Alert>
                ...
                <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                  <AlertDialogTrigger asChild>
                    <Button
                      variant="destructive"
                      className="font-black uppercase text-[10px] tracking-widest shadow-lg shadow-destructive/20 h-11"
                    >
                    ...
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="space-y-4">
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                          Are you absolutely sure you want to delete your institutional account?
                        </p>
                        <p className="text-xs text-muted-foreground leading-relaxed italic">
                          This action will remove your profile and authentication record. Your submissions will be
                          preserved in the RSU EOMS Registry for quality auditing compliance.
                        </p>
