...
import { collection, addDoc, serverTimestamp, query, where, doc } from '@/firebase/firestore-wrapper';
  ...
  const onSubmit = async (values: z.infer<typeof sddSchema>) => {
    ...
    if (!settings?.isPublicEntryEnabled) {
      toast({
        ...
        description: 'Institutional GAD data collection is currently closed.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (totalSectorMale > values.male || totalSectorFemale > values.female) {
      toast({
        ...
        description: 'Total sector-based participants cannot exceed the grand total per sex identification.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (values.male + values.female === 0) {
      toast({
        ...
        description: 'Please provide at least one participant count.',
        variant: 'destructive',
      });
      return;
    }
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-800/50">
        <Card className="max-w-md text-center shadow-xl">
          <CardHeader>
            ...
            <CardTitle className="text-xl font-black uppercase text-slate-800 dark:text-slate-200">
              GAD Registry Closed
            </CardTitle>
            <CardDescription>
              Public GAD data collection is currently offline. Please contact the Quality Assurance Office for
              assistance.
            </CardDescription>
          </CardHeader>
        </Card>
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-4 py-20 overflow-x-hidden">
      <div className="fixed inset-0 -z-10 h-full w-full">
        <Image src="/rsulogo.png" alt="RSU" fill unoptimized className="object-cover opacity-10" />
        ...
        <Card className="shadow-2xl border-none overflow-hidden bg-white/95">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription className="text-xs font-bold uppercase tracking-widest">
                  Secure Device Identification Active
                </CardDescription>
              </div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3 border-b pb-2">
                    ...
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-800 dark:text-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-bold uppercase">Official Activity Code</FormLabel>
                          <FormControl>
                            <Input
                              {...field}
                              placeholder="e.g. QAO-2025-001"
                              className="bg-slate-50 dark:bg-slate-800/50 font-mono font-bold"
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-bold uppercase">Full Title of Activity</FormLabel>
                          <FormControl>
                            <Input
                              {...field}
                              placeholder="e.g. Sensitivity Workshop..."
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-bold uppercase">Implementing Office</FormLabel>
                          <Select onValueChange={field.onChange} value={field.value} disabled={!watchCampusId}>
                            <FormControl>
                              <SelectTrigger className="h-11 bg-slate-50 dark:bg-slate-800/50">
                                <SelectValue placeholder={watchCampusId ? 'Select Office' : 'Select Campus First'} />
                              </SelectTrigger>
                            </FormControl>
                            <SelectContent>
                              {units
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3 border-b pb-2">
                    ...
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-800 dark:text-slate-200">
                      2. Participant Headcount (Sex-Disaggregated)
                    </h4>
                  </div>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="p-6 rounded-2xl bg-indigo-50 border border-indigo-100 flex flex-col items-center">
                          <FormLabel className="text-xs font-black uppercase text-indigo-700 mb-4">
                            Total Male
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                              {...field}
                              className="h-14 text-3xl font-black text-center bg-white border-none shadow-inner"
                            />
                          </FormControl>
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="p-6 rounded-2xl bg-rose-50 border border-rose-100 flex flex-col items-center">
                          <FormLabel className="text-xs font-black uppercase text-rose-700 mb-4">
                            Total Female
                          </FormLabel>
                          <FormControl>
                            <Input
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {sectors.map((sector) => (
                      <div
                        key={sector}
                        className="p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col gap-4 group hover:border-primary/20 transition-all"
                      >
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-black uppercase text-slate-600 dark:text-slate-400">{sector}</p>
                          <Badge
                            variant="secondary"
                            className="h-5 text-[10px] font-black bg-white border-none shadow-sm"
                          >
                            {watchSectors[sector]?.male + watchSectors[sector]?.female} TOTAL
                          </Badge>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <FormField
                            control={form.control}
                            name={`sectors.${sector}.male`}
                            render={({ field }) => (
                              <div className="space-y-1">
                                ...
                                <Input
                                  type="number"
                                  {...field}
                                  className="h-8 text-xs font-black text-center bg-white"
                ...
                <div className="space-y-4 pt-4 border-t border-dashed">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-800 dark:text-slate-200">
                      4. Evidences & Documentation (Google Drive)
                    </h4>
                  </div>
                  <FormField
                    control={form.control}
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            ...
                            className="h-12 bg-blue-50/20 border-blue-200 font-mono text-xs font-medium"
                          />
                        </FormControl>
                        <FormDescription className="text-[10px] italic text-muted-foreground">
                          Paste public Google Drive URL containing event photos, signed attendance sheets, or
                          verification reports. Saved directly to Firestore.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <div className="p-6 rounded-2xl bg-blue-50 border border-blue-100 flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-blue-800/80 leading-relaxed font-medium italic">
                      Your input is utilized strictly for GAD Accomplishment Reporting (GAD AR) as mandated by the
                      Philippine Commission on Women. Total participants per sector must not exceed the total headcount
                ...
                <Button
                  ...
                  className="w-full h-16 text-lg font-black uppercase tracking-widest shadow-2xl shadow-primary/30 animate-in slide-in-from-bottom-4"
                >
                  {isSubmitting ? (
          ...
          <CardFooter className="bg-muted/30 border-t py-6 px-10 text-center flex flex-col gap-2">
            <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">
              &copy; 2025 Romblon State University - Quality Assurance Office
            </p>
            <p className="text-[8px] font-bold text-muted-foreground/40 uppercase tracking-tighter">
              Integrated GAD Digital System v2.0
            </p>
          </CardFooter>
        </Card>
      </div>
