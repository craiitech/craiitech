    ...
    <>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark
            key={i}
            className="bg-yellow-100 text-slate-900 dark:text-slate-100 rounded font-black px-0.5 select-none"
          >
            {part}
          </mark>
        ) : (
  ...
  const filteredQuestions = useMemo(() => {
    return allQuestions.filter((faq) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesBlocks = faq.answerBlocks
          ? faq.answerBlocks.some((b: any) => b.content.toLowerCase().includes(query))
          : false;
      ...
      <CardHeader className="bg-primary/5 border-b py-8 px-6 sm:px-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <h2 className="text-2xl font-black uppercase text-slate-800 dark:text-slate-200 tracking-tight">
              EOMS Support Desk
            </h2>
                ...
                >
                  <AccordionTrigger className="text-xs font-black uppercase tracking-tight text-slate-800 dark:text-slate-200 hover:no-underline py-4">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-left">
                      ...
                      <Badge
                        variant="outline"
                        className="w-fit border-none bg-primary/5 text-primary text-[8px] font-black uppercase tracking-widest leading-none py-1"
                      >
                        {faq.category}
                      </Badge>
                    </div>
                  ...
                  <AccordionContent className="pb-4 pt-1">
                    <div className="prose prose-sm max-w-none text-muted-foreground text-xs leading-relaxed font-semibold italic space-y-3">
                      {faq.answer && (
                        <p className="text-slate-600 dark:text-slate-400">"{highlightText(faq.answer, searchQuery)}"</p>
                      )}
                      {faq.answerBlocks && (
            ...
            <div className="p-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl space-y-4 max-w-md mx-auto">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground italic font-medium">
                  "We couldn't find any FAQs or instructions matching '{searchQuery}'."
                </p>
              </div>
              <Button
                onClick={openChatbot}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold text-muted-foreground uppercase mt-0.5">
              Contact QAO romblon state university
            </p>
          </div>
        </div>
