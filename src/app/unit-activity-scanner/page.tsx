...
function UnitActivityScannerTerminal() {
      ...
      try {
        ...
        scanner
          ...
          .catch((err: any) => {
            ...
            setScanResult({
              status: 'error',
              message: `Camera access failed: ${err.message || err}. Please ensure camera permission is granted.`,
            });
            setScannerActive(false);
          });
      } catch (err: any) {
  ...
  const activeActivityUnit = activeActivity
    ? units?.find((u) => u.id === activeActivity.unitId)?.name || activeActivity.unitId
    : 'N/A';
  const registrationUrl =
    typeof window !== 'undefined' && activeActivity
    ...
    <div className="relative w-full h-dvh overflow-hidden bg-[#0d2a18] font-sans text-white flex flex-col">
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none z-0">
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(135deg, rgba(13,42,24,0.80) 0%, rgba(10,30,18,0.72) 50%, rgba(18,48,30,0.80) 100%)',
          }}
        />
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(circle, rgba(27,101,53,0.25) 0%, transparent 70%)',
      ...
      {/* ================================================================== */}
      <header
        className="relative z-20 flex items-center justify-between px-5 py-3 shrink-0 animate-in slide-in-from-top duration-300"
        style={{
        ...
        <section className="flex-1 flex flex-col p-4 gap-4 overflow-hidden min-w-0">
          ...
          <div className="shrink-0 animate-in slide-in-from-left duration-300">
            {scanResult.status === 'none' ? (
              ...
              <div
                className="relative rounded-2xl flex items-center gap-8 p-6 overflow-hidden"
                style={{
              ...
              >
                ...
                <div
                  ...
                  style={{ background: 'radial-gradient(circle, rgba(27,101,53,0.10) 0%, transparent 70%)' }}
                />
                ...
                <div className="flex-1 flex flex-col items-start text-left">
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-4 w-full max-w-2xl">
                    {[
                      { label: 'Total Present', val: sortedLogs.length, color: 'text-[#D4AF37]' },
                      {
                        label: 'On Time',
                        val: sortedLogs.filter((l) => l.status === 'ON_TIME').length,
                        color: 'text-emerald-400',
                      },
                      {
                        label: 'Late',
                        val: sortedLogs.filter((l) => l.status === 'LATE').length,
                      ...
                      >
                        <span className={`block text-xl sm:text-2xl lg:text-3xl font-black ${color} tracking-tight`}>
                          {val}
                        </span>
                        <span className="block text-[7px] sm:text-[8px] font-black text-slate-400 uppercase tracking-widest text-center mt-0.5">
                          {label}
                        </span>
                      </div>
                    ))}
              ...
              <div
                className={`relative rounded-2xl flex items-center p-6 transition-all duration-300 overflow-hidden ${
                ...
                style={{
                  ...
                  boxShadow:
                    scanResult.status === 'success'
                      ? '0 0 40px rgba(52,211,153,0.12), inset 0 1px 0 rgba(255,255,255,0.10)'
                      : scanResult.status === 'warning'
                        ? '0 0 40px rgba(251,191,36,0.10), inset 0 1px 0 rgba(255,255,255,0.10)'
                        : '0 0 40px rgba(248,113,113,0.12), inset 0 1px 0 rgba(255,255,255,0.08)',
                }}
              >
                {/* Animated glow pulse ring */}
                <div
                  className={`absolute -inset-px rounded-2xl pointer-events-none animate-pulse opacity-30`}
                  style={{
                    border:
                      scanResult.status === 'success'
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 w-full z-10">
                  ...
                  <div className="shrink-0">
                    {scanResult.status === 'success' ? (
                      <div
                        className="h-24 w-24 rounded-2xl flex items-center justify-center text-emerald-300 animate-bounce"
                        style={{
                      ...
                      <div
                        className="h-24 w-24 rounded-2xl flex items-center justify-center text-amber-300 animate-pulse"
                        style={{
                      ...
                      <div
                        className="h-24 w-24 rounded-2xl flex items-center justify-center text-rose-300 animate-shake"
                        style={{
                  ...
                  <div className="flex-1 min-w-0 text-center sm:text-left space-y-2">
                    <span
                      className={`inline-block text-[9px] sm:text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full ${
                        scanResult.status === 'success'
                          ? 'text-emerald-300'
                          : scanResult.status === 'warning'
                            ? 'text-amber-300'
                    ...
                    {scanResult.details ? (
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1.5 pt-2 text-[10px] sm:text-xs font-black text-slate-300 uppercase tracking-widest">
                          ...
                          <span
                            ...
                            style={{ background: 'rgba(0,0,0,0.30)', border: '1px solid rgba(255,255,255,0.08)' }}
                          >
                            <Users className="h-3.5 w-3.5 text-slate-400" /> Log type:{' '}
                            <strong
                              className={scanResult.details.status === 'LOGOUT' ? 'text-cyan-300' : 'text-emerald-300'}
                            >
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-slate-200 font-bold max-w-2xl leading-relaxed">
                          {scanResult.message}
                        </p>
                      </div>
                    )}
                ...
                <div
                  className="absolute bottom-0 left-0 right-0 h-1.5 overflow-hidden rounded-b-2xl"
          ...
          {/* ---- CHECKED-IN REGISTRY ---- */}
          <div
            className="flex-1 min-h-0 flex flex-col p-4 overflow-hidden relative animate-in slide-in-from-bottom duration-300 rounded-2xl"
            style={{
              background: 'rgba(255,255,255,0.05)',
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-[10px] font-black uppercase text-white tracking-wider">
                    Checked-In Registry Logs
                  </h3>
                  ...
                  {sortedLogs.map((log) => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className="font-extrabold text-[11px] text-white uppercase truncate leading-tight">
                          {log.userName}
                        </p>
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 py-1">
                {activeCode.split('').map((char, index) => (
                  <div
                    key={index}
                    className="w-8 h-9 rounded-lg flex items-center justify-center text-sm font-black text-[#D4AF37] font-mono"
              ...
              <p className="text-[7px] font-bold text-slate-400 text-center uppercase tracking-wide">
      ...
      {!paramActivityId && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/85 backdrop-blur-sm text-center p-8 space-y-4">
          ...
          <p className="text-sm font-bold text-slate-400 max-w-sm leading-relaxed uppercase">
            No activity session detected. Please select an active session from the Unit Activity manager and click Open
            Scanner.
          </p>
        </div>
      )}
      ...
      {!isFullscreen && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-950/95 backdrop-blur-md p-6 text-center space-y-6 animate-in fade-in duration-300">
          ...
          <div className="space-y-2 max-w-md">
            <h3 className="text-2xl font-black uppercase tracking-tight text-white">Scanner Terminal Halted</h3>
            ...
            <p className="text-sm font-medium text-slate-300 pt-2 leading-relaxed">
              For security and strict physical lock validation, the visitor/participant QR scanner terminal must operate
              in fullscreen mode.
            </p>
          </div>
